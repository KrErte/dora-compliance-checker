...
/**
 * Simple in-memory rate limiter per IP address. Limits requests to AI-powered endpoints to prevent
 * API cost abuse.
 */
@Component
@Order(1)
public class RateLimitFilter implements Filter {
  ...
  private static final String[] AI_PATHS = {
    "/api/contracts/analyze", "/api/v2/assessments", "/api/clause-rewriter"
  };
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    if (isAiPath(path)) {
      TokenBucket aiBucket =
          aiBuckets.computeIfAbsent(clientIp, k -> new TokenBucket(maxAiRequestsPerMinute));
      if (!aiBucket.tryConsume()) {
        ...
        httpRes
            .getWriter()
            .write(
                "{\"error\":\"Rate limit exceeded. AI analysis limited to "
    ...
    if (path.startsWith("/api/")) {
      TokenBucket bucket =
          buckets.computeIfAbsent(clientIp, k -> new TokenBucket(maxRequestsPerMinute));
