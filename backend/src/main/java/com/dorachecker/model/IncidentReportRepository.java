...
public interface IncidentReportRepository extends JpaRepository<IncidentReportEntity, String> {
  ...
  List<IncidentReportEntity> findByUserIdAndReportingStatusOrderByCreatedAtDesc(
      String userId, String reportingStatus);

  @Query(
      "SELECT i FROM IncidentReportEntity i WHERE i.userId = :userId AND i.isMajor = true ORDER BY i.createdAt DESC")
  List<IncidentReportEntity> findMajorIncidents(String userId);

  @Query(
      "SELECT i FROM IncidentReportEntity i WHERE i.reportingStatus NOT IN ('CLOSED', 'FINAL_SENT') "
