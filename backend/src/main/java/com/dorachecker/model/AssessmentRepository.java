...
public interface AssessmentRepository extends JpaRepository<AssessmentEntity, String> {
  ...
  @Query(
      value =
          "SELECT percentile_cont(0.5) WITHIN GROUP (ORDER BY score_percentage) FROM assessments",
      nativeQuery = true)
  Double findMedianScore();

  @Query(
      value =
          "SELECT percentile_cont(0.25) WITHIN GROUP (ORDER BY score_percentage) FROM assessments",
      nativeQuery = true)
  Double findPercentile25();

  @Query(
      value =
          "SELECT percentile_cont(0.75) WITHIN GROUP (ORDER BY score_percentage) FROM assessments",
      nativeQuery = true)
