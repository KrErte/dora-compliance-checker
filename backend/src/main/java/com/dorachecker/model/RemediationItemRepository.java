...
public interface RemediationItemRepository extends JpaRepository<RemediationItemEntity, String> {
  ...
  @Transactional
  @Query(
      "UPDATE RemediationItemEntity r SET r.status = :status, r.updatedAt = CURRENT_TIMESTAMP WHERE r.id IN :ids AND r.userId = :userId")
  ...
  @Transactional
  @Query(
      "UPDATE RemediationItemEntity r SET r.priority = :priority, r.updatedAt = CURRENT_TIMESTAMP WHERE r.id IN :ids AND r.userId = :userId")
  int bulkUpdatePriority(
      @Param("ids") List<String> ids,
      @Param("userId") String userId,
  ...
  List<RemediationItemEntity> findByDueDateBetweenAndStatusNot(
