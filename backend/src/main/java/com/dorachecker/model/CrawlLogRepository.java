...
public interface CrawlLogRepository extends JpaRepository<CrawlLogEntity, Long> {
  ...
  @Query(
      "SELECT c FROM CrawlLogEntity c WHERE c.companyProfile.id = :profileId AND c.source = :source ORDER BY c.crawledAt DESC")
  List<CrawlLogEntity> findByProfileAndSource(
      @Param("profileId") Long profileId, @Param("source") String source);
