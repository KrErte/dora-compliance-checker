...
public interface AiActObligationRepository extends JpaRepository<AiActObligationEntity, String> {
  ...
  @Query(
      "SELECT o.status, COUNT(o) FROM AiActObligationEntity o WHERE o.aiSystemId = :aiSystemId GROUP BY o.status")
  ...
  @Query(
      "SELECT COUNT(o) FROM AiActObligationEntity o WHERE o.userId = :userId AND o.status = 'COMPLETED'")
