...
/**
 * Initializes regulation data (DORA, NIS2) on application startup. Only seeds data if the
 * regulations table is empty.
 */
@Component
public class RegulationDataInitializer implements CommandLineRunner {
  ...
  public RegulationDataInitializer(
  ...
  public void run(String... args) {
    ...
    log.info(
        "RegulationDataInitializer: Seeding complete - {} regulations, {} domains, {} questions",
        regulationRepository.count(),
        domainRepository.count(),
        questionRepository.count());
  }

  private void seedDora() {
    RegulationEntity dora =
        new RegulationEntity(
            "DORA",
    ...
    // DORA Domains
    ComplianceDomainEntity serviceLevel =
        createDomain(dora, "SERVICE_LEVEL", "Service Level", "Teenustase", 1);
    ComplianceDomainEntity exitStrategy =
        createDomain(dora, "EXIT_STRATEGY", "Exit Strategy", "Väljumisstrateegia", 2);
    ComplianceDomainEntity audit =
        createDomain(dora, "AUDIT", "Audit Rights", "Auditeerimisõigused", 3);
    ComplianceDomainEntity incident =
        createDomain(dora, "INCIDENT", "Incident Notification", "Intsidentidest teavitamine", 4);
    ComplianceDomainEntity data = createDomain(dora, "DATA", "Data Location", "Andmete asukoht", 5);
    ComplianceDomainEntity subcontracting =
        createDomain(dora, "SUBCONTRACTING", "Subcontracting", "Allhange", 6);
    ComplianceDomainEntity risk = createDomain(dora, "RISK", "Risk Management", "Riskihaldus", 7);
    ComplianceDomainEntity legal =
        createDomain(dora, "LEGAL", "Legal Terms", "Juriidilised tingimused", 8);
    ComplianceDomainEntity continuity =
        createDomain(dora, "CONTINUITY", "Business Continuity", "Äritegevuse jätkuvus", 9);
    ComplianceDomainEntity ictRisk =
        createDomain(dora, "ICT_RISK_MANAGEMENT", "ICT Risk Management", "IKT riskihaldus", 10);
    ComplianceDomainEntity incidentMgmt =
        createDomain(dora, "INCIDENT_MANAGEMENT", "Incident Management", "Intsidentide haldus", 11);
    ComplianceDomainEntity testing =
        createDomain(dora, "TESTING", "Resilience Testing", "Vastupidavuse testimine", 12);
    ComplianceDomainEntity infoSharing =
        createDomain(dora, "INFORMATION_SHARING", "Information Sharing", "Teabe jagamine", 13);
    ...
    createQuestion(
        testing,
        "Are TLPT tests conducted for critical ICT systems?",
    ...
    createQuestion(
        infoSharing,
        "Does the entity participate in a cyber threat intelligence sharing community?",
  ...
  private void seedNis2() {
    ...
    ComplianceDomainEntity riskPolicies =
        createDomain(
            nis2,
            "RISK_POLICIES",
            "Risk Analysis & Security Policies",
            "Riskianalüüs ja turvapoliitikad",
            1);
    ComplianceDomainEntity incidentHandling =
        createDomain(nis2, "INCIDENT_HANDLING", "Incident Handling", "Intsidentide käsitlemine", 2);
    ComplianceDomainEntity businessContinuity =
        createDomain(nis2, "BUSINESS_CONTINUITY", "Business Continuity", "Äritegevuse jätkuvus", 3);
    ComplianceDomainEntity supplyChain =
        createDomain(nis2, "SUPPLY_CHAIN", "Supply Chain Security", "Tarneahela turvalisus", 4);
    ComplianceDomainEntity networkSecurity =
        createDomain(
            nis2,
            "NETWORK_SECURITY",
            "Network & System Security",
            "Võrgu- ja süsteemiturvalisus",
            5);
    ComplianceDomainEntity vulnerabilityMgmt =
        createDomain(
            nis2, "VULNERABILITY_MGMT", "Vulnerability Management", "Haavatavuste haldus", 6);
    ComplianceDomainEntity securityAssessment =
        createDomain(
            nis2, "SECURITY_ASSESSMENT", "Security Assessment", "Turvalisuse hindamine", 7);
    ComplianceDomainEntity cryptography =
        createDomain(
            nis2, "CRYPTOGRAPHY", "Cryptography & Encryption", "Krüptograafia ja krüpteerimine", 8);
    ComplianceDomainEntity hrSecurity =
        createDomain(nis2, "HR_SECURITY", "Human Resources Security", "Personaliturvalisus", 9);
    ComplianceDomainEntity accessControl =
        createDomain(
            nis2, "ACCESS_CONTROL", "Access Control & MFA", "Juurdepääsukontroll ja MFA", 10);

    // NIS2 Questions - Risk Policies
    createQuestion(
        riskPolicies,
        "Has the organization established a comprehensive information security risk analysis process?",
        "Kas organisatsioon on kehtestanud põhjaliku infoturberiski analüüsi protsessi?",
        "NIS2 Art. 21(2)(a)",
        1);
    createQuestion(
        riskPolicies,
        "Are information security policies documented and approved by management?",
    ...
    createQuestion(
        incidentHandling,
        "Is there a documented incident handling procedure?",
        "Kas on olemas dokumenteeritud intsidentide käsitlemise protseduur?",
        "NIS2 Art. 21(2)(b)",
        1);
    createQuestion(
        incidentHandling,
        "Can you notify CERT-EE of significant incidents within 24 hours?",
    ...
    createQuestion(
        businessContinuity,
        "Is there a business continuity plan covering critical services?",
        "Kas on olemas äritegevuse jätkuvuse plaan kriitiliste teenuste jaoks?",
        "NIS2 Art. 21(2)(c)",
        1);
    createQuestion(
        businessContinuity,
        "Are critical systems backed up regularly with tested recovery procedures?",
    ...
    createQuestion(
        supplyChain,
        "Is there a process to assess cybersecurity risks in the supply chain?",
    ...
    createQuestion(
        networkSecurity,
        "Is the network segmented to protect critical systems?",
        "Kas võrk on segmenteeritud kriitiliste süsteemide kaitseks?",
        "NIS2 Art. 21(2)(e)",
        1);
    createQuestion(
        networkSecurity,
        "Are network security measures (firewalls, IDS/IPS) in place and maintained?",
    ...
    createQuestion(
        vulnerabilityMgmt,
        "Is there a vulnerability disclosure and handling process?",
        "Kas on olemas haavatavuste avalikustamise ja käsitlemise protsess?",
        "NIS2 Art. 21(2)(f)",
        1);
    createQuestion(
        vulnerabilityMgmt,
        "Are systems regularly scanned for vulnerabilities and patched?",
    ...
    createQuestion(
        securityAssessment,
        "Are cybersecurity measures regularly tested for effectiveness?",
    ...
    createQuestion(
        cryptography,
        "Is there a cryptographic key management process?",
    ...
    createQuestion(
        hrSecurity,
        "Is there a security awareness training program for all employees?",
        "Kas on olemas turvateadlikkuse koolitusprogramm kõigile töötajatele?",
        "NIS2 Art. 21(2)(i)",
        1);
    createQuestion(
        hrSecurity,
        "Are background checks performed for employees with access to critical systems?",
    ...
    createQuestion(
        accessControl,
        "Is there a privileged access management system?",
  ...
  private ComplianceDomainEntity createDomain(
      RegulationEntity regulation, String code, String nameEn, String nameEt, int order) {
    ComplianceDomainEntity domain =
        new ComplianceDomainEntity(regulation, code, nameEn, nameEt, order);
  ...
  private void createQuestion(
      ...
      String articleRef,
      int order) {
    ComplianceQuestionEntity question =
        new ComplianceQuestionEntity(domain, questionEn, questionEt, articleRef, order);
