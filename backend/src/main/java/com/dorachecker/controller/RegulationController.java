...
public class RegulationController {
  ...
  /** GET /api/regulations List all active regulations. */
  @GetMapping
  public List<RegulationDto> getAllRegulations() {
    return regulationService.getAllActive().stream().map(this::toDto).collect(Collectors.toList());
  }

  /** GET /api/regulations/{code} Get a specific regulation by code. */
  @GetMapping("/{code}")
  ...
  /** GET /api/regulations/{code}/domains Get all domains for a regulation. */
  ...
  public ResponseEntity<List<DomainDto>> getDomains(@PathVariable String code) {
    ...
    List<DomainDto> domains =
        regulationService.getDomainsByRegulation(code).stream()
            .map(this::toDomainDto)
            .collect(Collectors.toList());
    return ResponseEntity.ok(domains);
  }

  /** GET /api/regulations/{code}/questions Get all questions for a regulation (flat list). */
  ...
  public ResponseEntity<List<QuestionDto>> getQuestions(@PathVariable String code) {
    ...
    List<QuestionDto> questions =
        regulationService.getQuestionsByRegulation(code).stream()
  ...
  /**
   * GET /api/regulations/{code}/full Get regulation with domains and questions (for assessment
   * page).
   */
        ...
        .map(
            r -> {
              List<ComplianceDomainEntity> domains =
                  regulationService.getDomainsWithQuestions(code);
  ...
  private RegulationFullDto toFullDto(
      RegulationEntity regulation, List<ComplianceDomainEntity> domains) {
    List<DomainWithQuestionsDto> domainDtos =
        domains.stream()
            .map(
                d ->
                    new DomainWithQuestionsDto(
                        d.getId(),
                        d.getCode(),
                        d.getNameEn(),
                        d.getNameEt(),
