...
public class AiActObligationController {
  ...
  public ResponseEntity<?> getObligations(
      @PathVariable String aiSystemId, Authentication authentication) {
  ...
  public ResponseEntity<?> updateObligation(
    ...
    var opt =
        obligationService.updateObligation(
            obligationId,
            aiSystemId,
            request.status(),
            request.notes(),
            ...
            request.dueDate());
    return opt.map(o -> ResponseEntity.ok(toDto(o))).orElse(ResponseEntity.notFound().build());
  }
