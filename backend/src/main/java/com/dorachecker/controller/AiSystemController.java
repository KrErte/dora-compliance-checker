...
public class AiSystemController {
    ...
    public ResponseEntity<?> list(
        ...
        Page<AiSystemEntity> result = aiSystemService.listAiSystems(userId, search, riskLevel, status, deploymentContext, page, size);
    ...
    public ResponseEntity<?> get(@PathVariable String id, Authentication authentication) {
        ...
        String userId = (String) authentication.getPrincipal();
        return aiSystemService.getAiSystem(id, userId)
                .map(s -> ResponseEntity.ok(toDto(s)))
                .orElse(ResponseEntity.notFound().build());
    }

    @PostMapping
    public ResponseEntity<?> create(@RequestBody CreateAiSystemRequest request, Authentication authentication) {
        if (authentication == null) {
            return ResponseEntity.status(401).body(Map.of("error", "Not authenticated"));
        }
        String userId = (String) authentication.getPrincipal();
    ...
    @PutMapping("/{id}")
    public ResponseEntity<?> update(@PathVariable String id, @RequestBody UpdateAiSystemRequest request, Authentication authentication) {
        if (authentication == null) {
            return ResponseEntity.status(401).body(Map.of("error", "Not authenticated"));
        }
        String userId = (String) authentication.getPrincipal();
        ...
        if (request.deploymentContext() != null) system.setDeploymentContext(request.deploymentContext());
    ...
    private AiSystemDto toDto(AiSystemEntity s) {
        return new AiSystemDto(
            s.getId(), s.getName(), s.getDescription(), s.getVendor(), s.getVersion(),
            s.getPurpose(), s.getDeploymentContext(), s.getOrganizationRole(),
            s.getStatus(), s.getRiskLevel(), s.getComplianceStatus(), s.getComplianceScore(),
            s.getClassifiedAt(), s.getClassifiedByUserId(),
            s.getCreatedAt(), s.getUpdatedAt()
        );
    }

    public record AiSystemDto(
        String id, String name, String description, String vendor, String version,
        String purpose, String deploymentContext, String organizationRole,
        String status, String riskLevel, String complianceStatus, Integer complianceScore,
        LocalDateTime classifiedAt, String classifiedByUserId,
        LocalDateTime createdAt, LocalDateTime updatedAt
    ) {}

    public record CreateAiSystemRequest(
        String name, String description, String vendor, String version,
        String purpose, String deploymentContext, String organizationRole
    ) {}

    public record UpdateAiSystemRequest(
        String name, String description, String vendor, String version,
        String purpose, String deploymentContext, String organizationRole, String status
    ) {}
}
