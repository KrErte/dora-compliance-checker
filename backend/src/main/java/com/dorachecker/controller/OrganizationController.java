...
public class OrganizationController {
  ...
  public ResponseEntity<OrganizationEntity> create(
      @Valid @RequestBody CreateOrgRequest body, Authentication auth) {
  ...
  public ResponseEntity<OrganizationEntity> update(
      @PathVariable String orgId, @Valid @RequestBody CreateOrgRequest body, Authentication auth) {
    OrganizationEntity org =
        organizationService.update(orgId, body.name(), body.description(), getUserId(auth));
    return ResponseEntity.ok(org);
  }
  ...
  public ResponseEntity<List<Map<String, Object>>> members(
      @PathVariable String orgId, Authentication auth) {
    return ResponseEntity.ok(organizationService.getMembers(orgId, getUserId(auth)));
  }

  @PostMapping("/{orgId}/invite")
  public ResponseEntity<OrganizationInviteEntity> invite(
      @PathVariable String orgId, @Valid @RequestBody InviteRequest body, Authentication auth) {
  ...
  public ResponseEntity<List<OrganizationInviteEntity>> pendingInvites(
      @PathVariable String orgId, Authentication auth) {
  ...
  public ResponseEntity<Void> cancelInvite(
      @PathVariable String orgId, @PathVariable String inviteId, Authentication auth) {
  ...
  public ResponseEntity<Void> removeMember(
      @PathVariable String orgId, @PathVariable String memberId, Authentication auth) {
  ...
  public record InviteRequest(
      @NotBlank @Email @Size(max = 255) String email, @Size(max = 20) String role) {}
