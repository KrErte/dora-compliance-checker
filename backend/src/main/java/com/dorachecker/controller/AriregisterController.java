...
public class AriregisterController {
  ...
  public ResponseEntity<String> autocomplete(
      @RequestParam("q") String query, HttpServletRequest request) {
    ...
    try {
      ...
      return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
          .body(
              "{\"error\": \"Äriregister service unavailable\", \"details\": \""
  ...
  private void cleanupRateLimitMap(long now) {
    ...
    if (rateLimitMap.size() > 1000) {
      rateLimitMap
          .entrySet()
          .removeIf(e -> now - e.getValue().windowStart() > RATE_LIMIT_WINDOW_MS);
    }
  }
