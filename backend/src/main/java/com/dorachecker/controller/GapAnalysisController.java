...
public class GapAnalysisController {
  ...
  public ResponseEntity<?> analyze(
    ...
    if (!subscriptionGuardService.canAccess(
        userId, null, SubscriptionGuardService.Feature.EVIDENCE_GAP_ANALYZER)) {
      return ResponseEntity.status(403)
          .body(Map.of("error", "Evidence Gap Analyzer requires an Enterprise subscription"));
    }
    ...
    List<String> articles =
        Arrays.stream(articleNumbers.split(","))
            .map(String::trim)
            .filter(s -> !s.isEmpty())
            .toList();
    ...
    if (articles.isEmpty()) {
      return ResponseEntity.badRequest()
          .body(Map.of("error", "At least one article number is required"));
    }

    try {
      GapAnalysisResult result =
          gapAnalysisService.analyze(userId, documentTitle, documentCategory, articles, file);
  ...
  @GetMapping("/{id}")
  public ResponseEntity<GapAnalysisResult> getById(
  ...
  public ResponseEntity<List<Map<String, Object>>> getSupportedArticles() {
    List<Map<String, Object>> articles =
        DoraArticleRequirements.ALL_REQUIREMENTS.stream()
            .collect(
  ...
  public ResponseEntity<?> linkEvidence(
  ...
  public ResponseEntity<?> syncTracker(@PathVariable String id, Authentication authentication) {
    ...
    return ResponseEntity.ok(
        Map.of(
            "synced", true, "findingsCount", result.findings().size(), "articlesSynced", synced));
  }
