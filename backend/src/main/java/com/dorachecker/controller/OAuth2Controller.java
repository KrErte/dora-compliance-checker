...
public class OAuth2Controller {
  ...
  @GetMapping("/google")
  public void initiateGoogleAuth(HttpServletResponse response, HttpSession session)
      throws IOException {
  ...
  public void initiateMicrosoftAuth(HttpServletResponse response, HttpSession session)
      throws IOException {
    String state = UUID.randomUUID().toString();
    session.setAttribute("oauth2_state", state);
  ...
  public void googleCallback(
    ...
    try {
      ...
      // Find or create user
      UserEntity user =
          findOrCreateOAuthUser(email, name, googleId, UserEntity.AuthProvider.GOOGLE);
  ...
  public void microsoftCallback(
    ...
    try {
      ...
      // Find or create user
      UserEntity user =
          findOrCreateOAuthUser(email, name, microsoftId, UserEntity.AuthProvider.MICROSOFT);
  ...
  private Map<String, Object> getMicrosoftUserInfo(String accessToken) {
    ...
    ResponseEntity<Map> response =
        restTemplate.exchange(
            "https://graph.microsoft.com/v1.0/me", HttpMethod.GET, request, Map.class);
    if (response.getBody() == null) {
  ...
  private UserEntity findOrCreateOAuthUser(
      String email, String name, String providerId, UserEntity.AuthProvider provider) {
    ...
    if (existingUser.isPresent()) {
      ...
      if (user.getAuthProvider() == provider) {
        ...
        throw new RuntimeException(
            "Account exists with email/password login. Please use password to sign in.");
      }
