...
public class IntegrationController {
    ...
    @PostMapping
    public ResponseEntity<?> create(@RequestBody CreateIntegrationRequest request, Authentication authentication) {
        if (authentication == null) return ResponseEntity.status(401).body(Map.of("error", "Not authenticated"));
        String userId = (String) authentication.getPrincipal();
    ...
    @PutMapping("/{id}")
    public ResponseEntity<?> update(@PathVariable String id, @RequestBody CreateIntegrationRequest request, Authentication authentication) {
        if (authentication == null) return ResponseEntity.status(401).body(Map.of("error", "Not authenticated"));
        String userId = (String) authentication.getPrincipal();
    ...
    public ResponseEntity<?> getEventTypes() {
        return ResponseEntity.ok(List.of(
            ...
            Map.of("code", "VENDOR_QUESTIONNAIRE_SUBMITTED", "label", "Vendor Questionnaire Submitted"),
