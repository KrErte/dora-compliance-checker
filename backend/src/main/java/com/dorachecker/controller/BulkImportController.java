...
public class BulkImportController {
  ...
  private static final Set<String> VALID_ENTITY_TYPES =
      Set.of("REMEDIATION", "ICT_ASSET", "VENDOR", "EVIDENCE");
  ...
  public ResponseEntity<?> validate(
    ...
    @SuppressWarnings("unchecked")
    List<Map<String, Object>> parseErrors =
        (List<Map<String, Object>>) parseResult.get("parseErrors");
    ...
    if (rows.size() > MAX_ROWS) {
      return ResponseEntity.badRequest()
          .body(
              Map.of(
                  "error", "File exceeds maximum of " + MAX_ROWS + " rows. Found: " + rows.size()));
    }
    ...
    List<Map<String, Object>> validationErrors =
        bulkImportService.validate(type, rows, suggestedMapping);
  ...
  public ResponseEntity<?> execute(@RequestBody Map<String, Object> body, Authentication auth) {
    ...
    if (entityType == null || !VALID_ENTITY_TYPES.contains(entityType.toUpperCase())) {
      return ResponseEntity.badRequest()
          .body(
              Map.of("error", "Invalid or missing entityType. Valid types: " + VALID_ENTITY_TYPES));
    }
    ...
    if (rows.size() > MAX_ROWS) {
      return ResponseEntity.badRequest()
          .body(
              Map.of(
                  "error", "Exceeds maximum of " + MAX_ROWS + " rows. Provided: " + rows.size()));
    }
