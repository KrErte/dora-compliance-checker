...
public class VendorQuestionnaireController {
  ...
  public VendorQuestionnaireController(
      VendorQuestionnaireRepository repository, VendorQuestionnaireService service) {
  ...
  public ResponseEntity<?> create(
      @RequestBody CreateRequest request, Authentication authentication) {
    if (authentication == null)
      return ResponseEntity.status(401).body(Map.of("error", "Not authenticated"));
    String userId = (String) authentication.getPrincipal();

    VendorQuestionnaireEntity q =
        service.create(userId, request.vendorName(), request.vendorEmail(), request.providerId());
  ...
  public ResponseEntity<?> stats(Authentication authentication) {
    ...
    return ResponseEntity.ok(
        Map.of(
            "pending",
                repository.countByUserIdAndStatus(userId, VendorQuestionnaireEntity.Status.PENDING),
            "submitted",
                repository.countByUserIdAndStatus(
                    userId, VendorQuestionnaireEntity.Status.SUBMITTED),
            "reviewed",
                repository.countByUserIdAndStatus(
                    userId, VendorQuestionnaireEntity.Status.REVIEWED)));
  }
              ...
              if (q.getStatus() == VendorQuestionnaireEntity.Status.SUBMITTED) {
                return ResponseEntity.ok(
                    Map.of(
                        ...
                        q.getVendorName(),
                        "submittedAt",
                        q.getSubmittedAt() != null ? q.getSubmittedAt().toString() : ""));
              }
              if (q.getExpiresAt() != null
                  && q.getExpiresAt().isBefore(java.time.LocalDateTime.now())) {
                return ResponseEntity.ok(
                    Map.of("status", "EXPIRED", "vendorName", q.getVendorName()));
              }
              return ResponseEntity.ok(
                  Map.of(
                      "status", "PENDING",
  ...
  public ResponseEntity<?> submitPublic(
      @PathVariable String token, @RequestBody SubmitRequest request) {
    try {
      VendorQuestionnaireEntity q = service.submitResponses(token, request.responses());
      return ResponseEntity.ok(
          Map.of(
              "status", "SUBMITTED", "riskScore", q.getRiskScore() != null ? q.getRiskScore() : 0));
