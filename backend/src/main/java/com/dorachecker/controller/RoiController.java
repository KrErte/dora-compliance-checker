...
public class RoiController {
  ...
  public RoiController(
      ...
      RoiValidationService validationService,
      RoiExportService exportService,
  ...
  public ResponseEntity<?> getRegister(@PathVariable String id, Authentication auth) {
    ...
    if (!register.getUserId().equals(auth.getPrincipal()))
  ...
  public ResponseEntity<?> updateRegister(
      @PathVariable String id, @RequestBody Map<String, Object> data, Authentication auth) {
    RoiRegisterEntity register = roiService.getRegister(id);
    if (register == null) return ResponseEntity.notFound().build();
    if (!register.getUserId().equals(auth.getPrincipal()))
      return ResponseEntity.status(403).build();
  ...
  public ResponseEntity<?> addGroupEntity(
      @PathVariable String id, @RequestBody Map<String, Object> data, Authentication auth) {
    RoiRegisterEntity register = roiService.getRegister(id);
    if (register == null) return ResponseEntity.notFound().build();
    if (!register.getUserId().equals(auth.getPrincipal()))
  ...
  public ResponseEntity<?> removeGroupEntity(
      @PathVariable String id, @PathVariable String entryId, Authentication auth) {
    RoiRegisterEntity register = roiService.getRegister(id);
    if (register == null) return ResponseEntity.notFound().build();
    if (!register.getUserId().equals(auth.getPrincipal()))
      return ResponseEntity.status(403).build();
  ...
  public ResponseEntity<?> removeBranch(
      @PathVariable String id, @PathVariable String branchId, Authentication auth) {
    RoiRegisterEntity register = roiService.getRegister(id);
    if (register == null) return ResponseEntity.notFound().build();
    if (!register.getUserId().equals(auth.getPrincipal()))
      return ResponseEntity.status(403).build();
  ...
  public ResponseEntity<?> removeProvider(
      @PathVariable String id, @PathVariable String providerId, Authentication auth) {
    RoiRegisterEntity register = roiService.getRegister(id);
    if (register == null) return ResponseEntity.notFound().build();
    if (!register.getUserId().equals(auth.getPrincipal()))
      return ResponseEntity.status(403).build();
  ...
  public ResponseEntity<?> addSupplyChain(
      @PathVariable String id, @RequestBody Map<String, Object> data, Authentication auth) {
    RoiRegisterEntity register = roiService.getRegister(id);
    if (register == null) return ResponseEntity.notFound().build();
    if (!register.getUserId().equals(auth.getPrincipal()))
  ...
  public ResponseEntity<?> removeSupplyChain(
      @PathVariable String id, @PathVariable String scId, Authentication auth) {
    RoiRegisterEntity register = roiService.getRegister(id);
    if (register == null) return ResponseEntity.notFound().build();
    if (!register.getUserId().equals(auth.getPrincipal()))
      return ResponseEntity.status(403).build();
  ...
  public ResponseEntity<?> removeFunction(
      @PathVariable String id, @PathVariable String funcId, Authentication auth) {
    RoiRegisterEntity register = roiService.getRegister(id);
    if (register == null) return ResponseEntity.notFound().build();
    if (!register.getUserId().equals(auth.getPrincipal()))
      return ResponseEntity.status(403).build();
  ...
  public ResponseEntity<?> removeContract(
      @PathVariable String id, @PathVariable String contractId, Authentication auth) {
    RoiRegisterEntity register = roiService.getRegister(id);
    if (register == null) return ResponseEntity.notFound().build();
    if (!register.getUserId().equals(auth.getPrincipal()))
      return ResponseEntity.status(403).build();
  ...
  public ResponseEntity<?> addContractDetail(
      @PathVariable String id, @RequestBody Map<String, Object> data, Authentication auth) {
    RoiRegisterEntity register = roiService.getRegister(id);
    if (register == null) return ResponseEntity.notFound().build();
    if (!register.getUserId().equals(auth.getPrincipal()))
  ...
  public ResponseEntity<?> removeContractDetail(
      @PathVariable String id, @PathVariable String detailId, Authentication auth) {
    RoiRegisterEntity register = roiService.getRegister(id);
    if (register == null) return ResponseEntity.notFound().build();
    if (!register.getUserId().equals(auth.getPrincipal()))
      return ResponseEntity.status(403).build();
  ...
  public ResponseEntity<?> addIntraGroupContract(
      @PathVariable String id, @RequestBody Map<String, Object> data, Authentication auth) {
    RoiRegisterEntity register = roiService.getRegister(id);
    if (register == null) return ResponseEntity.notFound().build();
    if (!register.getUserId().equals(auth.getPrincipal()))
  ...
  public ResponseEntity<?> addProviderSigning(
      @PathVariable String id, @RequestBody Map<String, Object> data, Authentication auth) {
    RoiRegisterEntity register = roiService.getRegister(id);
    if (register == null) return ResponseEntity.notFound().build();
    if (!register.getUserId().equals(auth.getPrincipal()))
  ...
  public ResponseEntity<?> addInternalProvider(
      @PathVariable String id, @RequestBody Map<String, Object> data, Authentication auth) {
    RoiRegisterEntity register = roiService.getRegister(id);
    if (register == null) return ResponseEntity.notFound().build();
    if (!register.getUserId().equals(auth.getPrincipal()))
  ...
  public ResponseEntity<?> addServiceUser(
      @PathVariable String id, @RequestBody Map<String, Object> data, Authentication auth) {
    RoiRegisterEntity register = roiService.getRegister(id);
    if (register == null) return ResponseEntity.notFound().build();
    if (!register.getUserId().equals(auth.getPrincipal()))
  ...
  public ResponseEntity<?> removeAssessment(
      @PathVariable String id, @PathVariable String assessmentId, Authentication auth) {
    RoiRegisterEntity register = roiService.getRegister(id);
    if (register == null) return ResponseEntity.notFound().build();
    if (!register.getUserId().equals(auth.getPrincipal()))
      return ResponseEntity.status(403).build();
  ...
  public ResponseEntity<?> validateTemplate(
      @PathVariable String id, @PathVariable String templateCode, Authentication auth) {
    RoiRegisterEntity register = roiService.getRegister(id);
    if (register == null) return ResponseEntity.notFound().build();
    if (!register.getUserId().equals(auth.getPrincipal()))
      return ResponseEntity.status(403).build();
  ...
  public ResponseEntity<byte[]> exportXbrlCsv(@PathVariable String id, Authentication auth) {
    ...
    try {
      ...
      String lei =
          (register.getEntityLei() != null ? register.getEntityLei() : "UNKNOWN")
              .replaceAll("[^a-zA-Z0-9._-]", "_");
      String country =
          (register.getCountry() != null ? register.getCountry() : "EU")
    ...
    try {
      ...
      return ResponseEntity.ok()
          .header(
              ...
              "attachment; filename=\"RoI_"
                  + (register.getEntityName() != null
                      ? register.getEntityName().replaceAll("[^a-zA-Z0-9._\\- ]", "_")
                      : "export")
                  + ".xlsx\"")
          .contentType(
