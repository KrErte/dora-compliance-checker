...
public class IctProviderController {
  ...
  public ResponseEntity<?> createProvider(
      @RequestBody CreateProviderRequest request, Authentication authentication) {
    if (authentication == null) {
      return ResponseEntity.status(401).body(Map.of("error", "Not authenticated"));
    }
    String userId = (String) authentication.getPrincipal();
    ...
    int calculatedRisk =
        calculateRiskScore(request.countryCode(), request.criticality(), request.hasExitStrategy());
  ...
  public ResponseEntity<?> createProvidersBatch(
      @RequestBody List<CreateProviderRequest> requests, Authentication authentication) {
    if (authentication == null) {
      return ResponseEntity.status(401).body(Map.of("error", "Not authenticated"));
    }
    String userId = (String) authentication.getPrincipal();
                  ...
                  request -> {
                    ...
                    int calculatedRisk =
                        calculateRiskScore(
                            request.countryCode(),
                            request.criticality(),
                            request.hasExitStrategy());
                    provider.setRiskScore(
                        request.riskScore() != null ? request.riskScore() : calculatedRisk);
                    provider.setHasExitStrategy(request.hasExitStrategy());
                    provider.setExitStrategyDescription(request.exitStrategyDescription());
                    provider.setSubcontractingInfo(request.subcontractors());
    ...
    return ResponseEntity.ok(Map.of("imported", created.size(), "providers", created));
  }
  ...
  public record CategorizeRequest(
      String criticality, @jakarta.validation.constraints.Size(max = 2000) String reason) {}

  @DeleteMapping("/{id}")
  public ResponseEntity<?> deleteProvider(@PathVariable String id, Authentication authentication) {
    if (authentication == null) {
      return ResponseEntity.status(401).body(Map.of("error", "Not authenticated"));
    }
    String userId = (String) authentication.getPrincipal();
  ...
  private static final Set<String> TRUSTED_NON_EU =
      Set.of("US", "GB", "CH", "NO", "IS", "CA", "AU", "NZ", "JP");
  private static final Set<String> HIGH_RISK = Set.of("CN", "RU", "BY", "IR", "KP");

  /**
   * Calculate risk score based on country code and criticality. Base risks: EE=20, EU=25, trusted
   * non-EU=35, other non-EU=40, high-risk=60
   */
  private int calculateRiskScore(String countryCode, String criticality, Boolean hasExitStrategy) {
    ...
    int criticalityMod =
        switch (criticality != null ? criticality : "normal") {
          case "critical" -> 25;
          case "important" -> 10;
          default -> 5;
        };
