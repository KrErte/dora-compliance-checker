...
public class SsoConfigController {
  ...
  public SsoConfigController(
      SsoConfigRepository ssoConfigRepository, OrganizationMemberRepository memberRepository) {
  ...
  @GetMapping
  public ResponseEntity<List<SsoConfigEntity>> list(
  ...
  @PostMapping
  public ResponseEntity<SsoConfigEntity> create(
      @PathVariable String orgId, @RequestBody SsoConfigEntity config, Authentication auth) {
  ...
  @PutMapping("/{configId}")
  public ResponseEntity<SsoConfigEntity> update(
  ...
  public ResponseEntity<SsoConfigEntity> activate(
      @PathVariable String orgId, @PathVariable String configId, Authentication auth) {
    if (!isOrgAdmin(orgId, getUserId(auth))) return ResponseEntity.status(403).build();
    return ssoConfigRepository
        .findById(configId)
        .filter(c -> c.getOrganizationId().equals(orgId))
  ...
  public ResponseEntity<SsoConfigEntity> disable(
      @PathVariable String orgId, @PathVariable String configId, Authentication auth) {
    if (!isOrgAdmin(orgId, getUserId(auth))) return ResponseEntity.status(403).build();
    return ssoConfigRepository
        .findById(configId)
