...
public class GlobalIctProviderController {
    ...
    public ResponseEntity<List<GlobalIctProviderEntity>> getByServiceType(@PathVariable String serviceType) {
        List<GlobalIctProviderEntity> providers = repository.findByServiceTypeIgnoreCaseOrderByUsageCountDesc(serviceType);
        return ResponseEntity.ok(providers);
    }
    ...
    /**
     * Add new provider to global registry (community contribution)
     * This is called when user adds a new provider that doesn't exist in global registry
     */
    @PostMapping("/contribute")
    public ResponseEntity<GlobalIctProviderEntity> contribute(@RequestBody ContributeRequest request) {
