...
public class ContractAnalysisController {
  ...
  public ResponseEntity<ContractAnalysisResult> analyzeContract(
    ...
    ContractAnalysisResult result =
        contractAnalysisService.analyze(companyName, contractName, file);
    return ResponseEntity.ok(result);
  }

  /**
   * Bulk contract analysis — upload multiple files at once. Analyzes each contract sequentially and
   * returns a portfolio summary.
   */
  ...
  public ResponseEntity<Map<String, Object>> analyzeBulk(
    ...
    for (MultipartFile file : files) {
      if (file.getSize() > 5 * 1024 * 1024) {
        return ResponseEntity.badRequest()
            .body(Map.of("error", "File " + file.getOriginalFilename() + " exceeds 5MB limit"));
      }
    }
    ...
    response.put(
        "averageScore", successCount > 0 ? Math.round(scoreSum / successCount * 10) / 10.0 : 0);
    ...
    response.put(
        "portfolioLevel",
        successCount > 0
            ? (scoreSum / successCount >= 80
                ? "GREEN"
                : (scoreSum / successCount >= 50 ? "YELLOW" : "RED"))
            : "UNKNOWN");
