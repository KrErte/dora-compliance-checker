...
public class ProportionalityController {
  ...
  public ResponseEntity<?> getClassification(Authentication auth) {
    String userId = (String) auth.getPrincipal();
    Optional<ProportionalityClassificationEntity> classification =
        service.getClassification(userId);
    return classification.map(ResponseEntity::ok).orElse(ResponseEntity.noContent().build());
  }
