...
public class WorkspaceController {
  ...
  public ResponseEntity<?> createProject(
      @RequestBody @jakarta.validation.Valid CreateProjectRequest request, Authentication auth) {
    try {
      String email = auth != null ? (String) auth.getName() : request.email();
      WorkspaceProjectEntity project = workspaceService.createProject(request.name(), email);
  ...
  public ResponseEntity<?> analyzeContract(
      ...
      Authentication auth) {
    try {
      WorkspaceProjectEntity project =
          projectRepository
              .findById(id)
      ...
      if (text == null || text.isBlank()) {
        return ResponseEntity.badRequest()
            .body(Map.of("error", "Could not extract text from document"));
      }
      ...
      logAudit(
          ...
          "owner",
          "analyzed",
          "Document analyzed: " + file.getOriginalFilename(),
          null);
    ...
    try {
      ...
      return ResponseEntity.ok(
          Map.of(
              ...
              gapResult.getCompliancePercentage()));
    } catch (Exception e) {
  ...
  private String buildAnalysisPrompt(
      String contractText, String regulation, List<WorkspaceService.RegulationCheck> checks) {
    StringBuilder prompt = new StringBuilder();
    prompt
        .append("Analyze this ICT contract against ")
        .append(regulation)
        .append(" requirements.\n\n");
    prompt
        .append("CONTRACT TEXT:\n")
        .append(contractText.substring(0, Math.min(contractText.length(), 15000)))
        .append("\n\n");
    ...
    prompt.append(
        "Be strict - only mark as 'passed' if clearly addressed. Mark 'partial' if mentioned but incomplete. Mark 'failed' if not found.");

    return prompt.toString();
  }

  private WorkspaceGapResultEntity parseGapAnalysis(
      WorkspaceProjectEntity project,
      String regulation,
      List<WorkspaceService.RegulationCheck> checks,
      String aiResponse) {
    int passed = 0, failed = 0, partial = 0;
    List<Map<String, Object>> details = new ArrayList<>();
  ...
  private void updateProjectStatus(WorkspaceProjectEntity project) {
    ...
    long completedReviews =
        project.getReviews().stream().filter(r -> !"pending".equals(r.getStatus())).count();
  ...
  record ManualCheckRequest(
      @jakarta.validation.constraints.Size(max = 50) String regulation,
