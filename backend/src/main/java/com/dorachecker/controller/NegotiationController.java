...
public class NegotiationController {
  ...
  @PostMapping
  public ResponseEntity<NegotiationResult> create(
      @Valid @RequestBody CreateNegotiationRequest request, Authentication auth) {
  ...
  public ResponseEntity<NegotiationMessageResult> addMessage(
      @PathVariable String id, @Valid @RequestBody AddMessageRequest body, Authentication auth) {
    NegotiationMessageResult result =
        negotiationService.addMessage(
            id,
            body.itemId(),
  ...
  public ResponseEntity<List<NegotiationMessageResult>> getMessages(
      @PathVariable String id, Authentication auth) {
    return ResponseEntity.ok(negotiationService.getMessages(id, getUserId(auth)));
  }

  public record StatusUpdateRequest(@NotBlank @Size(max = 50) String status) {}
