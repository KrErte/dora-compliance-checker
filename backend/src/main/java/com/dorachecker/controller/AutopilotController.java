...
public class AutopilotController {
  ...
  public AutopilotController(
      AutopilotService autopilotService, SubscriptionGuardService subscriptionGuard) {
  ...
  public ResponseEntity<?> snoozeInsight(
      @PathVariable String id, @RequestBody Map<String, Integer> body, Authentication auth) {
    String userId = getUserId(auth);
    if (!subscriptionGuard.isPremium(userId, null)) {
      return ResponseEntity.status(403).body(Map.of("error", "Premium subscription required"));
    }
