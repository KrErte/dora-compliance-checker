...
public class AiActDocumentController {
    ...
    public AiActDocumentController(AiActDocumentService documentService,
    ...
    public ResponseEntity<?> generate(
        ...
        String userId = (String) authentication.getPrincipal();
        AiActDocumentEntity doc = documentService.startGeneration(
    ...
    public ResponseEntity<?> update(
        ...
        return documentService.updateDocument(documentId, request.title(), request.content())
                .map(d -> ResponseEntity.ok(toDto(d)))
                .orElse(ResponseEntity.notFound().build());
    }
    ...
    public ResponseEntity<?> exportPdf(@PathVariable String documentId, Authentication authentication) {
        if (authentication == null) {
            return ResponseEntity.status(401).body(Map.of("error", "Not authenticated"));
        }
    ...
    private DocumentDto toDto(AiActDocumentEntity d) {
        return new DocumentDto(
            d.getId(), d.getAiSystemId(), d.getDocumentType(), d.getTitle(),
            d.getContent(), d.getFormat(), d.getStatus(), d.getVersion(),
            d.getParentId(), d.getCreatedAt(), d.getUpdatedAt()
        );
    }
    ...
    public record DocumentDto(
        String id, String aiSystemId, String documentType, String title,
        String content, String format, String status, int version,
