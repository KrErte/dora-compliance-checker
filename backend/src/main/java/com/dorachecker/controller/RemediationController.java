...
public class RemediationController {
    ...
    public RemediationController(RemediationService remediationService, RemediationItemRepository remediationRepository) {
    ...
    public ResponseEntity<?> bulkUpdateStatus(@RequestBody Map<String, Object> body, Authentication auth) {
    ...
    public ResponseEntity<?> bulkUpdatePriority(@RequestBody Map<String, Object> body, Authentication auth) {
        @SuppressWarnings("unchecked")
        List<String> ids = (List<String>) body.get("ids");
        String priority = (String) body.get("priority");
        if (ids == null || ids.isEmpty() || priority == null) return ResponseEntity.badRequest().build();
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(file.getInputStream(), StandardCharsets.UTF_8))) {
            ...
            while ((line = reader.readLine()) != null && row <= 500) {
                ...
                try {
                    ...
                    entity.setPriority(fields.length > 2 && !fields[2].isBlank() ? fields[2].trim().toUpperCase() : "MEDIUM");
                    entity.setStatus(fields.length > 3 && !fields[3].isBlank() ? fields[3].trim().toUpperCase() : "OPEN");
                    if (fields.length > 4 && !fields[4].isBlank()) {
                        try { entity.setDueDate(LocalDate.parse(fields[4].trim())); } catch (Exception ignored) {}
                    }
                    entity.setPillar(fields.length > 5 && !fields[5].isBlank() ? fields[5].trim().toUpperCase() : "ICT_RISK_MANAGEMENT");
            ...
            return ResponseEntity.status(500).body(Map.of("error", "Failed to parse CSV: " + e.getMessage()));
        }
