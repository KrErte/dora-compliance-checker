...
public class AlertDigestService {
    ...
    public AlertDigestService(ComplianceProfileRepository profileRepository,
    ...
    public void sendDailyDigest() {
        ...
        sendDigest("DAILY", ComplianceProfileEntity.AlertFrequency.DAILY, LocalDateTime.now().minusDays(1));
    }
    ...
    public void sendWeeklyDigest() {
        if (!digestEnabled) return;
        sendDigest("WEEKLY", ComplianceProfileEntity.AlertFrequency.WEEKLY, LocalDateTime.now().minusWeeks(1));
    }

    private void sendDigest(String digestType, ComplianceProfileEntity.AlertFrequency frequency, LocalDateTime since) {
        log.info("Alustan {} digest e-kirjade saatmist...", digestType);
        int sent = 0;

        List<ComplianceProfileEntity> profiles = profileRepository.findAll().stream()
        ...
        for (ComplianceProfileEntity profile : profiles) {
            try {
                List<UserAlertEntity> unreadAlerts = alertRepository.findByUserIdAndIsReadFalse(profile.getUserId());
                // Filter to alerts created since the period start
                List<UserAlertEntity> recentAlerts = unreadAlerts.stream()
                ...
                String subject = digestType.equals("DAILY")
                ...
                log.error("Digest saatmine ebaõnnestus kasutajale {}: {}", profile.getUserId(), e.getMessage());
            }
        }
    ...
    private String buildDigestHtml(List<UserAlertEntity> alerts, String digestType) {
        ...
        sb.append("body{font-family:Arial,sans-serif;background:#0f172a;color:#e2e8f0;margin:0;padding:0}");
        sb.append(".container{max-width:600px;margin:0 auto;padding:20px}");
        sb.append(".header{background:linear-gradient(135deg,#059669,#0891b2);padding:24px;border-radius:12px 12px 0 0;text-align:center}");
        ...
        sb.append(".content{background:#1e293b;padding:20px;border:1px solid #334155;border-top:none;border-radius:0 0 12px 12px}");
        sb.append(".alert-card{background:#0f172a;border:1px solid #334155;border-radius:8px;padding:14px;margin-bottom:12px}");
        sb.append(".badge{display:inline-block;padding:2px 8px;border-radius:4px;font-size:10px;font-weight:bold;text-transform:uppercase}");
        sb.append(".badge-critical{background:#7f1d1d;color:#fca5a5}.badge-high{background:#7c2d12;color:#fdba74}");
        sb.append(".badge-medium{background:#713f12;color:#fde047}.badge-low{background:#1e3a5f;color:#93c5fd}");
        ...
        sb.append(".cta{display:inline-block;background:linear-gradient(135deg,#059669,#0891b2);color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-size:14px;font-weight:600;margin-top:16px}");
        ...
        sb.append("<p>").append(digestType.equals("DAILY") ? "Päevane" : "Nädala").append(" ülevaade — ");
        sb.append(alerts.size()).append(" uut hoiatust</p></div>");

        sb.append("<div class='content'>");
        for (UserAlertEntity alert : alerts) {
            Optional<RegulatoryUpdateEntity> updateOpt = updateRepository.findById(alert.getRegulatoryUpdateId());
            RegulatoryUpdateEntity update = updateOpt.orElse(null);

            String severity = update != null && update.getSeverity() != null ? update.getSeverity() : "INFO";
            String badgeClass = "badge-" + severity.toLowerCase();

            sb.append("<div class='alert-card'>");
            sb.append("<span class='badge ").append(badgeClass).append("'>").append(severity).append("</span>");
            sb.append("<div class='alert-title'>").append(escapeHtml(update != null ? update.getTitle() : "Regulatiivne uuendus")).append("</div>");
            sb.append("<div class='alert-meta'>").append(escapeHtml(update != null ? update.getSource() : "")).append("</div>");
            if (update != null && update.getAiSummary() != null) {
                sb.append("<div class='alert-summary'>").append(escapeHtml(update.getAiSummary())).append("</div>");
            }
            sb.append("</div>");
        }

        sb.append("<div style='text-align:center'><a href='https://doraaudit.eu/alerts' class='cta'>Vaata kõiki hoiatusi</a></div>");
        sb.append("</div>");
        sb.append("<div class='footer'>DoraAudit — DORA vastavusplatvorm Balti finantsasutustele</div>");
