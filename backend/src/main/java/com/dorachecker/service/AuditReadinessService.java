...
public class AuditReadinessService {
    ...
    public AuditReadinessService(AssessmentRepository assessmentRepository,
    ...
    public Map<String, Object> getReadinessScore(String userId) {
        ...
        // Weighted composite
        double overallScore = assessmentScore * 0.25
            + evidenceScore * 0.20
        ...
        // Top action items
        result.put("actions", computeActionItems(userId, assessment, evidence, remediation, incidents, thirdParty, roi));
    ...
    private Map<String, Object> computeAssessmentScore(String userId) {
        ...
        if (assessments.isEmpty()) {
            return Map.of("score", 0.0, "label", "Assessment", "detail", "No assessment completed",
                "count", 0, "latest", "");
        }
        ...
        return Map.of("score", score, "label", "Assessment",
            "detail", latest.getComplianceLevel(),
            "count", assessments.size(),
            "latest", latest.getAssessmentDate().toString());
    }

    private Map<String, Object> computeEvidenceScore(String userId) {
        long total = evidenceRepository.countByUserId(userId);
        if (total == 0) {
            return Map.of("score", 0.0, "label", "Evidence Vault", "detail", "No evidence uploaded",
                "total", 0L, "verified", 0L, "expired", 0L, "coveragePercent", 0);
        }
        ...
        for (EvidenceEntity item : items) {
            if (item.getArticleNumbers() != null) {
                for (String art : item.getArticleNumbers().split(",")) {
                    try { coveredArticles.add(Integer.parseInt(art.trim())); } catch (NumberFormatException ignored) {}
                }
            }
        }
        ...
        return Map.of("score", Math.min(score, 100), "label", "Evidence Vault",
            "detail", verified + "/" + total + " verified",
            "total", total, "verified", verified, "expired", expired,
    ...
    private Map<String, Object> computeRemediationScore(String userId) {
        ...
        if (total == 0) {
            return Map.of("score", 50.0, "label", "Remediation", "detail", "No actions tracked",
                "total", 0L, "completed", 0L, "open", 0L);
        }
        ...
        List<RemediationItemEntity> openCritical = remediationRepository
            .findByUserIdAndStatusOrderByPriorityAsc(userId, "OPEN").stream()
        ...
        return Map.of("score", Math.min(score, 100), "label", "Remediation",
            "detail", completed + "/" + total + " completed",
            "total", total, "completed", completed, "open", open);
    }
    ...
    private Map<String, Object> computeIncidentScore(String userId) {
        ...
        if (total == 0) {
            ...
            return Map.of("score", 60.0, "label", "Incidents", "detail", "No incidents reported",
                "total", 0L, "closed", 0L, "overdue", 0L);
        }
        ...
        List<IncidentReportEntity> allIncidents = incidentRepository.findByUserIdOrderByCreatedAtDesc(userId);
        long overdue = allIncidents.stream()
            .filter(i -> i.getInitialReportDueAt() != null && i.getInitialReportSentAt() == null
        ...
        return Map.of("score", Math.min(score, 100), "label", "Incidents",
            "detail", resolved + "/" + total + " resolved",
            "total", total, "closed", resolved, "overdue", overdue);
    }
    ...
    private Map<String, Object> computeThirdPartyScore(String userId) {
        ...
        if (providers.isEmpty()) {
            return Map.of("score", 30.0, "label", "Third-Party Risk", "detail", "No providers registered",
                "total", 0, "withExitStrategy", 0, "critical", 0);
        }

        long total = providers.size();
        long critical = providers.stream().filter(IctProviderEntity::isCritical).count();
        long withExitStrategy = providers.stream()
        ...
        return Map.of("score", Math.min(score, 100), "label", "Third-Party Risk",
            "detail", withExitStrategy + "/" + total + " with exit strategy",
            "total", (long) total, "withExitStrategy", withExitStrategy, "critical", critical);
    }

    private Map<String, Object> computeRoiScore(String userId) {
        List<RoiRegisterEntity> registers = roiRegisterRepository.findByUserIdOrderByUpdatedAtDesc(userId);
        if (registers.isEmpty()) {
            return Map.of("score", 0.0, "label", "Register of Information", "detail", "No RoI entries",
                "count", 0);
        }

        // Score based on having entries + status
        long validCount = registers.stream()
            .filter(r -> r.getStatus() == RoiRegisterEntity.Status.VALID || r.getStatus() == RoiRegisterEntity.Status.EXPORTED)
            .count();
        long total = registers.size();

        double score = Math.min(100, (double) validCount / Math.max(total, 1) * 80 + 20);

        return Map.of("score", score, "label", "Register of Information",
            "detail", validCount + "/" + total + " validated",
            "count", (long) total);
    }
    ...
    private Map<String, Object> computePillarScores(String userId) {
        ...
        String[] pillars = {"ICT_RISK_MANAGEMENT", "INCIDENT_MANAGEMENT", "TESTING", "THIRD_PARTY", "INFORMATION_SHARING"};
        Map<String, Object> pillarScores = new LinkedHashMap<>();

        for (String pillar : pillars) {
            List<EvidenceEntity> pillarEvidence = evidenceRepository.findByUserIdAndPillarOrderByCreatedAtDesc(userId, pillar);
            long evidenceCount = pillarEvidence.size();
            long verifiedCount = pillarEvidence.stream().filter(e -> "VERIFIED".equals(e.getStatus())).count();

            List<RemediationItemEntity> pillarRemediation = remediationRepository.findByUserIdOrderByCreatedAtDesc(userId).stream()
                .filter(r -> pillar.equals(r.getPillar()))
                .toList();
            long remTotal = pillarRemediation.size();
            long remCompleted = pillarRemediation.stream().filter(r -> "COMPLETED".equals(r.getStatus())).count();
            ...
            pillarScores.put(pillar, Map.of(
    ...
    private List<Map<String, String>> computeActionItems(String userId,
            Map<String, Object> assessment, Map<String, Object> evidence,
            Map<String, Object> remediation, Map<String, Object> incidents,
            Map<String, Object> thirdParty, Map<String, Object> roi) {

        List<Map<String, String>> actions = new ArrayList<>();

        if ((double) assessment.get("score") == 0) {
            actions.add(Map.of("priority", "HIGH", "action", "Complete your first DORA self-assessment",
                "module", "assessment", "impact", "+25 points"));
        } else if ((double) assessment.get("score") < 60) {
            actions.add(Map.of("priority", "MEDIUM", "action", "Improve assessment score — review low-scoring areas",
                "module", "assessment", "impact", "+10-15 points"));
        }

        if ((double) evidence.get("score") == 0) {
            actions.add(Map.of("priority", "HIGH", "action", "Upload compliance evidence to the Evidence Vault",
                "module", "evidence", "impact", "+20 points"));
        } else {
            long expired = (long) evidence.get("expired");
            if (expired > 0) {
                actions.add(Map.of("priority", "HIGH", "action", "Replace " + expired + " expired evidence document(s)",
                    "module", "evidence", "impact", "+" + (expired * 3) + " points"));
            }
            long unverified = (long) evidence.get("total") - (long) evidence.get("verified") - expired;
            if (unverified > 0) {
                actions.add(Map.of("priority", "MEDIUM", "action", "Verify " + unverified + " pending evidence document(s)",
                    "module", "evidence", "impact", "+" + (unverified * 2) + " points"));
            }
        }

        long openRem = (long) remediation.get("open");
        if (openRem > 0) {
            actions.add(Map.of("priority", "HIGH", "action", "Resolve " + openRem + " open remediation action(s)",
                "module", "remediation", "impact", "+" + (openRem * 3) + " points"));
        }

        long overdueInc = (long) incidents.get("overdue");
        if (overdueInc > 0) {
            actions.add(Map.of("priority", "CRITICAL", "action", "Submit " + overdueInc + " overdue incident report(s)",
                "module", "incidents", "impact", "+" + (overdueInc * 5) + " points"));
        }

        if ((double) thirdParty.get("score") < 50) {
            actions.add(Map.of("priority", "MEDIUM", "action", "Register ICT providers and add exit strategies",
                "module", "thirdParty", "impact", "+10 points"));
        }

        if ((double) roi.get("score") == 0) {
            actions.add(Map.of("priority", "MEDIUM", "action", "Create your Register of Information (Art. 28)",
                "module", "roi", "impact", "+10 points"));
        }

        // Sort by priority: CRITICAL > HIGH > MEDIUM
        actions.sort((a, b) -> {
            Map<String, Integer> order = Map.of("CRITICAL", 0, "HIGH", 1, "MEDIUM", 2);
            return order.getOrDefault(a.get("priority"), 3) - order.getOrDefault(b.get("priority"), 3);
        });
