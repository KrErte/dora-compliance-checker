...
public class BoardPackageService {
  ...
  public Map<String, Object> getBoardPackageData(String userId) {
    ...
    evidenceData.put(
        "verificationRate",
        totalEvidence > 0 ? Math.round((double) verifiedEvidence / totalEvidence * 100) : 0);
    ...
    long inProgressRemediation =
        remediationRepository.countByUserIdAndStatus(userId, "IN_PROGRESS");
    ...
    remediationData.put(
        "completionRate",
        totalRemediation > 0
            ? Math.round((double) completedRemediation / totalRemediation * 100)
            : 0);
    ...
    List<Map<String, String>> topGaps =
        openItems.stream()
            .limit(5)
            .map(
                r -> {
    ...
    long openIncidents =
        incidentRepository.countByUserIdAndReportingStatus(userId, "DETECTED")
    ...
    long closedIncidents =
        incidentRepository.countByUserIdAndReportingStatus(userId, "CLOSED")
    ...
    List<RegulatoryUpdateEntity> recentUpdates =
        regulatoryUpdateRepository.findAllByOrderByPublishedDateDesc();
    data.put(
        "regulatoryUpdates",
  ...
  private int calculateOverallHealth(
      ...
      long completedRemediation,
      long totalRemediation,
      long openIncidents) {
