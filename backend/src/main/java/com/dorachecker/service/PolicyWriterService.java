...
public class PolicyWriterService {
    ...
    private static final Map<String, PolicyTypeMeta> POLICY_TYPES = Map.ofEntries(
        Map.entry("ict-risk", new PolicyTypeMeta("IKT riskihalduse raamistik", "ICT Risk Management Framework", "Art. 5-6",
            Set.of(QuestionCategory.ICT_RISK_MANAGEMENT, QuestionCategory.RISK, QuestionCategory.CONTINUITY))),
        Map.entry("incident-response", new PolicyTypeMeta("Intsidentide reageerimise plaan", "Incident Response Plan", "Art. 17-23",
            Set.of(QuestionCategory.INCIDENT, QuestionCategory.INCIDENT_MANAGEMENT))),
        Map.entry("bcp", new PolicyTypeMeta("Äritegevuse jätkuvuse plaan", "Business Continuity Plan", "Art. 11-12",
            Set.of(QuestionCategory.CONTINUITY, QuestionCategory.RISK))),
        Map.entry("tprm", new PolicyTypeMeta("Kolmanda osapoole riskihalduse poliitika", "Third-Party Risk Management Policy", "Art. 28-30",
            Set.of(QuestionCategory.SUBCONTRACTING, QuestionCategory.EXIT_STRATEGY, QuestionCategory.AUDIT, QuestionCategory.SERVICE_LEVEL))),
        Map.entry("testing", new PolicyTypeMeta("Vastupidavuse testimise programm", "Resilience Testing Programme", "Art. 24-27",
            Set.of(QuestionCategory.TESTING, QuestionCategory.ICT_RISK_MANAGEMENT))),
        Map.entry("infosec", new PolicyTypeMeta("Infoturbe poliitika", "Information Security Policy", "Art. 9",
            Set.of(QuestionCategory.DATA, QuestionCategory.ICT_RISK_MANAGEMENT, QuestionCategory.RISK))),
        Map.entry("change-mgmt", new PolicyTypeMeta("Muudatuste halduse poliitika", "Change Management Policy", "Art. 8(4), 9(4)(e)",
            Set.of(QuestionCategory.ICT_RISK_MANAGEMENT, QuestionCategory.RISK))),
        Map.entry("asset-mgmt", new PolicyTypeMeta("IKT varade halduse poliitika", "ICT Asset Management Policy", "Art. 8(1), 8(4)",
            Set.of(QuestionCategory.ICT_RISK_MANAGEMENT, QuestionCategory.RISK))),
        Map.entry("crypto", new PolicyTypeMeta("Krüptograafia ja andmekaitse poliitika", "Cryptography and Data Protection Policy", "Art. 9(2), 9(4)(d)",
            Set.of(QuestionCategory.DATA, QuestionCategory.ICT_RISK_MANAGEMENT))),
        Map.entry("access-control", new PolicyTypeMeta("Juurdepääsu kontrolli poliitika", "Access Control Policy", "Art. 9(4)(c)",
            Set.of(QuestionCategory.DATA, QuestionCategory.ICT_RISK_MANAGEMENT)))
    );

    public PolicyWriterService(ClaudeApiService claudeApiService,
    ...
    public void delete(String id, String userId) {
        policyRepository.findById(id).ifPresent(entity -> {
    ...
    private String buildGapsContext(String assessmentId, Set<QuestionCategory> relevantCategories, boolean isEstonian) {
        return assessmentRepository.findById(assessmentId).map(assessment -> {
            try {
                Map<Integer, String> answers = objectMapper.readValue(
                ...
                for (DoraQuestion q : questions) {
                    ...
                    if ("no".equals(answer) || "partial".equals(answer)) {
                        String gap = isEstonian
                            ? q.questionEt() + " [" + q.articleReference() + "] - " +
                              ("no".equals(answer) ? "Ei vasta" : "Osaliselt vastab")
                            : q.questionEn() + " [" + q.articleReference() + "] - " +
                ...
                sb.append(isEstonian ? "\n\nHINDAMISEST TUVASTATUD LÜNGAD:\n" : "\n\nASSESSMENT GAPS FOUND:\n");
                sb.append(isEstonian
                    ? "Ettevõtte hindamine (skoor: " + String.format("%.0f", assessment.getScorePercentage()) + "%) tuvastas järgmised lüngad selle poliitikaga seotud valdkondades:\n"
                    : "The company's assessment (score: " + String.format("%.0f", assessment.getScorePercentage()) + "%) identified the following gaps relevant to this policy:\n");
                for (String gap : gaps) {
    ...
    private String buildPrompt(PolicyWriterRequest request, PolicyTypeMeta meta, String gapsContext, boolean isEstonian) {
        StringBuilder sb = new StringBuilder();
        ...
        if (isEstonian) {
            ...
            sb.append("Create a professional, comprehensive, and implementation-ready policy document.\n\n");
        ...
        if (isEstonian) {
            sb.append("REEGLID:\n");
            sb.append("- gapsSummary: lühike kokkuvõte, kuidas dokument käsitleb tuvastatud lünki (kui lünki pole, siis null)\n");
            sb.append("- Vasta AINULT JSON objektiga, ilma lisatekstita\n");
        } else {
            sb.append("RULES:\n");
            sb.append("- gapsSummary: brief summary of how the document addresses identified gaps (null if no gaps)\n");
