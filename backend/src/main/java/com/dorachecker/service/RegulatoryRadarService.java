...
public class RegulatoryRadarService {
    ...
    public RegulatoryRadarService(RegulatoryUpdateRepository regulatoryUpdateRepository,
    ...
    public Map<String, Object> getRadarUpdates(String userId, String severity, String regulation, int page, int size) {
        ...
        if (regulation != null && !regulation.isBlank()) {
            allUpdates = allUpdates.stream()
                .filter(u -> u.getRegulations() != null && u.getRegulations().toLowerCase().contains(regulation.toLowerCase()))
                .collect(Collectors.toList());
        }

        // Get user's monitored contracts and providers for impact counts
        List<MonitoredContractEntity> userContracts = monitoredContractRepository.findByUserIdOrderByUpdatedAtDesc(userId);
        List<IctProviderEntity> userProviders = ictProviderRepository.findByUserIdOrderByCreatedAtDesc(userId);
        ...
        List<Map<String, Object>> items = pageContent.stream().map(u -> {
            ...
            item.put("publishedDate", u.getPublishedDate() != null ? u.getPublishedDate().toString() : null);
            ...
            item.put("effectiveDate", u.getEffectiveDate() != null ? u.getEffectiveDate().toString() : null);
    ...
    public Map<String, Object> getImpactAnalysis(String updateId, String userId) {
        ...
        List<Map<String, String>> contracts = userContracts.stream()
        ...
        List<Map<String, String>> providers = userProviders.stream()
            .map(p -> {
                Map<String, String> m = new LinkedHashMap<>();
                m.put("id", p.getId());
    ...
    private int countAffectedContracts(RegulatoryUpdateEntity update, List<MonitoredContractEntity> contracts) {
        return (int) contracts.stream()
            .filter(c -> "ACTIVE".equals(c.getMonitoringStatus()))
            .count();
    }

    private int countAffectedProviders(RegulatoryUpdateEntity update, List<IctProviderEntity> providers) {
