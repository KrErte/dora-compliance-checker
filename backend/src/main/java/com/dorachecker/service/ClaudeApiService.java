...
public class ClaudeApiService {
   ...
   * per-item strategies.
   */
  public String generateNegotiationStrategy(
      String companyName,
      String contractName,
      String vendorType,
      double score,
    ...
    sb.append(
        "Sa oled IKT-lepingute läbirääkimiste ekspert, kes tunneb hästi DORA (EU 2022/2554) nõudeid ja Eesti võlaõigusseadust (VÕS).\n\n");
    ...
    sb.append("TEENUSEPAKKUJA TÜÜP: ")
        .append(vendorType != null ? vendorType : "Määramata")
        .append("\n");
    sb.append("PRAEGUNE SKOOR: ").append(String.format("%.1f", score)).append("%\n\n");

    sb.append("TUVASTATUD LÜNGAD:\n");
    for (ContractFinding gap : gaps) {
      sb.append(gap.requirementId())
          .append(". [")
          .append(gap.doraReference())
          .append("] ")
          .append(gap.requirementEt())
          .append("\n");
    ...
    sb.append(
        "      \"strategy\": \"<konkreetne läbirääkimiste lähenemine selle nõude jaoks>\",\n");
  ...
  /** Generate a negotiation email draft for vendor. Returns raw JSON string with subject + body. */
  public String generateNegotiationEmail(
      String companyName,
      String contractName,
      String vendorType,
    ...
    sb.append(
        "Koosta professionaalne e-kiri teenusepakkujale lepingu täiendamise ettepanekuga.\n\n");
    ...
    for (NegotiationEmailItem item : items) {
      sb.append("- [")
          .append(item.articleReference())
          .append("] ")
          .append(item.requirementText())
          .append("\n");
  ...
  /**
   * Simple analyze method for generic prompts. Returns the AI response text (expects JSON format
   * from prompt).
   */
  ...
  /**
   * Assess whether a regulatory update is relevant to DORA Art. 30 requirements. Returns JSON with
   * relevanceScore (0-1), affectedArticles, and status.
   */
  public String assessRegulatoryRelevance(String title, String summary) {
    ...
    StringBuilder sb = new StringBuilder();
    sb.append(
        "Sa oled regulatiivse vastavuse ekspert, kes hindab uute regulatiivsete uuenduste mõju DORA (EU 2022/2554) artikli 30 nõuetele.\n\n");
  ...
  /**
   * Multi-turn chat API call with a system prompt. Returns raw assistant text (no JSON extraction).
   */
  public String analyzeChat(
      String systemPrompt, List<Map<String, String>> messages, int maxTokens) {
    if (apiKey == null || apiKey.isBlank()) {
    ...
    try {
      requestBody =
          objectMapper.writeValueAsString(
              new ApiRequest(model, maxTokens, List.of(new Message("user", prompt))));
    } catch (Exception e) {
  ...
  /**
   * Enrich a regulatory update with AI analysis. Returns JSON with summary, impactAnalysis,
   * regulations, companyTypes, countries, severity.
   */
  public String enrichRegulatoryUpdate(String title, String summary, String source) {
    ...
    StringBuilder sb = new StringBuilder();
    sb.append(
        "Sa oled regulatiivse vastavuse ekspert finantssektoris. Analüüsi järgmist regulatiivset uuendust ja koosta struktureeritud kokkuvõte.\n\n");
    sb.append("PEALKIRI: ").append(title).append("\n");
    if (summary != null && !summary.isBlank()) {
    ...
    sb.append(
        "  \"impactAnalysis\": \"<mõjuanalüüs eesti keeles - kuidas see mõjutab finantsettevõtteid>\",\n");
    sb.append(
        "  \"regulations\": \"<komaga eraldatud mõjutatud regulatsioonid: DORA, NIS2, AI_ACT, CSRD - ainult need, mis on asjakohased>\",\n");
    sb.append(
        "  \"companyTypes\": \"<komaga eraldatud mõjutatud ettevõttetüübid: BANK, FINTECH, INSURANCE, INVESTMENT_FIRM, ICT_PROVIDER, OTHER>\",\n");
    sb.append(
        "  \"countries\": \"<komaga eraldatud mõjutatud riigid: EST, LVA, LTU, EU - kasuta EU kui kehtib kogu EL-is>\",\n");
    sb.append("  \"severity\": \"<CRITICAL | HIGH | MEDIUM | LOW | INFO>\"\n");
    sb.append("}\n\n");
    sb.append("REEGLID:\n");
    sb.append(
        "- severity CRITICAL: kohene tegevus vajalik (uus kohustuslik nõue, tähtaeg läheneb)\n");
  ...
  public record NegotiationEmailItem(
      String articleReference, String requirementText, String suggestedClause) {}
