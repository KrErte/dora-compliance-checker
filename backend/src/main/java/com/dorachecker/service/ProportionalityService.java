...
public class ProportionalityService {
  ...
  public ProportionalityClassificationEntity classify(String userId, Map<String, Object> request) {
    ProportionalityClassificationEntity entity =
        repository.findByUserId(userId).orElse(new ProportionalityClassificationEntity());

    entity.setUserId(userId);
    entity.setEntityType((String) request.getOrDefault("entityType", ""));

    int employees =
        request.containsKey("employeeCount")
            ? ((Number) request.get("employeeCount")).intValue()
            : 0;
    BigDecimal revenue =
        request.containsKey("annualRevenue")
            ? new BigDecimal(request.get("annualRevenue").toString())
            : BigDecimal.ZERO;
    BigDecimal assets =
        request.containsKey("totalAssets")
    ...
    // Complexity
    int ictSystems =
        request.containsKey("ictSystemCount")
            ? ((Number) request.get("ictSystemCount")).intValue()
            : 0;
  ...
  public Map<String, Object> getEntityTypes() {
    ...
    for (Map.Entry<String, String> entry : DoraProportionalityRules.ENTITY_TYPES.entrySet()) {
      ...
      bySector
          .computeIfAbsent(sector, k -> new ArrayList<>())
