...
public class ComplianceAlertService {
    ...
    public ComplianceAlertService(EvidenceRepository evidenceRepository,
    ...
    public Map<String, Long> getAlertCounts(String userId) {
        ...
        return Map.of("critical", critical, "warning", warning, "info", info, "total", (long) alerts.size());
    }
    ...
    private void checkEvidenceAlerts(String userId, List<Map<String, Object>> alerts) {
        ...
        if (expired > 0) {
            alerts.add(createAlert("CRITICAL", "EVIDENCE", "evidence_expired",
        ...
        if (expiringUrgent > 0) {
            alerts.add(createAlert("CRITICAL", "EVIDENCE", "evidence_expiring_7d",
                expiringUrgent + " evidence document(s) expire within 7 days",
                "Renew these documents immediately to avoid audit gaps.",
                "/evidence-vault"));
        }
        if (expiringSoon > 0) {
            alerts.add(createAlert("WARNING", "EVIDENCE", "evidence_expiring_30d",
        ...
        if (unverified > 5) {
            alerts.add(createAlert("WARNING", "EVIDENCE", "evidence_unverified",
    ...
    private void checkRemediationAlerts(String userId, List<Map<String, Object>> alerts) {
        ...
        if (highOverdue > 0) {
            ...
            alerts.add(createAlert("WARNING", "REMEDIATION", "remediation_overdue",
        ...
        if (criticalOpen > 0 && highOverdue == 0) {
            alerts.add(createAlert("WARNING", "REMEDIATION", "remediation_critical_open",
    ...
    private void checkIncidentAlerts(String userId, List<Map<String, Object>> alerts) {
        ...
        long overdueInitial = incidents.stream()
            .filter(i -> i.getInitialReportDueAt() != null && i.getInitialReportSentAt() == null
                && i.getInitialReportDueAt().isBefore(now))
            .count();

        long overdueIntermediate = incidents.stream()
            .filter(i -> i.getIntermediateReportDueAt() != null && i.getIntermediateReportSentAt() == null
                && i.getIntermediateReportDueAt().isBefore(now))
            .count();

        long overdueFinal = incidents.stream()
            .filter(i -> i.getFinalReportDueAt() != null && i.getFinalReportSentAt() == null
        ...
        if (overdueInitial > 0) {
            alerts.add(createAlert("CRITICAL", "INCIDENT", "incident_initial_overdue",
        ...
        if (overdueIntermediate > 0) {
            alerts.add(createAlert("CRITICAL", "INCIDENT", "incident_intermediate_overdue",
                overdueIntermediate + " intermediate incident report(s) overdue",
                "Intermediate reports are due within 72 hours of the incident.",
                "/incident-reporting"));
        }
        if (overdueFinal > 0) {
            alerts.add(createAlert("WARNING", "INCIDENT", "incident_final_overdue",
        ...
        long urgentDeadlines = incidents.stream()
            .filter(i -> {
                if (i.getInitialReportDueAt() != null && i.getInitialReportSentAt() == null
                    && i.getInitialReportDueAt().isAfter(now) && i.getInitialReportDueAt().isBefore(now.plusHours(24)))
                    return true;
                if (i.getIntermediateReportDueAt() != null && i.getIntermediateReportSentAt() == null
                    && i.getIntermediateReportDueAt().isAfter(now) && i.getIntermediateReportDueAt().isBefore(now.plusHours(24)))
                    return true;
                return false;
            }).count();

        if (urgentDeadlines > 0) {
            alerts.add(createAlert("WARNING", "INCIDENT", "incident_deadline_24h",
    ...
    private void checkThirdPartyAlerts(String userId, List<Map<String, Object>> alerts) {
        ...
        long criticalWithoutExit = providers.stream()
            .filter(p -> p.isCritical() && !Boolean.TRUE.equals(p.getHasExitStrategy()))
            .count();

        long highRisk = providers.stream()
            .filter(p -> p.getRiskScore() != null && p.getRiskScore() >= 8)
            .count();

        LocalDate today = LocalDate.now();
        long contractsExpiring = providers.stream()
            .filter(p -> p.getContractEndDate() != null
        ...
        if (criticalWithoutExit > 0) {
            alerts.add(createAlert("CRITICAL", "THIRD_PARTY", "provider_no_exit",
        ...
        if (highRisk > 0) {
            alerts.add(createAlert("WARNING", "THIRD_PARTY", "provider_high_risk",
        ...
        if (contractsExpiring > 0) {
            alerts.add(createAlert("WARNING", "THIRD_PARTY", "provider_contract_expiring",
    ...
    private void checkAssessmentAlerts(String userId, List<Map<String, Object>> alerts) {
        ...
        if (assessments.isEmpty()) {
            alerts.add(createAlert("WARNING", "ASSESSMENT", "no_assessment",
        ...
        long daysSinceAssessment = ChronoUnit.DAYS.between(latest.getAssessmentDate(), LocalDateTime.now());

        if (daysSinceAssessment > 180) {
            alerts.add(createAlert("WARNING", "ASSESSMENT", "assessment_stale",
            ...
            alerts.add(createAlert("INFO", "ASSESSMENT", "assessment_aging",
        ...
        if (latest.getScorePercentage() < 40) {
            alerts.add(createAlert("CRITICAL", "ASSESSMENT", "assessment_critical_score",
            ...
            alerts.add(createAlert("WARNING", "ASSESSMENT", "assessment_low_score",
    ...
    private Map<String, Object> createAlert(String severity, String category, String alertKey,
