...
public class SubscriptionGuardService {
  ...
  public SubscriptionGuardService(
      UserSubscriptionRepository subscriptionRepository, UserRepository userRepository) {
  ...
  public boolean isPremium(String userId, String sessionId) {
    boolean hasPaidSub =
        findSubscription(userId, sessionId).map(UserSubscriptionEntity::isPremium).orElse(false);
    if (hasPaidSub) return true;

    // Check trial
    if (userId != null && !userId.isEmpty()) {
      return userRepository.findById(userId).map(UserEntity::isTrialValid).orElse(false);
    }
    return false;
  }
  ...
  public record SubscriptionStatus(
      String plan, boolean isPremium, String validUntil, String trialEndsAt) {}
}
