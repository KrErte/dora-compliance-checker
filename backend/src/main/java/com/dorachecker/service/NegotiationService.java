...
public class NegotiationService {
  ...
  public NegotiationResult create(String contractAnalysisId, String vendorType, String userId) {
    ...
    for (ContractFinding gap : gaps) {
      NegotiationItemEntity item =
          new NegotiationItemEntity(
              negotiation.getId(),
              gap.requirementId(),
              gap.doraReference(),
              gap.requirementEt(),
              gap.status().equals("missing") ? "HIGH" : "MEDIUM",
              gap.status(),
              gap.recommendationEt(),
              priority++);
      itemRepo.save(item);
    }
  ...
  public NegotiationResult generateStrategy(String negotiationId, String userId) {
    ...
    String responseJson =
        claudeApiService.generateNegotiationStrategy(
            negotiation.getCompanyName(),
            negotiation.getContractName(),
            negotiation.getVendorType(),
    ...
    try {
      ...
      if (root.has("items") && root.get("items").isArray()) {
        List<NegotiationItemEntity> items =
            itemRepo.findByNegotiationIdOrderByPriorityAsc(negotiationId);
        for (JsonNode itemNode : root.get("items")) {
          ...
          items.stream()
              ...
              .ifPresent(
                  item -> {
                    if (itemNode.has("strategy"))
                      item.setStrategy(itemNode.get("strategy").asText());
                    if (itemNode.has("suggestedClause"))
                      item.setSuggestedClause(itemNode.get("suggestedClause").asText());
                    if (itemNode.has("priority"))
                      item.setPriority(itemNode.get("priority").asInt());
  ...
  public NegotiationMessageResult generateEmail(String negotiationId, String userId) {
    ...
    List<NegotiationEmailItem> emailItems =
        items.stream()
            .map(
                i ->
                    new NegotiationEmailItem(
                        i.getArticleReference(), i.getRequirementText(), i.getSuggestedClause()))
            .toList();

    String responseJson =
        claudeApiService.generateNegotiationEmail(
            negotiation.getCompanyName(),
            negotiation.getContractName(),
            negotiation.getVendorType(),
            emailItems);
    ...
    try {
      JsonNode root = objectMapper.readTree(responseJson);
      String subject =
          root.has("subject") ? root.get("subject").asText() : "DORA lepingu täiendamine";
      String body = root.has("body") ? root.get("body").asText() : responseJson;

      NegotiationMessageEntity message =
          new NegotiationMessageEntity(
  ...
  public NegotiationMessageResult generateItemEmail(String itemId, String userId) {
    NegotiationItemEntity item =
        itemRepo
            .findById(itemId)
            .orElseThrow(
                () -> new IllegalArgumentException("Läbirääkimise punkti ei leitud: " + itemId));
    ...
    List<NegotiationEmailItem> emailItems =
        List.of(
            new NegotiationEmailItem(
                item.getArticleReference(), item.getRequirementText(), item.getSuggestedClause()));
    ...
    try {
      ...
      String subject =
          root.has("subject")
              ...
              : "DORA nõue: " + item.getArticleReference();
      String body = root.has("body") ? root.get("body").asText() : responseJson;
  ...
  public void updateItemStatus(String itemId, String newStatus, String userId) {
    ...
    List<NegotiationItemEntity> allItems =
        itemRepo.findByNegotiationIdOrderByPriorityAsc(negotiation.getId());
  ...
  public NegotiationMessageResult addMessage(
    ...
    NegotiationMessageEntity message =
        new NegotiationMessageEntity(negotiationId, itemId, messageType, direction, subject, body);
    message = messageRepo.save(message);
  ...
  private NegotiationEntity getNegotiationEntity(String negotiationId, String userId) {
    NegotiationEntity negotiation =
        negotiationRepo
            ...
            .orElseThrow(
                () -> new IllegalArgumentException("Läbirääkimist ei leitud: " + negotiationId));
  ...
  private NegotiationResult toResult(NegotiationEntity entity) {
    List<NegotiationItemEntity> items =
        itemRepo.findByNegotiationIdOrderByPriorityAsc(entity.getId());

    List<NegotiationItemResult> itemResults =
        items.stream()
            .map(
                item -> {
                  List<NegotiationMessageEntity> messages =
                      messageRepo.findByNegotiationItemIdOrderByCreatedAtAsc(item.getId());
                  List<NegotiationMessageResult> msgResults =
                      messages.stream()
                          .map(
                              m ->
                                  new NegotiationMessageResult(
                  ...
                  return new NegotiationItemResult(
                      ...
                      item.getArticleReference(),
                      item.getRequirementText(),
                      item.getGapSeverity(),
                      item.getCoverageStatus(),
                      item.getStrategy(),
                      item.getSuggestedClause(),
                      item.getStatus(),
                      item.getPriority(),
                      item.getNotes(),
                      msgResults);
