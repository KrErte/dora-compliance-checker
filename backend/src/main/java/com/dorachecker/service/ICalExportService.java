...
public class ICalExportService {
    ...
    public String generateIcal(String userId) {
        List<RemediationItemEntity> items = remediationRepository.findByUserIdOrderByCreatedAtDesc(userId)
                .stream()
        ...
        for (RemediationItemEntity item : items) {
            ...
            sb.append("DTEND;VALUE=DATE:").append(item.getDueDate().plusDays(1).format(ICAL_DATE)).append("\r\n");
            ...
            if (item.getPriority() != null) desc.append("Priority: ").append(item.getPriority()).append("\\n");
            if (item.getArticleReference() != null) desc.append("Article: ").append(item.getArticleReference()).append("\\n");
            if (item.getDescription() != null) desc.append("\\n").append(item.getDescription());
            sb.append("DESCRIPTION:").append(escapeIcal(desc.toString())).append("\r\n");

            sb.append("STATUS:").append("IN_PROGRESS".equals(item.getStatus()) ? "IN-PROCESS" : "NEEDS-ACTION").append("\r\n");

            // Set alarm 1 day before
            sb.append("BEGIN:VALARM\r\n");
            sb.append("TRIGGER:-P1D\r\n");
            sb.append("ACTION:DISPLAY\r\n");
            sb.append("DESCRIPTION:DORA deadline tomorrow: ").append(escapeIcal(item.getTitle())).append("\r\n");
