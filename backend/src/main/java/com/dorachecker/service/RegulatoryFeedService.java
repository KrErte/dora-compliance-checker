...
public class RegulatoryFeedService {
    ...
    /**
     * Scheduled check for regulatory updates from RSS feeds.
    ...
    public void checkRegulatoryFeeds() {
        ...
        try {
            ...
            List<RegulatoryUpdateEntity> needsEnrichment = updateRepository.findByStatusOrderByFetchedAtDesc("NEW");
            for (RegulatoryUpdateEntity update : needsEnrichment) {
                if (enriched >= maxEnrichmentPerCycle) {
                    log.info("AI rikastamise limiit ({}) saavutatud, jätkame järgmisel tsüklil", maxEnrichmentPerCycle);
                    break;
                }
                try {
                    enrichWithAi(update);
            ...
            log.info("Regulatiivsete uuenduste kontroll lõpetatud. Uusi: {}, AI rikastatud: {}", totalNew, enriched);
        } catch (Exception e) {
    ...
    private int fetchAndProcessSource(RegulatorySourceEntity source) {
        ...
        for (RssFeedParser.FeedItem item : items) {
            try {
                ...
                if (externalId != null && source.getId() != null &&
                ...
                RegulatoryUpdateEntity update = new RegulatoryUpdateEntity(
                        source.getName(),
    ...
    private void enrichWithAi(RegulatoryUpdateEntity update) {
        String enrichmentJson = claudeApiService.enrichRegulatoryUpdate(
                update.getTitle(),
        ...
        try {
            String relevanceJson = claudeApiService.assessRegulatoryRelevance(update.getTitle(), update.getSummary());
