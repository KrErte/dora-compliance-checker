...
public class MonitoredContractService {
  ...
  public MonitoredContractEntity startMonitoring(
      String contractAnalysisId, String contractText, String userId) {
    // Check if already monitored
    monitoredRepo
        .findByContractAnalysisIdAndUserId(contractAnalysisId, userId)
        .ifPresent(
            existing -> {
              throw new IllegalArgumentException(
                  "See leping on juba seire all: " + existing.getId());
            });

    ContractAnalysisEntity analysis =
        analysisRepo
            .findById(contractAnalysisId)
            .orElseThrow(
                () -> new IllegalArgumentException("Analüüsi ei leitud: " + contractAnalysisId));

    MonitoredContractEntity monitored =
        new MonitoredContractEntity(
            userId,
            contractAnalysisId,
            analysis.getCompanyName(),
            analysis.getContractName(),
            analysis.getFileName(),
            contractText,
            analysis.getScorePercentage(),
            analysis.getComplianceLevel());
  ...
  public MonitoredContractEntity getContract(String id, String userId) {
    MonitoredContractEntity contract =
        monitoredRepo
            .findById(id)
            .orElseThrow(
                () -> new IllegalArgumentException("Seiratavat lepingut ei leitud: " + id));
  ...
  public MonitoredContractEntity reanalyze(String id, String userId) {
    ...
    ContractAnalysisResult result =
        analysisService.analyzeFromText(
