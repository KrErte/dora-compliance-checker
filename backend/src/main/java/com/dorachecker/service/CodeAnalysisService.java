        ...
        if (qualityContext != null && !qualityContext.isBlank()) {
            qualityOverride = switch (qualityContext) {
                case "strong" -> """
                    ...
                    what happens when the original developer leaves, hidden coupling, performance cliffs under load.
                    Calculate: maintenance cost per year in developer-hours, onboarding cost for new developers in hours,
                ...
                case "average" -> """
                    ...
                    the 3am-on-Sunday bugs that cost 10x to fix, technical debt interest rate.
                    Calculate: hours per month spent on bug fixes, hours per incident, average incidents per quarter,
                ...
                case "weak" -> """
                    ...
                    Focus on: how many developer-hours will be WASTED debugging this, how many hours to rewrite vs patch,
                    the false economy of "fast" AI-generated code (saved 40 hours writing, will cost 400 hours maintaining).
                    Calculate: total hours already wasted (bugs found * avg 8h per bug), hours to properly rewrite,
                    cost comparison: rewrite now vs maintain for 2 years.
                    Show the PM the BRUTAL math: €85/h × hours_wasted = money burned. Make it hurt.
