...
public class RegulatoryTranslatorService {
  ...
  public RegulatoryTranslatorService(
      RegulatoryTranslationRepository translationRepo,
  ...
  public Map<String, Object> translateForUser(String userId, String updateId) {
    // Check cache first
    Optional<RegulatoryTranslationEntity> existing =
        translationRepo.findByUserIdAndRegulatoryUpdateId(userId, updateId);
    ...
    try {
      ...
      log.error(
          "Translation failed for userId={}, updateId={}: {}", userId, updateId, e.getMessage(), e);
  ...
  private Map<String, Object> buildUserContext(String userId) {
    ...
    ctx.put(
        "expiredEvidence",
        expiredEvidence.size() > 5 ? expiredEvidence.subList(0, 5) : expiredEvidence);
    ...
    // Top 5 contracts
    List<ContractAnalysisEntity> contracts =
        contractRepo.findByUserIdOrderByAnalysisDateDesc(userId);
    ...
    List<RemediationItemEntity> openItems =
        remediationRepo.findByUserIdAndStatusOrderByPriorityAsc(userId, "OPEN");
    List<RemediationItemEntity> inProgressItems =
        remediationRepo.findByUserIdAndStatusOrderByPriorityAsc(userId, "IN_PROGRESS");
    ...
    for (RemediationItemEntity r : openItems) {
      remediationItems.add(
          Map.of(
              ...
              r.getPriority(),
              "pillar",
              r.getPillar() != null ? r.getPillar() : "N/A"));
    }
    ...
    ctx.put(
        "remediationItems",
        remediationItems.size() > 15 ? remediationItems.subList(0, 15) : remediationItems);
  ...
  private String buildPrompt(RegulatoryUpdateEntity update, Map<String, Object> context) {
    ...
    sb.append(
        "Analyze it against this specific organization's compliance data and generate personalized action items.\n\n");
    ...
    if (update.getSummary() != null)
      sb.append("Summary: ").append(update.getSummary()).append("\n");
    if (update.getAiSummary() != null)
      sb.append("AI Summary: ").append(update.getAiSummary()).append("\n");
    if (update.getAffectedArticles() != null)
      sb.append("Affected Articles: ").append(update.getAffectedArticles()).append("\n");
    if (update.getAffectedPillars() != null)
      sb.append("Affected Pillars: ").append(update.getAffectedPillars()).append("\n");
    if (update.getEffectiveDate() != null)
      sb.append("Effective Date: ").append(update.getEffectiveDate()).append("\n");
    ...
    sb.append("{\n");
    sb.append(
        "  \"plainSummary\": \"A 2-3 sentence plain-language summary of what this update means for THIS organization specifically\",\n");
    sb.append(
        "  \"personalImpactScore\": <number 0-100, how much this update impacts THIS organization>,\n");
    sb.append("  \"riskLevel\": \"CRITICAL|HIGH|MEDIUM|LOW\",\n");
    sb.append(
        "  \"riskIfIgnored\": \"What could happen if this organization ignores this update\",\n");
    sb.append("  \"affectedAreas\": {\n");
    sb.append(
        "    \"assessmentGaps\": [\"specific gap 1 based on their assessment data\", ...],\n");
    sb.append(
        "    \"evidenceGaps\": [\"missing or expired evidence they need to address\", ...],\n");
    ...
    sb.append(
        "      \"overlapsWithExisting\": <true|false if similar to their existing remediation items>\n");
    sb.append("    }\n");
    sb.append("  ]\n");
    sb.append("}\n");
    sb.append(
        "Generate 3-8 actionable, specific action items based on the organization's actual data. ");
  ...
  public Map<String, Object> applyActionItems(
      String userId, String translationId, List<Integer> indices) {
    ...
    try {
      ...
      return Map.of(
          ...
          entity.getStatus(),
          "appliedActionItems",
          entity.getAppliedActionItems(),
          "totalActionItems",
          entity.getTotalActionItems());
  ...
  public List<Map<String, Object>> getTranslations(String userId) {
    List<RegulatoryTranslationEntity> entities =
        translationRepo.findByUserIdOrderByCreatedAtDesc(userId);
  ...
  public Map<String, Object> getStats(String userId) {
    ...
    List<RegulatoryTranslationEntity> all =
        translationRepo.findByUserIdOrderByCreatedAtDesc(userId);
    double avgImpact =
        all.stream()
    ...
    int totalActions =
        all.stream().mapToInt(RegulatoryTranslationEntity::getTotalActionItems).sum();
    int appliedActions =
        all.stream().mapToInt(RegulatoryTranslationEntity::getAppliedActionItems).sum();
  ...
  public Map<String, Object> getUpdatesWithTranslationStatus(String userId, int page, int size) {
    ...
    List<RegulatoryTranslationEntity> translations =
        translationRepo.findByUserIdOrderByCreatedAtDesc(userId);
    ...
    if (start >= allUpdates.size()) {
      return Map.of(
          "content", List.of(), "totalElements", allUpdates.size(), "page", page, "size", size);
    }
    ...
    try {
      if (e.getActionItemsJson() != null) {
        m.put(
            "actionItems",
            objectMapper.readValue(
                e.getActionItemsJson(), new TypeReference<List<Map<String, Object>>>() {}));
      }
      if (e.getAffectedAreasJson() != null) {
        m.put(
            "affectedAreas",
            objectMapper.readValue(
                e.getAffectedAreasJson(), new TypeReference<Map<String, Object>>() {}));
      }
    } catch (Exception ex) {
