...
/**
 * Crawler service for FI-licensed financial entities.
 * Populates company_profiles table with data from:
 * 1. Finantsinspektsioon (FI) register - license types and status
 * 2. Estonian Business Registry (Äriregister) - company details
 * 3. Domain security checks - SSL, headers, DNS
 */
@Service
public class CompanyProfileCrawlerService {
    ...
    private static final Map<String, Integer> LICENSE_TO_DORA_TIER = Map.ofEntries(
    ...
    public void runScheduledCrawl() {
        if (!crawlerEnabled) {
            log.info("Company Profile Crawler is disabled. Set crawler.company.enabled=true to activate.");
            return;
        }
        runFullCrawl();
    }
    ...
    public Map<String, Object> runFullCrawl() {
        ...
        try {
            ...
            log.info("New: {}, Updated: {}, Errors: {}", newCount.get(), updatedCount.get(), errorCount.get());
    ...
    private void crawlFiPage(String path, String licenseType) {
        ...
        try {
            Document doc = Jsoup.connect(url)
                    .userAgent("DoraAudit/1.0 (Compliance Checker)")
                    .timeout(30000)
                    .get();

            // FI website typically has tables or lists with company names
            // Try multiple selectors
            Elements companies = doc.select("table tbody tr, .entity-list li, .company-item, article.node");
            ...
            logCrawl(null, CrawlLogEntity.SOURCE_FI_REGISTER, CrawlLogEntity.STATUS_SUCCESS,
                    String.format("Crawled %s, found %d entries", path, companies.size()));

        } catch (Exception e) {
            log.error("Failed to crawl FI page {}: {}", path, e.getMessage());
            logCrawl(null, CrawlLogEntity.SOURCE_FI_REGISTER, CrawlLogEntity.STATUS_FAILED, e.getMessage());
            errorCount.incrementAndGet();
        ...
        for (Map<String, String> company : companies) {
            try {
                ...
                if (company.get("registryCode") != null) {
                    Optional<CompanyProfileEntity> profile = profileRepository.findByNameContaining(company.get("name"))
                            .stream().findFirst();
    ...
    private void processCompanyFromFi(String name, String licenseType) {
        Optional<CompanyProfileEntity> existing = profileRepository.findByNameContaining(name)
                .stream().findFirst();
    ...
    private void enrichFromAriregister(CompanyProfileEntity profile, String registryCode) {
        try {
            ...
            logCrawl(profile, CrawlLogEntity.SOURCE_ARIREGISTER, CrawlLogEntity.STATUS_FAILED, e.getMessage());
        }
    }

    private String findRegistryCodeByName(String companyName) {
        try {
            String url = ARIREGISTER_API + "/autocomplete?q=" + java.net.URLEncoder.encode(companyName, "UTF-8");
    ...
    public void checkDomainSecurity(CompanyProfileEntity profile) {
        ...
        try {
            ...
            logCrawl(profile, CrawlLogEntity.SOURCE_DOMAIN_CHECK, CrawlLogEntity.STATUS_PARTIAL, e.getMessage());
        }
    }
        ...
        try {
            ...
            try (SSLSocket socket = (SSLSocket) factory.createSocket(domain, 443)) {
                ...
                if (certs.length > 0) {
                    ...
                    long daysValid = ChronoUnit.DAYS.between(LocalDate.now(),
                            cert.getNotAfter().toInstant().atZone(java.time.ZoneId.systemDefault()).toLocalDate());
    ...
    private void logCrawl(CompanyProfileEntity profile, String source, String status, String details) {
    ...
    private List<Map<String, String>> getKnownFiCompanies(String licenseType) {
        ...
        switch (licenseType.toLowerCase()) {
            case "krediidiasutus" -> {
                companies.add(Map.of("name", "Swedbank AS", "registryCode", "10060701", "website", "swedbank.ee"));
                companies.add(Map.of("name", "SEB Pank AS", "registryCode", "10004252", "website", "seb.ee"));
                companies.add(Map.of("name", "Luminor Bank AS", "registryCode", "11315936", "website", "luminor.ee"));
                companies.add(Map.of("name", "LHV Pank AS", "registryCode", "10539549", "website", "lhv.ee"));
                companies.add(Map.of("name", "Coop Pank AS", "registryCode", "10237832", "website", "cooppank.ee"));
                companies.add(Map.of("name", "Bigbank AS", "registryCode", "10183757", "website", "bigbank.ee"));
                companies.add(Map.of("name", "Citadele banka Eesti filiaal", "registryCode", "11058255", "website", "citadele.ee"));
                companies.add(Map.of("name", "TBB pank AS", "registryCode", "10107368", "website", "tbb.ee"));
                companies.add(Map.of("name", "Holm Bank AS", "registryCode", "14895521", "website", "holmbank.ee"));
                companies.add(Map.of("name", "Inbank AS", "registryCode", "12001988", "website", "inbank.ee"));
            }
            case "makseasutus" -> {
                companies.add(Map.of("name", "Maksekeskus AS", "registryCode", "12268475", "website", "maksekeskus.ee"));
                companies.add(Map.of("name", "EveryPay AS", "registryCode", "12049458", "website", "every-pay.com"));
                companies.add(Map.of("name", "Montonio Finance OÜ", "registryCode", "14178439", "website", "montonio.com"));
                companies.add(Map.of("name", "Decta Limited Eesti filiaal", "registryCode", "14601044", "website", "decta.com"));
                companies.add(Map.of("name", "ConnectPay UAB Eesti filiaal", "registryCode", "16015229", "website", "connectpay.com"));
            }
            case "e-raha asutus" -> {
                companies.add(Map.of("name", "Wise Europe SA", "registryCode", "12259658", "website", "wise.com"));
                companies.add(Map.of("name", "Monese Ltd Eesti filiaal", "registryCode", "12513487", "website", "monese.com"));
                companies.add(Map.of("name", "Paysera LT, UAB Eesti filiaal", "registryCode", "12856056", "website", "paysera.ee"));
            }
            case "investeerimisühing" -> {
                companies.add(Map.of("name", "LHV Varahaldus AS", "registryCode", "10572453", "website", "lhv.ee/varahaldus"));
                companies.add(Map.of("name", "Admiral Markets AS", "registryCode", "10932555", "website", "admiralmarkets.com"));
                companies.add(Map.of("name", "Avaron Asset Management AS", "registryCode", "11319869", "website", "avaron.com"));
                companies.add(Map.of("name", "Trigon Asset Management AS", "registryCode", "10507141", "website", "trigoncapital.com"));
            }
            case "kindlustusselts" -> {
                companies.add(Map.of("name", "If P&C Insurance AS", "registryCode", "10100168", "website", "if.ee"));
                companies.add(Map.of("name", "Compensa Vienna Insurance Group", "registryCode", "10055752", "website", "compensa.ee"));
                companies.add(Map.of("name", "ERGO Insurance SE", "registryCode", "10017013", "website", "ergo.ee"));
                companies.add(Map.of("name", "Salva Kindlustuse AS", "registryCode", "10284984", "website", "salva.ee"));
                companies.add(Map.of("name", "Swedbank P&C Insurance AS", "registryCode", "10290546", "website", "swedbank.ee/kindlustus"));
                companies.add(Map.of("name", "Seesam Insurance AS", "registryCode", "10055752", "website", "seesam.ee"));
            }
            case "fondivalitseja" -> {
                companies.add(Map.of("name", "LHV Varahaldus AS", "registryCode", "10572453", "website", "lhv.ee"));
                companies.add(Map.of("name", "Swedbank Investeerimisfondid AS", "registryCode", "10328362", "website", "swedbank.ee/fondid"));
                companies.add(Map.of("name", "SEB Varahaldus AS", "registryCode", "10035169", "website", "seb.ee"));
                companies.add(Map.of("name", "Tuleva Fondid AS", "registryCode", "14497630", "website", "tuleva.ee"));
            }
        }
