...
public class IctAssetMapService {
  ...
  public Map<String, Object> addBusinessFunction(String userId, Map<String, Object> data) {
    ...
    if (map.functions.size() >= MAX_ITEMS_PER_USER) {
      return Map.of(
          "error", "Maximum number of business functions reached (" + MAX_ITEMS_PER_USER + ")");
    }
  ...
  public Map<String, Object> addLink(String userId, Map<String, Object> data) {
    ...
    if (map.links.size() >= MAX_ITEMS_PER_USER) {
      return Map.of(
          "error", "Maximum number of dependency links reached (" + MAX_ITEMS_PER_USER + ")");
    }

    // Validate source exists (must be a function or asset)
    List<IctProviderEntity> providers = providerRepo.findByUserIdOrderByCreatedAtDesc(userId);
    boolean sourceExists =
        map.functions.stream().anyMatch(f -> sourceId.equals(f.get("id")))
            || map.assets.stream().anyMatch(a -> sourceId.equals(a.get("id")));
    boolean targetExists =
        map.assets.stream().anyMatch(a -> targetId.equals(a.get("id")))
  ...
  public Map<String, Object> deleteBusinessFunction(String userId, String functionId) {
    ...
    map.links.removeIf(
        l -> functionId.equals(l.get("sourceId")) || functionId.equals(l.get("targetId")));
  ...
  public Map<String, Object> analyzeRisks(String userId) {
    ...
    for (Map<String, Object> func : funcsCopy) {
      ...
      } else if (supportingAssets == 1
          && ("CRITICAL".equals(criticality) || "IMPORTANT".equals(criticality))) {
        Map<String, Object> risk = new LinkedHashMap<>();
        risk.put("function", func.get("name"));
        risk.put("criticality", criticality);
        risk.put(
            "risk", "Single point of failure — only one ICT asset supports this critical function");
    ...
    for (var entry : providerDependencies.entrySet()) {
      if (entry.getValue() >= 3) {
        ...
        risk.put(
            ...
            "High concentration — "
                + entry.getValue()
                + " assets/functions depend on this provider");
    ...
    result.put(
        "totalRisks",
        singlePointsOfFailure.size() + concentrationRisks.size() + unmappedFunctions.size());
    result.put(
        "riskLevel",
        calculateOverallRisk(singlePointsOfFailure, concentrationRisks, unmappedFunctions));
    return result;
  }
  ...
  private Map<String, Object> calculateStats(
      UserAssetMap map, List<Map<String, Object>> providerNodes) {
    ...
    long criticalFunctions =
        map.functions.stream().filter(f -> "CRITICAL".equals(f.get("criticality"))).count();
  ...
  private String calculateOverallRisk(
    ...
    long criticalCount =
        spof.stream()
                .filter(
                    r -> "CRITICAL".equals(r.get("severity")) || "HIGH".equals(r.get("severity")))
                .count()
