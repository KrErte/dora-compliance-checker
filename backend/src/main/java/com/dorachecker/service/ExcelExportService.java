...
public class ExcelExportService {
  ...
  public byte[] generateAssessmentExcel(AssessmentEntity assessment) {
    try (XSSFWorkbook workbook = new XSSFWorkbook();
      ...
      row =
          addSummaryRow(
              summarySheet, row, boldStyle, "Compliance Level", assessment.getComplianceLevel());
      row =
          addSummaryRow(
              summarySheet,
              row,
              boldStyle,
              "Total Questions",
              String.valueOf(assessment.getTotalQuestions()));
      row =
          addSummaryRow(
              summarySheet,
              row,
              boldStyle,
              "Compliant",
              String.valueOf(assessment.getCompliantCount()));
      row =
          addSummaryRow(
              summarySheet,
              row,
              boldStyle,
              "Partial",
              String.valueOf(assessment.getPartialCount()));
      int nonCompliant =
          assessment.getTotalQuestions()
              - assessment.getCompliantCount()
              - assessment.getPartialCount();
      ...
      if (assessment.getAnswersJson() != null && !assessment.getAnswersJson().isBlank()) {
        try {
          ...
          for (Map<String, Object> a : answers) {
            ...
            dataRow
                .createCell(2)
                .setCellValue(getString(a, "question", getString(a, "questionEn", "")));
            dataRow.createCell(3).setCellValue(getString(a, "status", getString(a, "answer", "")));
    ...
    try (XSSFWorkbook workbook = new XSSFWorkbook();
      ...
      row =
          addSummaryRow(
              ...
              boldStyle,
              "Date",
              contract.getAnalysisDate() != null
                  ? contract.getAnalysisDate().format(DATE_FMT)
                  : "N/A");
      row =
          addSummaryRow(
              summarySheet,
              ...
              boldStyle,
              "Score",
              String.format("%.0f%%", contract.getScorePercentage()));
      row =
          addSummaryRow(
              summarySheet, row, boldStyle, "Compliance Level", contract.getComplianceLevel());
      row =
          addSummaryRow(
              summarySheet,
              row,
              boldStyle,
              "Total Requirements",
              String.valueOf(contract.getTotalRequirements()));
      row =
          addSummaryRow(
              summarySheet, row, boldStyle, "Found", String.valueOf(contract.getFoundCount()));
      row =
          addSummaryRow(
              summarySheet, row, boldStyle, "Missing", String.valueOf(contract.getMissingCount()));
      addSummaryRow(
          summarySheet, row, boldStyle, "Partial", String.valueOf(contract.getPartialCount()));
      summarySheet.autoSizeColumn(0);
      summarySheet.autoSizeColumn(1);
      ...
      if (contract.getFindingsJson() != null && !contract.getFindingsJson().isBlank()) {
        try {
          List<Map<String, Object>> findings =
              objectMapper.readValue(contract.getFindingsJson(), new TypeReference<>() {});
  ...
  private int addSummaryRow(
      Sheet sheet, int rowNum, CellStyle boldStyle, String label, String value) {
