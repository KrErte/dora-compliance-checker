...
public class IntegrationService {
    ...
    public void fireEvent(String userId, EventType eventType, String title, String message) {
        ...
        for (IntegrationConfigEntity config : configs) {
            ...
            try {
                ...
                log.error("Failed to send integration notification to {}: {}", config.getType(), e.getMessage());
            }
        }
    }
    ...
    public void sendTestMessage(IntegrationConfigEntity config) {
        ...
        String message = "This is a test notification from DoraAudit. Your integration is working correctly!";
        switch (config.getType()) {
    ...
    private boolean isEventEnabled(IntegrationConfigEntity config, EventType eventType) {
        if (config.getEvents() == null || config.getEvents().isBlank()) return true; // all events if not specified
        try {
    ...
    private void sendSlack(IntegrationConfigEntity config, String title, String message, EventType eventType) {
        ...
        Map<String, Object> payload = Map.of(
            "blocks", List.of(
                Map.of("type", "header", "text", Map.of("type", "plain_text", "text", emoji + " " + title)),
                Map.of("type", "section", "text", Map.of("type", "mrkdwn", "text", message)),
                Map.of("type", "context", "elements", List.of(
                    Map.of("type", "mrkdwn", "text", "DoraAudit | " + LocalDateTime.now().format(DateTimeFormatter.ofPattern("dd.MM.yyyy HH:mm")))
                ))
            )
        );
    ...
    private void sendTeams(IntegrationConfigEntity config, String title, String message, EventType eventType) {
        if (config.getWebhookUrl() == null || config.getWebhookUrl().isBlank()) return;
        ...
        Map<String, Object> payload = Map.of(
            ...
            "attachments", List.of(Map.of(
                ...
                "content", Map.of(
                    ...
                    "version", "1.4",
                    "body", List.of(
                        Map.of("type", "TextBlock", "text", title, "weight", "Bolder", "size", "Medium", "color", color.equals("#dc2626") ? "Attention" : (color.equals("#f59e0b") ? "Warning" : "Good")),
                        Map.of("type", "TextBlock", "text", message, "wrap", true),
                        Map.of("type", "TextBlock", "text", "DoraAudit | " + LocalDateTime.now().format(DateTimeFormatter.ofPattern("dd.MM.yyyy HH:mm")), "size", "Small", "isSubtle", true)
                    )
                )
            ))
        );
    ...
    private void sendWebhook(IntegrationConfigEntity config, String title, String message, EventType eventType) {
        if (config.getWebhookUrl() == null || config.getWebhookUrl().isBlank()) return;

        Map<String, Object> payload = Map.of(
            "event", eventType != null ? eventType.name() : "TEST",
            "title", title,
            "message", message,
