...
public class GapAnalysisService {
    ...
    try {
      findings =
          objectMapper.readValue(
              entity.getFindingsJson(), new TypeReference<List<GapFinding>>() {});
    } catch (Exception e) {
      findings = List.of();
    }
    return toResult(entity, findings);
  ...
  private String buildPrompt(String documentText, List<Requirement> requirements) {
    ...
    sb.append(
        "IMPORTANT: The document may be a policy, procedure, framework, test report, audit report, etc. ");
    sb.append("It does NOT need to reference DORA directly. Evaluate based on substance — ");
    sb.append(
        "look for equivalent content using standard compliance, security, and business terminology.\n\n");
    sb.append(
        "For EACH requirement below, classify as \"found\", \"partial\", or \"missing\":\n\n");

    Map<String, List<Requirement>> grouped =
        requirements.stream()
            .collect(
                Collectors.groupingBy(
                    Requirement::articleNumber, LinkedHashMap::new, Collectors.toList()));
    ...
    for (Map.Entry<String, List<Requirement>> entry : grouped.entrySet()) {
      ...
      for (Requirement req : reqs) {
        sb.append(req.id())
            .append(". ")
            .append(req.requirementEn())
            .append(" - ")
            .append(req.doraReference())
            .append("\n");
    ...
    sb.append(
        "      \"quoteFromDocument\": \"<exact quote from document supporting the classification, empty if missing>\",\n");
    sb.append(
        "      \"recommendationEt\": \"<specific recommendation in Estonian, empty if found>\",\n");
    sb.append(
        "      \"recommendationEn\": \"<specific recommendation in English, empty if found>\",\n");
    ...
    sb.append("}\n\n");
    sb.append("The results array must contain exactly ")
