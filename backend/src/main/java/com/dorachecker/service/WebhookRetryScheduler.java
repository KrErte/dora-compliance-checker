...
public class WebhookRetryScheduler {
    ...
    public void retryFailedDeliveries() {
        List<WebhookDeliveryEntity> failed = deliveryRepository
                .findBySuccessFalseAndAttemptCountLessThanAndNextRetryAtBefore(MAX_ATTEMPTS, LocalDateTime.now());
        ...
        for (WebhookDeliveryEntity delivery : failed) {
            configRepository.findById(delivery.getIntegrationId()).ifPresent(config -> {
                ...
                try {
                    ...
                    delivery.setResponseBody(response != null && response.length() > 500 ? response.substring(0, 500) : response);
                    ...
                    log.warn("Webhook retry failed for delivery {} (attempt {}): {}",
