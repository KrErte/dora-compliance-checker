...
public class EvidenceHarvesterService {
  ...
  public EvidenceHarvesterService(
      HarvesterConnectorRepository connectorRepository, EvidenceRepository evidenceRepository) {
  ...
  public Map<String, Object> getSupportedConnectors() {
    ...
    connectors.add(
        buildConnectorDef(
            "JIRA",
            "Jira",
            "Import issues, sprints, and change records as compliance evidence",
            "bug_report",
            "PROJECT_MANAGEMENT",
    ...
    connectors.add(
        buildConnectorDef(
            "GITHUB",
            "GitHub",
            "Harvest pull requests, CI/CD logs, and code reviews",
            "code",
            "VERSION_CONTROL",
            List.of(
                "Pull Requests", "CI/CD Logs", "Code Reviews", "Release Notes", "Security Scans")));

    connectors.add(
        buildConnectorDef(
            "AZURE_DEVOPS",
            "Azure DevOps",
            "Collect work items, pipelines, and test results",
            "cloud_queue",
            "PROJECT_MANAGEMENT",
    ...
    connectors.add(
        buildConnectorDef(
            "AWS_CLOUDTRAIL",
            "AWS CloudTrail",
            "Ingest cloud audit logs and access events",
            "cloud_done",
            "CLOUD",
            List.of(
                "API Audit Logs", "Access Events", "Configuration Changes", "Security Findings")));

    connectors.add(
        buildConnectorDef(
            "SLACK",
            "Slack",
            "Capture incident channels, decision threads, and approvals",
            "chat",
            "COMMUNICATION",
            List.of(
                "Incident Channels", "Decision Records", "Approval Threads", "Status Updates")));

    connectors.add(
        buildConnectorDef(
            "CONFLUENCE",
            "Confluence",
            "Import policy documents, runbooks, and meeting notes",
            "article",
            "DOCUMENTATION",
    ...
    connectors.add(
        buildConnectorDef(
            "GOOGLE_DRIVE",
            "Google Drive",
            "Sync shared compliance folders and documents",
            "folder_shared",
            "DOCUMENTATION",
  ...
  public HarvesterConnectorEntity saveConnector(
      String userId,
      String connectorType,
      String displayName,
      String configJson,
      String mappingRulesJson) {
    Optional<HarvesterConnectorEntity> existing =
        connectorRepository.findByUserIdAndConnectorType(userId, connectorType);
  ...
  public Map<String, Object> getHarvestStats(String userId) {
    ...
    long activeConnectors =
        connectors.stream()
            ...
            .count();
    int totalHarvested =
        connectors.stream().mapToInt(HarvesterConnectorEntity::getEvidenceCount).sum();
    ...
    if (connectedTypes.contains("CONFLUENCE") || connectedTypes.contains("GOOGLE_DRIVE"))
      ictRisk += 20;
    ...
    if (connectedTypes.contains("CONFLUENCE") || connectedTypes.contains("GOOGLE_DRIVE"))
      thirdParty += 40;
    if (connectedTypes.contains("JIRA") || connectedTypes.contains("AZURE_DEVOPS"))
      thirdParty += 25;
    ...
    String[] sources = {
      "JIRA", "GITHUB", "AWS_CLOUDTRAIL", "CONFLUENCE", "SLACK", "AZURE_DEVOPS", "GOOGLE_DRIVE"
    };
    ...
    for (int i = 0; i < 10; i++) {
      ...
      LocalDateTime ts =
          LocalDateTime.now().minusHours(rand.nextInt(72)).minusMinutes(rand.nextInt(60));
  ...
  private Map<String, Object> buildConnectorDef(
      String type,
      String label,
      String description,
      String icon,
      String category,
      List<String> mappableEvidence) {
  ...
  private String[] getTemplatesForType(String type) {
    return switch (type) {
      ...
      case "AZURE_DEVOPS" ->
          new String[] {
            "Work Item", "Pipeline Build", "Test Suite Run", "Deployment Gate", "Board Review"
          };
      case "AWS_CLOUDTRAIL" ->
          new String[] {
            "API Call Audit",
