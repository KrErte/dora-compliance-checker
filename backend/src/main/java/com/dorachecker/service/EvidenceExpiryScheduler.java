...
public class EvidenceExpiryScheduler {
    ...
    public void sendExpiryReminders() {
        ...
        List<EvidenceEntity> expiringSoon = evidenceRepository.findByExpiryDateBetweenAndStatusNot(today, thirtyDays, "EXPIRED");
        ...
        log.info("Sending evidence expiry reminders to {} users for {} items", byUser.size(), expiringSoon.size());
        ...
        for (Map.Entry<String, List<EvidenceEntity>> entry : byUser.entrySet()) {
            ...
            try {
                String subject = "Tõendite aegumise meeldetuletus – " + items.size() + " dokument" + (items.size() > 1 ? "i" : "");
                ...
                log.error("Failed to send evidence expiry reminder to {}: {}", user.getEmail(), e.getMessage());
            }
        }
    }
    ...
    private String buildExpiryEmail(UserEntity user, List<EvidenceEntity> items) {
        ...
        for (EvidenceEntity item : items) {
            long daysLeft = java.time.temporal.ChronoUnit.DAYS.between(LocalDate.now(), item.getExpiryDate());
