...
/**
 * Pure classification engine encoding the EU AI Act risk classification logic. No database
 * dependencies — takes answers, returns result.
 *
 * <p>Classification order: 1. Prohibited AI check (Article 5) → UNACCEPTABLE 2. Annex III high-risk
 * areas → HIGH (unless exempted by Art. 6(3)) 3. Article 6(1) safety component → HIGH 4. Article 50
 * transparency → LIMITED 5. Default → MINIMAL
 */
@Component
public class AiActClassificationEngine {
  ...
  private static final Set<String> HIGH_RISK_ANNEX_III_KEYS =
      Set.of(
          "HIGH_BIOMETRICS",
  ...
  private static final Set<String> HIGH_RISK_SAFETY_KEYS =
      Set.of("HIGH_SAFETY_COMPONENT", "HIGH_SAFETY_THIRD_PARTY");

  private static final Set<String> LIMITED_RISK_KEYS =
      Set.of("LIMITED_CHATBOT", "LIMITED_SYNTHETIC_CONTENT", "LIMITED_EMOTION_RECOGNITION");

  private static final Set<String> EXEMPTION_KEYS =
      Set.of(
  ...
  private static final Map<String, String> PROHIBITED_ARTICLES =
      Map.of(
  ...
  private static final Map<String, String> HIGH_RISK_ARTICLES =
      Map.of(
  ...
  public ClassificationResult classify(Map<String, String> answers) {
    ...
    if (!triggeredSafety.isEmpty()) {
      List<String> articles =
          new ArrayList<>(List.of("Article 6(1) — Safety component of regulated product"));
      articles.addAll(getHighRiskObligationArticles());
      return new ClassificationResult(
          "HIGH",
    ...
    if (!triggeredLimited.isEmpty()) {
      ...
      if (isYes(answers, "LIMITED_CHATBOT"))
        articles.add("Article 50(1) — AI interaction disclosure");
      if (isYes(answers, "LIMITED_SYNTHETIC_CONTENT"))
        articles.add("Article 50(2)(4) — Synthetic content labeling");
      if (isYes(answers, "LIMITED_EMOTION_RECOGNITION"))
        articles.add("Article 50(3) — Emotion recognition disclosure");
      return new ClassificationResult(
          "LIMITED",
  ...
  private List<String> getTriggeredKeys(Map<String, String> answers, Set<String> keys) {
    return keys.stream().filter(key -> isYes(answers, key)).toList();
  }
