...
public class BulkImportService {
    ...
    @Autowired
    private RemediationItemRepository remediationRepository;

    @Autowired
    private IctProviderRepository ictProviderRepository;

    @Autowired
    private VendorRiskRepository vendorRiskRepository;
    ...
    private static final Map<String, String[]> TEMPLATE_HEADERS = Map.of(
        "REMEDIATION", new String[]{"title", "description", "priority", "status", "dueDate", "pillar", "assignee", "articleReference", "notes"},
        "ICT_ASSET", new String[]{"providerName", "serviceType", "serviceDescription", "providerCountry", "leiCode", "contractStartDate", "contractEndDate", "isCritical", "dataLocation", "exitStrategyStatus", "notes"},
        "VENDOR", new String[]{"vendorName", "vendorCategory", "riskLevel", "averageScore", "commonGaps"},
        "EVIDENCE", new String[]{"title", "description", "category", "pillar", "fileName", "expiryDate", "articleNumbers", "notes"}
    );
    ...
    static {
        ...
        VALID_VALUES.put("REMEDIATION.pillar", Set.of("ICT_RISK_MANAGEMENT", "INCIDENT_MANAGEMENT", "TESTING", "THIRD_PARTY", "INFORMATION_SHARING"));
        VALID_VALUES.put("ICT_ASSET.serviceType", Set.of("CLOUD", "SOFTWARE", "INFRASTRUCTURE", "NETWORK", "DATA_CENTER", "SECURITY", "COMMUNICATION", "OTHER"));
        VALID_VALUES.put("ICT_ASSET.exitStrategyStatus", Set.of("DEFINED", "IN_PROGRESS", "NOT_STARTED", "NOT_APPLICABLE"));
        VALID_VALUES.put("VENDOR.riskLevel", Set.of("LOW", "MEDIUM", "HIGH", "CRITICAL"));
        VALID_VALUES.put("EVIDENCE.category", Set.of("POLICY", "PROCEDURE", "TEST_REPORT", "TRAINING_CERTIFICATE", "CONTRACT", "MEETING_MINUTES", "AUDIT_REPORT", "RISK_ASSESSMENT", "OTHER"));
        VALID_VALUES.put("EVIDENCE.pillar", Set.of("ICT_RISK_MANAGEMENT", "INCIDENT_MANAGEMENT", "TESTING", "THIRD_PARTY", "INFORMATION_SHARING"));
    }

    // ─── Parse File ────────────────────────────────────────

    /**
     * Parses a CSV MultipartFile into a list of rows (header-keyed maps).
     * Reuses the parseCsvLine pattern from RemediationController.
     */
    public Map<String, Object> parseFile(MultipartFile file) {
        ...
        try (BufferedReader reader = new BufferedReader(
            ...
            while ((line = reader.readLine()) != null) {
                rowNum++;
                if (rowNum > MAX_ROWS) {
                    parseErrors.add(Map.of("row", rowNum, "message", "Exceeded maximum of " + MAX_ROWS + " rows"));
                    break;
                }
    ...
    /**
     * Validates rows against entity-specific rules.
     * Returns a list of error objects (row + field + message).
     */
    public List<Map<String, Object>> validate(String entityType, List<Map<String, String>> rows, Map<String, String> mapping) {
        ...
        for (int i = 0; i < rows.size(); i++) {
            ...
            for (String required : requiredFields) {
                ...
                if (csvCol == null) {
                    errors.add(Map.of("row", rowNum, "field", required, "message", "Required field '" + required + "' is not mapped"));
                } else {
                    String val = row.get(csvCol);
                    if (val == null || val.isBlank()) {
                        errors.add(Map.of("row", rowNum, "field", required, "message", "Required field '" + required + "' is empty"));
                    }
                }
            }
            ...
            for (Map.Entry<String, String> m : mapping.entrySet()) {
                ...
                if (allowed != null && !allowed.contains(val.toUpperCase())) {
                    errors.add(Map.of("row", rowNum, "field", entityField,
                        "message", "Invalid value '" + val + "' for " + entityField + ". Allowed: " + allowed));
                }
                ...
                if (entityField.toLowerCase().contains("date")) {
                    try {
                        ...
                        errors.add(Map.of("row", rowNum, "field", entityField,
                            "message", "Invalid date format '" + val + "'. Expected: YYYY-MM-DD"));
                    }
                }
                ...
                if (entityField.equals("averageScore")) {
                    try {
                        ...
                        errors.add(Map.of("row", rowNum, "field", entityField,
                            "message", "Invalid number '" + val + "' for " + entityField));
                    }
                }
            }
        }
    ...
    /**
     * Executes the actual import, saving rows to the appropriate repository.
     * Returns a result map with imported count, skipped count, and error details.
     */
    public Map<String, Object> executeImport(String userId, String entityType,
        ...
        for (int i = 0; i < rows.size(); i++) {
            ...
            try {
                ...
                errors.add(Map.of("row", rowNum, "message", e.getMessage() != null ? e.getMessage() : "Unknown error"));
                skipped++;
            }
        }
    ...
    private void importRemediation(String userId, Map<String, String> row, Map<String, String> reverseMapping) {
        ...
        if (dueDateStr != null && !dueDateStr.isBlank()) {
            try { entity.setDueDate(LocalDate.parse(dueDateStr)); } catch (Exception ignored) {}
        }

        remediationRepository.save(entity);
    }

    private void importIctAsset(String userId, Map<String, String> row, Map<String, String> reverseMapping) {
        ...
        entity.setCritical("true".equalsIgnoreCase(criticalStr) || "yes".equalsIgnoreCase(criticalStr) || "1".equals(criticalStr));

        String startDateStr = getMapped(row, reverseMapping, "contractStartDate", null);
        if (startDateStr != null && !startDateStr.isBlank()) {
            try { entity.setContractStartDate(LocalDate.parse(startDateStr)); } catch (Exception ignored) {}
        }

        String endDateStr = getMapped(row, reverseMapping, "contractEndDate", null);
        if (endDateStr != null && !endDateStr.isBlank()) {
            try { entity.setContractEndDate(LocalDate.parse(endDateStr)); } catch (Exception ignored) {}
        }
    ...
    private void importEvidence(String userId, Map<String, String> row, Map<String, String> reverseMapping) {
        ...
        if (expiryStr != null && !expiryStr.isBlank()) {
            try { entity.setExpiryDate(LocalDate.parse(expiryStr)); } catch (Exception ignored) {}
        }
    ...
    public String generateTemplate(String entityType) {
        ...
        // Add one example row
        switch (type) {
            case "REMEDIATION" -> sb.append("\"Implement MFA for admin accounts\",\"Deploy multi-factor authentication for all privileged accounts\",HIGH,OPEN,2026-06-30,ICT_RISK_MANAGEMENT,\"John Doe\",\"Art. 9(4)\",\"Urgent per audit finding\"");
            case "ICT_ASSET" -> sb.append("\"AWS\",CLOUD,\"Cloud infrastructure and compute services\",Ireland,,,2027-12-31,false,EU,DEFINED,\"Primary cloud provider\"");
            case "VENDOR" -> sb.append("\"Acme Cloud Services\",CLOUD,MEDIUM,72,\"Missing exit strategy clause\"");
            case "EVIDENCE" -> sb.append("\"ICT Risk Policy v2.1\",\"Board-approved ICT risk management policy\",POLICY,ICT_RISK_MANAGEMENT,\"ict-risk-policy-v2.1.pdf\",2027-01-15,\"5,6,7\",\"Approved in Q1 board meeting\"");
        }
        sb.append("\n");
    ...
    /**
     * Gets the mapped value from a row using the reverse mapping.
     * Falls back to defaultValue if the field is not mapped or empty.
     */
    private String getMapped(Map<String, String> row, Map<String, String> reverseMapping,
                              String entityField, String defaultValue) {
     ...
     * Reused from RemediationController pattern.
     */
