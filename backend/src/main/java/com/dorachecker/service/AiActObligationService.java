...
public class AiActObligationService {
  ...
  public AiActObligationService(
      AiActObligationRepository obligationRepository, AiSystemRepository aiSystemRepository) {
  ...
  @Transactional
  public Optional<AiActObligationEntity> updateObligation(
      String obligationId,
      String aiSystemId,
      String status,
      String notes,
      ...
      String dueDateStr) {
    Optional<AiActObligationEntity> opt =
        obligationRepository.findByIdAndAiSystemId(obligationId, aiSystemId);
  ...
  @Transactional
  public void generateObligations(
      String aiSystemId, String userId, AiActClassificationEngine.ClassificationResult result) {
  ...
  public void recalculateComplianceScore(String aiSystemId) {
    ...
    long notApplicable =
        obligationRepository.countByAiSystemIdAndStatus(aiSystemId, "NOT_APPLICABLE");
  ...
  private String generateDescription(String article, String riskLevel) {
    return String.format(
        "Compliance obligation under %s for %s risk AI system. "
            + "Review the AI Act regulation for detailed requirements.",
        article, riskLevel);
  }
}
