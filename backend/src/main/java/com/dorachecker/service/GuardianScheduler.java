...
public class GuardianScheduler {
  ...
  /**
   * Weekly re-analysis of all active monitored contracts. Runs every Monday at 03:00 to minimize
   * impact. Uses AI (Claude) for re-analysis, so gated by config flag.
   */
  ...
  public void reanalyzeActiveContracts() {
    ...
    log.info(
        "Guardian scheduler: starting re-analysis of {} active contracts", activeContracts.size());
    ...
    for (MonitoredContractEntity contract : activeContracts) {
      try {
        ...
        success++;
        log.info(
            "Re-analyzed contract '{}' (ID: {}) for user {}",
        ...
        failed++;
        log.error(
            "Failed to re-analyze contract '{}' (ID: {}): {}",
            contract.getContractName(),
            contract.getId(),
            e.getMessage());
      }
    ...
    log.info(
        "Guardian scheduler complete: {} succeeded, {} failed out of {} total",
        success,
        failed,
