...
public class RoiExportService {
    ...
    public byte[] exportXbrlCsvZip(RoiRegisterEntity register) throws IOException {
        ...
        String reportDate = register.getReportingDate() != null
            ? register.getReportingDate().toString() : LocalDate.now().toString();
        String timestamp = LocalDateTime.now().format(ZIP_TS);

        String zipName = lei + "." + scope + "_" + country + "_DORA010100_DORA_" + reportDate + "_" + timestamp;
        ...
        try (ZipOutputStream zos = new ZipOutputStream(baos)) {
            ...
            // parameters.csv
            String paramsCsv = "parameterId,parameterValue\nrs,rs:" + lei + "\nreportingPeriod," + reportDate + "\n";
    ...
    public byte[] exportExcel(RoiRegisterEntity register) throws IOException {
        try (XSSFWorkbook workbook = new XSSFWorkbook()) {
            ...
            addRow(s1, 0, headerStyle, "C0010_LEI","C0020_EntityName","C0030_Country","C0040_EntityType","C0050_CompetentAuthority","C0060_ConsolidationScope","C0070_ReportingDate");
            addRow(s1, 1, null, safe(register.getEntityLei()), safe(register.getEntityName()), safe(register.getCountry()), safe(register.getEntityType()), safe(register.getCompetentAuthority()), register.getConsolidationScope().name(), dateStr(register.getReportingDate()));

            // B_01.02
            Sheet s2 = workbook.createSheet("B_01.02");
            addRow(s2, 0, headerStyle, "C0010_LEI","C0020_EntityName","C0030_Country","C0040_EntityType","C0050_CompetentAuthority","C0060_ParentLEI");
            int row = 1;
            for (RoiGroupEntityEntry ge : register.getGroupEntities()) {
                addRow(s2, row++, null, safe(ge.getLei()), safe(ge.getEntityName()), safe(ge.getCountry()), safe(ge.getEntityType()), safe(ge.getCompetentAuthority()), safe(ge.getParentEntityLei()));
            }

            // B_02.01
            Sheet s3 = workbook.createSheet("B_02.01");
            addRow(s3, 0, headerStyle, "C0010_RefNumber","C0020_Type","C0030_OverarchingRef","C0040_Currency","C0050_AnnualCost","C0060_StartDate","C0070_EndDate","C0080_NoticePeriod","C0090_ReasonForCriticality");
            row = 1;
            for (RoiContractEntity c : register.getContracts()) {
                addRow(s3, row++, null, safe(c.getContractRefNumber()), safe(c.getContractType()), safe(c.getOverarchingRefNumber()), safe(c.getCurrency()), c.getAnnualCost() != null ? c.getAnnualCost().toString() : "", dateStr(c.getStartDate()), dateStr(c.getEndDate()), c.getNoticePeriodDays() != null ? c.getNoticePeriodDays().toString() : "", safe(c.getReasonForCriticality()));
            }

            // B_05.01
            Sheet s4 = workbook.createSheet("B_05.01");
            addRow(s4, 0, headerStyle, "C0010_Identifier","C0020_CodeType","C0030_Name","C0040_Type","C0050_Country","C0060_Currency","C0070_AnnualSpend","C0080_ParentLEI","C0090_ParentName","C0100_ParentCountry");
            row = 1;
            for (RoiProviderEntity p : register.getProviders()) {
                addRow(s4, row++, null, safe(p.getProviderIdentifier()), safe(p.getIdentificationCodeType()), safe(p.getProviderName()), safe(p.getProviderType()), safe(p.getCountryOfHq()), safe(p.getCurrencyOfContract()), p.getTotalAnnualSpend() != null ? p.getTotalAnnualSpend().toString() : "", safe(p.getUltimateParentLei()), safe(p.getUltimateParentName()), safe(p.getUltimateParentCountry()));
            }

            // B_06.01
            Sheet s5 = workbook.createSheet("B_06.01");
            addRow(s5, 0, headerStyle, "C0010_FunctionId","C0020_LicensedActivity","C0030_FunctionName","C0040_EntityLEI","C0050_Criticality","C0060_Reasons");
            row = 1;
            for (RoiFunctionEntity f : register.getFunctions()) {
                addRow(s5, row++, null, safe(f.getFunctionIdentifier()), safe(f.getLicensedActivity()), safe(f.getFunctionName()), safe(f.getEntityLei()), safe(f.getCriticalityAssessment()), safe(f.getReasonsForCriticality()));
            }

            // B_07.01
            Sheet s6 = workbook.createSheet("B_07.01");
            addRow(s6, 0, headerStyle, "C0010_ContractRef","C0020_AssessmentDate","C0030_Reliance","C0040_Alternatives","C0050_Substitutability","C0060_LastAudit","C0070_ExitStrategy","C0080_RiskLevel");
            row = 1;
            for (RoiAssessmentEntity a : register.getAssessments()) {
                addRow(s6, row++, null, safe(a.getContractRefNumber()), dateStr(a.getAssessmentDate()), safe(a.getLevelOfReliance()), a.getAlternativeProvidersIdentified() != null ? a.getAlternativeProvidersIdentified().toString() : "", safe(a.getSubstitutabilityAssessment()), dateStr(a.getLastAuditDate()), a.getExitStrategyExists() != null ? a.getExitStrategyExists().toString() : "", safe(a.getRiskLevel()));
            }
    ...
    private String buildB0201(RoiRegisterEntity r) {
        ...
        for (RoiContractEntity c : r.getContracts()) {
            ...
            sb.append(c.getNoticePeriodDays() != null ? c.getNoticePeriodDays().toString() : "").append(",");
    ...
    private String buildB0501(RoiRegisterEntity r) {
        ...
        for (RoiProviderEntity p : r.getProviders()) {
            ...
            sb.append(p.getTotalAnnualSpend() != null ? p.getTotalAnnualSpend().toString() : "").append(",");
    ...
    private String buildB0701(RoiRegisterEntity r) {
        ...
        for (RoiAssessmentEntity a : r.getAssessments()) {
            ...
            sb.append(a.getAlternativeProvidersIdentified() != null ? a.getAlternativeProvidersIdentified().toString() : "").append(",");
            sb.append(csv(a.getSubstitutabilityAssessment())).append(",");
            sb.append(csv(dateStr(a.getLastAuditDate()))).append(",");
            sb.append(a.getExitStrategyExists() != null ? a.getExitStrategyExists().toString() : "").append(",");
