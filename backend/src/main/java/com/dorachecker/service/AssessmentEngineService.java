...
public class AssessmentEngineService {
  ...
  public AssessmentResult calculateScore(String regulationCode, Map<String, Integer> answers) {
    List<ComplianceDomainEntity> domains =
        regulationService.getDomainsWithQuestions(regulationCode);
    ...
    for (ComplianceDomainEntity domain : domains) {
      ...
      double domainWeight =
          questions.stream().mapToDouble(ComplianceQuestionEntity::getWeight).average().orElse(1.0);
  ...
  private int getTotalQuestionCount(List<ComplianceDomainEntity> domains) {
    return domains.stream().mapToInt(d -> d.getQuestions().size()).sum();
  }
