...
public class B04ServiceUserRules implements RoiValidationRule {
  ...
  public List<RoiViolation> evaluate(RoiValidationContext ctx) {
    ...
    for (RoiServiceUserEntity su : ctx.getRegister().getServiceUsers()) {
      // LEI format
      if (su.getEntityLei() != null
          && !su.getEntityLei().isBlank()
      ...
      if (su.getContractRefNumber() != null
          && !su.getContractRefNumber().isBlank()
        ...
        violations.add(
            new RoiViolation(
                ...
                "B_04.01",
                "C0010",
                su.getContractRefNumber(),
                "Teenuse kasutaja lepingu viide '"
                    + su.getContractRefNumber()
                    + "' ei leidu B_02.01 tabelis",
                "Service user contract ref '"
                    + su.getContractRefNumber()
