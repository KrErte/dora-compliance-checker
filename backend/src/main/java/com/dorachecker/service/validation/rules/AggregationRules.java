...
public class AggregationRules implements RoiValidationRule {
    ...
    @Override public String getRuleId() { return "AGG"; }
    @Override public String getSeverity() { return "WARNING"; }
    @Override public String getTemplateCode() { return "AGG"; }
    @Override public String getCategory() { return "COMPLETENESS"; }
    ...
    public List<RoiViolation> evaluate(RoiValidationContext ctx) {
        ...
        if (reg.getContracts().isEmpty()) {
            violations.add(new RoiViolation("COMPL_01", "WARNING", "COMPLETENESS", "B_02.01", "-", null,
        ...
        if (reg.getProviders().isEmpty()) {
            violations.add(new RoiViolation("COMPL_02", "WARNING", "COMPLETENESS", "B_05.01", "-", null,
        ...
        if (reg.getFunctions().isEmpty()) {
            violations.add(new RoiViolation("COMPL_03", "WARNING", "COMPLETENESS", "B_06.01", "-", null,
        ...
        if (reg.getAssessments().isEmpty() && !reg.getContracts().isEmpty()) {
            violations.add(new RoiViolation("COMPL_04", "WARNING", "COMPLETENESS", "B_07.01", "-", null,
        ...
        if (reg.getContractDetails().isEmpty() && !reg.getContracts().isEmpty()) {
            violations.add(new RoiViolation("COMPL_05", "WARNING", "COMPLETENESS", "B_02.02", "-", null,
        ...
        if (!reg.getContracts().isEmpty() && !reg.getAssessments().isEmpty()) {
            ...
            if (assessments < contracts / 2) {
                violations.add(new RoiViolation("AGG_01", "INFO", "COMPLETENESS", "B_07.01", "-", null,
                    "Hindamisi on oluliselt vähem kui lepinguid (" + assessments + " vs " + contracts + ")",
                    "Significantly fewer assessments than contracts (" + assessments + " vs " + contracts + ")"));
            }
        }

        // Provider count should correlate with signing count
        if (!reg.getProviders().isEmpty() && reg.getProviderSignings().isEmpty()) {
            violations.add(new RoiViolation("AGG_02", "WARNING", "COMPLETENESS", "B_03.02", "-", null,
        ...
        if (!reg.getContracts().isEmpty() && reg.getRecipients().isEmpty()) {
            violations.add(new RoiViolation("AGG_03", "WARNING", "COMPLETENESS", "B_03.01", "-", null,
