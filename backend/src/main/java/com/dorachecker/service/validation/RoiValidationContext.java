...
public class RoiValidationContext {
    ...
    public RoiValidationContext(RoiRegisterEntity register) {
        ...
        this.contractRefs = register.getContracts().stream()
            .map(RoiContractEntity::getContractRefNumber)
            .filter(r -> r != null)
            .collect(Collectors.toSet());

        this.providerIds = register.getProviders().stream()
            .map(RoiProviderEntity::getProviderIdentifier)
            .filter(r -> r != null)
            .collect(Collectors.toSet());

        this.functionIds = register.getFunctions().stream()
        ...
        register.getGroupEntities().forEach(ge -> {
            if (ge.getLei() != null) entityLeis.add(ge.getLei());
        });

        this.signedProviders = register.getProviderSignings().stream()
            .map(RoiProviderSigningEntity::getProviderIdentifier)
            .filter(r -> r != null)
            .collect(Collectors.toSet());

        this.signedContracts = register.getProviderSignings().stream()
            .map(RoiProviderSigningEntity::getContractRefNumber)
            .filter(r -> r != null)
            .collect(Collectors.toSet());

        this.detailedContracts = register.getContractDetails().stream()
            .map(RoiContractDetailEntity::getContractRefNumber)
            .filter(r -> r != null)
            .collect(Collectors.toSet());

        this.linkedFunctions = register.getContractDetails().stream()
            .map(RoiContractDetailEntity::getFunctionIdentifier)
            .filter(fid -> fid != null && !fid.isBlank())
            .collect(Collectors.toSet());

        this.assessedContracts = register.getAssessments().stream()
            .map(RoiAssessmentEntity::getContractRefNumber)
            .filter(r -> r != null)
            .collect(Collectors.toSet());

        this.criticalFunctionIds = register.getFunctions().stream()
            .filter(f -> "Critical".equals(f.getCriticalityAssessment()) || "Important".equals(f.getCriticalityAssessment()))
            .map(RoiFunctionEntity::getFunctionIdentifier)
            .collect(Collectors.toSet());

        this.contractsWithExitPlan = register.getContractDetails().stream()
            .filter(cd -> Boolean.TRUE.equals(cd.getExitPlanExists()))
            .map(RoiContractDetailEntity::getContractRefNumber)
            .collect(Collectors.toSet());

        this.contractsLinkedToCriticalFunctions = register.getContractDetails().stream()
    ...
    public Set<String> getContractRefs() { return contractRefs; }
    public Set<String> getProviderIds() { return providerIds; }
    public Set<String> getFunctionIds() { return functionIds; }
    public Set<String> getEntityLeis() { return entityLeis; }
    public Set<String> getSignedProviders() { return signedProviders; }
    public Set<String> getSignedContracts() { return signedContracts; }
    public Set<String> getDetailedContracts() { return detailedContracts; }
    public Set<String> getLinkedFunctions() { return linkedFunctions; }
    public Set<String> getAssessedContracts() { return assessedContracts; }
    public Set<String> getCriticalFunctionIds() { return criticalFunctionIds; }
    public Set<String> getContractsWithExitPlan() { return contractsWithExitPlan; }
    public Set<String> getContractsLinkedToCriticalFunctions() { return contractsLinkedToCriticalFunctions; }
}
