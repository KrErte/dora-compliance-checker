...
public class B03LinkingRules implements RoiValidationRule {
    ...
    @Override public String getRuleId() { return "B03"; }
    @Override public String getSeverity() { return "ERROR"; }
    @Override public String getTemplateCode() { return "B_03"; }
    ...
    public List<RoiViolation> evaluate(RoiValidationContext ctx) {
        ...
        for (RoiRecipientEntity r : reg.getRecipients()) {
            if (!ctx.getEntityLeis().contains(r.getEntityLei())) {
                violations.add(new RoiViolation("VR_71", "ERROR", "FK_INTEGRITY", "B_03.01", "C0020", r.getEntityLei(),
            ...
            if (!ctx.getContractRefs().contains(r.getContractRefNumber())) {
                violations.add(new RoiViolation("VR_71_R", "ERROR", "FK_INTEGRITY", "B_03.01", "C0010", r.getContractRefNumber(),
        ...
        for (RoiProviderSigningEntity ps : reg.getProviderSignings()) {
            if (!ctx.getProviderIds().contains(ps.getProviderIdentifier())) {
                violations.add(new RoiViolation("VR_71", "ERROR", "FK_INTEGRITY", "B_03.02", "C0020", ps.getProviderIdentifier(),
            ...
            if (!ctx.getContractRefs().contains(ps.getContractRefNumber())) {
                violations.add(new RoiViolation("VR_71", "ERROR", "FK_INTEGRITY", "B_03.02", "C0010", ps.getContractRefNumber(),
            ...
            if (!seenSigningPairs.add(pair)) {
                violations.add(new RoiViolation("DUP_SIGN", "WARNING", "DUPLICATE", "B_03.02", "C0010", pair,
        ...
        for (RoiInternalProviderEntity ip : reg.getInternalProviders()) {
            if (ip.getEntityLei() != null && !LEI_PATTERN.matcher(ip.getEntityLei()).matches()) {
                violations.add(new RoiViolation("DOR_0035", "WARNING", "FORMAT", "B_03.03", "C0020", ip.getEntityLei(),
        ...
        for (RoiContractEntity c : reg.getContracts()) {
            boolean hasRecipient = reg.getRecipients().stream()
                .anyMatch(r -> c.getContractRefNumber().equals(r.getContractRefNumber()));
            if (!hasRecipient) {
                violations.add(new RoiViolation("VR_71_REC", "WARNING", "BUSINESS", "B_03.01", "C0010", c.getContractRefNumber(),
