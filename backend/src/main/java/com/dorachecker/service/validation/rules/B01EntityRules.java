...
public class B01EntityRules implements RoiValidationRule {
  ...
  private static final Set<String> EBA_ENTITY_TYPES =
      Set.of(
          "eba_CS:x1",
          "eba_CS:x2",
          "eba_CS:x3",
    ...
    if (reg.getReportingDate() == null) {
      ...
      violations.add(
          new RoiViolation(
              ...
              "B_01.01",
              "C0070",
              reg.getReportingDate().toString(),
              "Aruandekuupäev on tulevikus",
              "Reporting date is in the future"));
    }

    // CON scope requires group entities
    if (reg.getConsolidationScope() != null
        && "CON".equals(reg.getConsolidationScope().name())
      ...
      if (b.getBranchIdentifier() != null && !seenBranches.add(b.getBranchIdentifier())) {
        violations.add(
            new RoiViolation(
                ...
                "B_01.03",
                "C0010",
                b.getBranchIdentifier(),
                "Duplikaat filiaali ID: " + b.getBranchIdentifier(),
                "Duplicate branch identifier: " + b.getBranchIdentifier()));
      }
      if (b.getEntityLei() != null
          && !b.getEntityLei().isBlank()
          && !ctx.getEntityLeis().contains(b.getEntityLei())) {
        violations.add(
            new RoiViolation(
                "CROSS_05",
                "ERROR",
