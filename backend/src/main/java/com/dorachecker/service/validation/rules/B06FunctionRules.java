...
public class B06FunctionRules implements RoiValidationRule {
    ...
    @Override public String getRuleId() { return "B06"; }
    @Override public String getSeverity() { return "WARNING"; }
    @Override public String getTemplateCode() { return "B_06"; }
    ...
    public List<RoiViolation> evaluate(RoiValidationContext ctx) {
        ...
        for (RoiFunctionEntity f : ctx.getRegister().getFunctions()) {
            if (f.getFunctionIdentifier() != null && !seenFunctions.add(f.getFunctionIdentifier())) {
                violations.add(new RoiViolation("DUP_03", "ERROR", "DUPLICATE", "B_06.01", "C0010", f.getFunctionIdentifier(),
        ...
        for (RoiFunctionEntity f : ctx.getRegister().getFunctions()) {
            ...
            if (f.getEntityLei() != null && !f.getEntityLei().isBlank()
                && !ctx.getEntityLeis().contains(f.getEntityLei())) {
                violations.add(new RoiViolation("VR_FN_LEI", "WARNING", "FK_INTEGRITY", "B_06.01", "C0040", f.getEntityLei(),
            ...
            if (f.getCriticalityAssessment() != null && !f.getCriticalityAssessment().isBlank()
                ...
                violations.add(new RoiViolation("VR_FN_CRIT", "WARNING", "FORMAT", "B_06.01", "C0050", name,
                    "Kriitilisuse hinnang '" + f.getCriticalityAssessment() + "' ei ole lubatud väärtus",
                    "Criticality assessment '" + f.getCriticalityAssessment() + "' is not a valid value"));
            }
            ...
            if ("Critical".equals(f.getCriticalityAssessment())) {
                if (f.getLicensedActivity() == null || f.getLicensedActivity().isBlank()) {
                    violations.add(new RoiViolation("VR_FN_LIC", "WARNING", "BUSINESS", "B_06.01", "C0030", name,
                ...
                if (f.getReasonsForCriticality() == null || f.getReasonsForCriticality().isBlank()) {
                    violations.add(new RoiViolation("CROSS_10", "WARNING", "BUSINESS", "B_06.01", "C0060", name,
            ...
            if (f.getFunctionName() == null || f.getFunctionName().isBlank()) {
                violations.add(new RoiViolation("VR_FN_NAME", "WARNING", "REQUIRED", "B_06.01", "C0020", f.getFunctionIdentifier(),
