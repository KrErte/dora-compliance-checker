...
public class B02ContractRules implements RoiValidationRule {
  ...
  private static final Set<String> VALID_CURRENCIES =
      Set.of(
          ...
          "ISK", "JPY", "CNY", "SGD", "AUD", "CAD", "INR", "BRL");

  private static final Set<String> VALID_CONTRACT_TYPES =
      Set.of("overarching", "individual", "intra-group");
  ...
  public List<RoiViolation> evaluate(RoiValidationContext ctx) {
    ...
    for (RoiContractEntity c : reg.getContracts()) {
      ...
      if (c.getCurrency() != null
          && !c.getCurrency().isBlank()
          && !VALID_CURRENCIES.contains(c.getCurrency())) {
        violations.add(
            new RoiViolation(
                "VR_23",
                "ERROR",
      ...
      if (c.getContractType() != null
          && !c.getContractType().isBlank()
          && !VALID_CONTRACT_TYPES.contains(c.getContractType())) {
        violations.add(
            new RoiViolation(
                "VR_CTR_TYPE",
                "WARNING",
      ...
      if ("individual".equals(c.getContractType())
          && c.getOverarchingRefNumber() != null
        ...
        violations.add(
            new RoiViolation(
                ...
                "Overarching contract ref '"
                    + c.getOverarchingRefNumber()
                    + "' not found in B_02.01"));
      }
    ...
    for (RoiContractDetailEntity detail : reg.getContractDetails()) {
      ...
      if (detail.getFunctionIdentifier() != null
          && !detail.getFunctionIdentifier().isBlank()
        ...
        violations.add(
            new RoiViolation(
                ...
                "B_02.02",
                "C0030",
                detail.getFunctionIdentifier(),
                "Funktsiooni viide '"
                    + detail.getFunctionIdentifier()
                    + "' ei leidu B_06.01 tabelis",
                "Function ref '" + detail.getFunctionIdentifier() + "' not found in B_06.01"));
      }

      // DOR_0041: ICT service type from closed list
      if (detail.getIctServiceType() != null
          && !detail.getIctServiceType().isBlank()
      ...
      if (detail.getDataSensitivity() == null || detail.getDataSensitivity().isBlank()) {
        violations.add(
            new RoiViolation(
                ...
                "C0060",
                detail.getContractRefNumber(),
                "Andmete tundlikkuse klassifikatsioon puudub lepingul: "
                    + detail.getContractRefNumber(),
                "Data sensitivity classification missing for contract: "
                    + detail.getContractRefNumber()));
      }
      ...
      if (detail.getDataLocationProcessing() == null
          || detail.getDataLocationProcessing().isBlank()) {
        violations.add(
            new RoiViolation(
                "DQ_03",
                "WARNING",
      ...
      if (!ctx.getContractRefs().contains(igc.getContractRefNumber())) {
        violations.add(
            new RoiViolation(
                ...
                "B_02.03",
                "C0010",
                igc.getContractRefNumber(),
                "Grupisisese lepingu viide '"
                    + igc.getContractRefNumber()
                    + "' ei leidu B_02.01 tabelis",
                "Intra-group contract ref '"
                    + igc.getContractRefNumber()
                    + "' not found in B_02.01"));
      }
      if (igc.getProviderEntityLei() != null
          && !igc.getProviderEntityLei().isBlank()
        ...
        violations.add(
            new RoiViolation(
                ...
                "B_02.03",
                "C0020",
                igc.getProviderEntityLei(),
                "Grupisisese lepingu pakkuja LEI '"
                    + igc.getProviderEntityLei()
                    + "' ei leidu B_01 tabelis",
                "Intra-group contract provider LEI '"
                    + igc.getProviderEntityLei()
                    + "' not found in B_01"));
      }
      if (igc.getReceiverEntityLei() != null
          && !igc.getReceiverEntityLei().isBlank()
        ...
        violations.add(
            new RoiViolation(
                ...
                "B_02.03",
                "C0030",
                igc.getReceiverEntityLei(),
                "Grupisisese lepingu saaja LEI '"
                    + igc.getReceiverEntityLei()
                    + "' ei leidu B_01 tabelis",
                "Intra-group contract receiver LEI '"
                    + igc.getReceiverEntityLei()
                    + "' not found in B_01"));
      }
      // Provider must not equal receiver
      if (igc.getProviderEntityLei() != null
          && igc.getReceiverEntityLei() != null
