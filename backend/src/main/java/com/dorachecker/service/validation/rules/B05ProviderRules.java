...
public class B05ProviderRules implements RoiValidationRule {
  ...
  private static final Set<String> VALID_COUNTRIES =
      Set.of(
          ...
          "LV", "LT", "LU", "MT", "NL", "PL", "PT", "RO", "SK", "SI", "ES", "SE", "US", "GB", "CH",
          "NO", "IS", "LI", "CA", "AU", "JP", "SG", "HK", "IN", "CN", "BR");
  ...
  private static final Set<String> EBA_PROVIDER_TYPES =
      Set.of("eba_CS:x1", "eba_CS:x2", "eba_CS:x3", "eba_CS:x4", "eba_CS:x5", "eba_CS:x6");
  ...
  public List<RoiViolation> evaluate(RoiValidationContext ctx) {
    ...
    for (RoiProviderEntity p : reg.getProviders()) {
      ...
      if ("LEI".equals(p.getIdentificationCodeType())) {
        if (p.getProviderIdentifier() != null
            && !p.getProviderIdentifier().isBlank()
      ...
      if (p.getCountryOfHq() != null
          && !p.getCountryOfHq().isBlank()
          && !VALID_COUNTRIES.contains(p.getCountryOfHq())) {
        violations.add(
            new RoiViolation(
                "DOR_0039",
                "WARNING",
      ...
      if (p.getCurrencyOfContract() != null
          && !p.getCurrencyOfContract().isBlank()
      ...
      if (p.getTotalAnnualSpend() != null
          && p.getTotalAnnualSpend().compareTo(BigDecimal.ZERO) > 0) {
        BigDecimal contractCostSum =
            reg.getProviderSignings().stream()
                .filter(ps -> p.getProviderIdentifier().equals(ps.getProviderIdentifier()))
                .map(
                    ps ->
                        reg.getContracts().stream()
                            .filter(c -> c.getContractRefNumber().equals(ps.getContractRefNumber()))
                            .map(
                                c ->
                                    c.getAnnualCost() != null ? c.getAnnualCost() : BigDecimal.ZERO)
                            .findFirst()
        ...
        if (contractCostSum.compareTo(BigDecimal.ZERO) > 0) {
          BigDecimal ratio =
              p.getTotalAnnualSpend().divide(contractCostSum, 2, java.math.RoundingMode.HALF_UP);
          if (ratio.compareTo(new BigDecimal("0.5")) < 0
              || ratio.compareTo(new BigDecimal("2.0")) > 0) {
            violations.add(
                new RoiViolation(
                    "VR_SPEND",
                    "INFO",
                    ...
                    "Provider '"
                        + name
                        + "' annual spend differs significantly from sum of contract costs"));
          }
        }
      }
    }
