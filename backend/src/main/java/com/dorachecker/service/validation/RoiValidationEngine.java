...
public class RoiValidationEngine {
  ...
  public RoiValidationReport validate(RoiRegisterEntity register, String templateCodeFilter) {
    ...
    double passRate =
        totalRules > 0
            ? BigDecimal.valueOf((double) passed / totalRules * 100)
    ...
    Map<String, RoiValidationReport.TemplateSummary> templateSummaries =
        buildTemplateSummaries(allIssues, totalRules);
    Map<String, RoiValidationReport.TemplateCompleteness> completeness =
        calculateCompleteness(register);
  ...
  private Map<String, RoiValidationReport.TemplateSummary> buildTemplateSummaries(
      List<RoiViolation> issues, int totalRules) {
    Map<String, List<RoiViolation>> byTemplate =
        issues.stream().collect(Collectors.groupingBy(RoiViolation::templateCode));
    ...
    for (Map.Entry<String, List<RoiViolation>> entry : byTemplate.entrySet()) {
      ...
      result.put(
          code,
          new RoiValidationReport.TemplateSummary(
  ...
  private Map<String, RoiValidationReport.TemplateCompleteness> calculateCompleteness(
      RoiRegisterEntity register) {
    ...
    // B_01.01
    {
      ...
      if (register.getEntityLei() != null && !register.getEntityLei().isBlank()) filled++;
      else missing.add("LEI");
      if (register.getEntityName() != null && !register.getEntityName().isBlank()) filled++;
      else missing.add("Entity Name");
      if (register.getCountry() != null && !register.getCountry().isBlank()) filled++;
      else missing.add("Country");
      if (register.getEntityType() != null && !register.getEntityType().isBlank()) filled++;
      else missing.add("Entity Type");
      if (register.getCompetentAuthority() != null && !register.getCompetentAuthority().isBlank())
        filled++;
      else missing.add("Competent Authority");
      if (register.getConsolidationScope() != null) filled++;
      else missing.add("Consolidation Scope");
      if (register.getReportingDate() != null) filled++;
      else missing.add("Reporting Date");
      result.put(
          "B_01.01",
          new RoiValidationReport.TemplateCompleteness(
              "B_01.01",
              "Entity Information",
              total,
              filled,
              total > 0 ? (filled * 100 / total) : 0,
              missing));
    }
    ...
    // B_05.01
    {
      ...
      if (register.getProviders().isEmpty()) {
        ...
        result.put(
            "B_05.01",
            new RoiValidationReport.TemplateCompleteness(
                "B_05.01", "ICT Providers", 1, 0, 0, missing));
      } else {
    ...
    // B_06.01
    {
      ...
      if (register.getFunctions().isEmpty()) {
        ...
        result.put(
            "B_06.01",
            new RoiValidationReport.TemplateCompleteness("B_06.01", "Functions", 1, 0, 0, missing));
      } else {
        for (RoiFunctionEntity f : register.getFunctions()) {
          ...
          if (f.getCriticalityAssessment() != null && !f.getCriticalityAssessment().isBlank())
            filled++;
    ...
    // B_02.01
    {
      ...
      if (register.getContracts().isEmpty()) {
        ...
        result.put(
            "B_02.01",
            new RoiValidationReport.TemplateCompleteness("B_02.01", "Contracts", 1, 0, 0, missing));
      } else {
    ...
    // B_02.02
    {
      ...
      if (register.getContractDetails().isEmpty()) {
        ...
        result.put(
            "B_02.02",
            new RoiValidationReport.TemplateCompleteness(
                "B_02.02", "Contract Details", 1, 0, 0, missing));
      } else {
    ...
    // B_07.01
    {
      ...
      if (register.getAssessments().isEmpty()) {
        ...
        result.put(
            "B_07.01",
            new RoiValidationReport.TemplateCompleteness(
                "B_07.01", "Assessments", 1, 0, 0, missing));
      } else {
