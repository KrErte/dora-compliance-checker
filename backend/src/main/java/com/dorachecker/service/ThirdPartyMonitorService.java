...
public class ThirdPartyMonitorService {
    ...
    public ThirdPartyMonitorService(IctProviderRepository ictProviderRepository,
    ...
    public Map<String, Object> getDashboard(String userId) {
        ...
        long criticalProviders = providers.stream()
            .filter(p -> "CRITICAL".equals(p.getCriticality()) || Boolean.TRUE.equals(p.isCritical()))
            .count();
        long highRiskProviders = providers.stream()
            .filter(p -> p.getRiskScore() != null && p.getRiskScore() >= 70)
            .count();
        long withExitStrategy = providers.stream()
            .filter(p -> Boolean.TRUE.equals(p.getHasExitStrategy()))
            .count();

        // Build vendor cards
        List<Map<String, Object>> vendorCards = providers.stream().map(provider -> {
            ...
            card.put("criticality", provider.getCriticality() != null ? provider.getCriticality() :
                (Boolean.TRUE.equals(provider.isCritical()) ? "CRITICAL" : "NORMAL"));
            card.put("riskScore", provider.getRiskScore() != null ? provider.getRiskScore() : 0);
            card.put("countryCode", provider.getCountryCode());
            card.put("hasExitStrategy", provider.getHasExitStrategy());
            card.put("contractEndDate", provider.getContractEndDate() != null ? provider.getContractEndDate().toString() : null);

            // Try to find matching vendor risk data
            Optional<VendorRiskEntity> vendorRisk = allVendorRisks.stream()
                .filter(vr -> vr.getVendorName() != null &&
                    vr.getVendorName().equalsIgnoreCase(provider.getProviderName()))
                .findFirst();
            ...
            List<ContractAlertEntity> alerts = contractAlertRepository.findByUserIdOrderByCreatedAtDesc(userId);
        ...
        result.put("exitStrategyPercentage", totalProviders > 0 ? Math.round((double) withExitStrategy / totalProviders * 100) : 0);
    ...
    public Map<String, Object> getConcentrationRisk(String userId) {
        ...
        Map<String, Long> byServiceType = providers.stream()
            .filter(p -> p.getServiceType() != null)
            .collect(Collectors.groupingBy(IctProviderEntity::getServiceType, Collectors.counting()));

        // Group by country
        Map<String, Long> byCountry = providers.stream()
            .filter(p -> p.getCountryCode() != null)
            .collect(Collectors.groupingBy(IctProviderEntity::getCountryCode, Collectors.counting()));

        // Group by criticality
        Map<String, Long> byCriticality = providers.stream()
            .collect(Collectors.groupingBy(p ->
                p.getCriticality() != null ? p.getCriticality() :
        ...
        if (total > 0) {
            byServiceType.forEach((type, count) -> {
