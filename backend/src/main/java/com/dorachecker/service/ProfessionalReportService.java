...
public class ProfessionalReportService {
    ...
    private static final Map<String, List<QuestionCategory>> PILLAR_CATEGORIES = Map.of(
            "ICT Risk Management", List.of(QuestionCategory.ICT_RISK_MANAGEMENT),
            "Incident Management", List.of(QuestionCategory.INCIDENT, QuestionCategory.INCIDENT_MANAGEMENT),
            "Resilience Testing", List.of(QuestionCategory.TESTING),
            "Third-Party Risk", List.of(
    ...
    private static final List<String> PILLAR_ORDER = List.of(
            "ICT Risk Management", "Incident Management", "Resilience Testing",
            "Third-Party Risk", "Information Sharing"
    );

    // Estonian pillar names
    private static final Map<String, String> PILLAR_NAMES_ET = Map.of(
    ...
    public ProfessionalReportService(QuestionService questionService,
    ...
    public byte[] generate(AssessmentEntity assessment, String userId, String language) {
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            String companyName = assessment.getCompanyName() != null ? assessment.getCompanyName() : "N/A";
    ...
    private void addCoverPage(Document doc, PdfDocument pdf, AssessmentEntity assessment,
                               UserBrandingEntity branding, PdfFont fontBold, PdfFont font,
                               boolean isEt, String companyName) {
        ...
        doc.add(new Paragraph(isEt ? "HINDAMISARUANNE" : "ASSESSMENT REPORT")
        ...
        if (assessment.getContractName() != null) {
            doc.add(new Paragraph(assessment.getContractName())
                    .setFont(font).setFontSize(14).setFontColor(new DeviceRgb(100, 116, 139))
                    .setTextAlignment(TextAlignment.CENTER));
        }
        doc.add(new Paragraph(assessment.getAssessmentDate() != null
        ...
        String level = assessment.getComplianceLevel() != null ? assessment.getComplianceLevel() : "N/A";
        ...
        badge.addCell(new Cell().add(new Paragraph(badgeText).setFont(fontBold).setFontSize(12).setFontColor(badgeFg))
                .setBackgroundColor(badgeBg).setPadding(8).setPaddingLeft(24).setPaddingRight(24)
                .setBorder(null).setBorderRadius(null));
        doc.add(badge);
    ...
    private void addExecutiveSummary(Document doc, AssessmentEntity assessment,
                                      List<Map<String, Object>> answers, List<DoraQuestion> allQuestions,
                                      PdfFont fontBold, PdfFont font, boolean isEt, String language) {
        ...
        // Key metrics table
        Table metrics = new Table(UnitValue.createPercentArray(new float[]{1, 1, 1, 1}))
                .useAllAvailableWidth().setMarginBottom(16);

        addMetricCell(metrics, isEt ? "Skoor" : "Score", String.format("%.0f%%", score), fontBold, font, getLevelFg(assessment.getComplianceLevel()));
        addMetricCell(metrics, isEt ? "Tase" : "Level", getLevelLabel(assessment.getComplianceLevel(), isEt), fontBold, font, getLevelFg(assessment.getComplianceLevel()));
        addMetricCell(metrics, isEt ? "Vastav" : "Compliant", String.valueOf(compliant), fontBold, font, GREEN_FG);
        addMetricCell(metrics, isEt ? "Mittevastav" : "Non-Compliant", String.valueOf(nonCompliant), fontBold, font, RED_FG);
        doc.add(metrics);
        ...
        // Top 3 critical gaps
        List<Map<String, Object>> gaps = answers.stream()
                .filter(a -> "non_compliant".equalsIgnoreCase(getString(a, "status", getString(a, "answer", ""))))
                .limit(3)
                .toList();
        ...
        if (!gaps.isEmpty()) {
            ...
            for (int i = 0; i < gaps.size(); i++) {
                ...
                doc.add(new Paragraph((i + 1) + ". " + q)
                        .setFont(font).setFontSize(10).setFontColor(RED_FG).setMarginBottom(2));
            }
            doc.add(new Paragraph(" ").setFontSize(6));
        }

        // Top 3 strengths
        List<Map<String, Object>> strengths = answers.stream()
                .filter(a -> "compliant".equalsIgnoreCase(getString(a, "status", getString(a, "answer", ""))))
                .limit(3)
                .toList();
        ...
        if (!strengths.isEmpty()) {
            ...
            for (int i = 0; i < strengths.size(); i++) {
                ...
                String q = getQuestionText(a, language);
                doc.add(new Paragraph((i + 1) + ". " + q)
                        .setFont(font).setFontSize(10).setFontColor(GREEN_FG).setMarginBottom(2));
            }
        }
    }
    ...
    private void addPillarOverview(Document doc, List<Map<String, Object>> answers,
        ...
        Table pillarTable = new Table(UnitValue.createPercentArray(new float[]{2.5f, 1, 4, 1.5f}))
                .useAllAvailableWidth().setMarginBottom(16);
        ...
        for (String pillar : PILLAR_ORDER) {
            ...
            pillarTable.addCell(new Cell().add(new Paragraph(pillarName).setFont(fontBold).setFontSize(9).setFontColor(DARK)).setPadding(6));
            pillarTable.addCell(new Cell().add(new Paragraph(String.valueOf(pTotal)).setFont(font).setFontSize(9)).setPadding(6).setTextAlignment(TextAlignment.CENTER));
            ...
            Table barTable = new Table(UnitValue.createPercentArray(new float[]{pScore, 100 - pScore})).useAllAvailableWidth();
            ...
            pillarTable.addCell(new Cell().add(new Paragraph(String.format("%.0f%%", pScore)).setFont(fontBold).setFontSize(9).setFontColor(scoreFg))
    ...
    private void addDetailedFindings(Document doc, List<Map<String, Object>> answers,
                                      List<DoraQuestion> allQuestions,
                                      PdfFont fontBold, PdfFont font, boolean isEt, String language) {
        ...
        for (Map.Entry<String, List<Map<String, Object>>> entry : grouped.entrySet()) {
            ...
            doc.add(new Paragraph(categoryLabel + " (" + catCompliant + "/" + catAnswers.size() + ")")
                    .setFont(fontBold).setFontSize(11).setFontColor(DARK)
                    .setMarginTop(12).setMarginBottom(6));

            Table table = new Table(UnitValue.createPercentArray(new float[]{0.6f, 1.2f, 3.5f, 1.2f, 3.5f}))
            ...
            for (Map<String, Object> a : catAnswers) {
                ...
                table.addCell(new Cell().add(new Paragraph(String.valueOf(qId)).setFontSize(8)).setPadding(4));
    ...
    private void addActionPlan(Document doc, List<Map<String, Object>> answers,
                                List<DoraQuestion> allQuestions,
                                PdfFont fontBold, PdfFont font, boolean isEt, String language) {
        addSectionTitle(doc, isEt ? "4. Tegevuskava" : "4. Remediation Action Plan", fontBold);

        List<Map<String, Object>> nonCompliant = answers.stream()
                .filter(a -> "non_compliant".equalsIgnoreCase(getString(a, "status", getString(a, "answer", ""))))
                .toList();
        List<Map<String, Object>> partial = answers.stream()
                .filter(a -> "partial".equalsIgnoreCase(getString(a, "status", getString(a, "answer", ""))))
                .toList();
        ...
        if (nonCompliant.isEmpty() && partial.isEmpty()) {
            doc.add(new Paragraph(isEt
                    ...
                    .setFont(font).setFontSize(11).setFontColor(GREEN_FG)
                    .setBackgroundColor(GREEN_BG).setPadding(12).setMarginBottom(16));
            return;
        }

        // Phase 1: Critical (0-3 months) - non-compliant items
        if (!nonCompliant.isEmpty()) {
            addPhaseHeader(doc, isEt ? "Faas 1: Kriitilised puudused (0-3 kuud)" : "Phase 1: Critical Gaps (0-3 months)",
                    fontBold, RED_FG, RED_BG);

            Table phase1 = new Table(UnitValue.createPercentArray(new float[]{0.5f, 1, 3, 1.5f, 1}))
                    .useAllAvailableWidth().setFontSize(8).setMarginBottom(12);
            ...
            for (int i = 0; i < nonCompliant.size(); i++) {
                ...
                phase1.addCell(new Cell().add(new Paragraph(String.valueOf(i + 1)).setFontSize(8)).setPadding(4));
                phase1.addCell(new Cell().add(new Paragraph(isEt ? "Kriitiline" : "Critical").setFontSize(8).setFontColor(RED_FG))
                        .setBackgroundColor(RED_BG).setPadding(4));
                ...
                phase1.addCell(new Cell().add(new Paragraph(getEstimatedEffort(i)).setFontSize(8)).setPadding(4));
            }
            doc.add(phase1);
        }
        ...
        if (!partial.isEmpty()) {
            addPhaseHeader(doc, isEt ? "Faas 2: Olulised parandused (3-6 kuud)" : "Phase 2: Important Fixes (3-6 months)",
                    fontBold, YELLOW_FG, YELLOW_BG);

            Table phase2 = new Table(UnitValue.createPercentArray(new float[]{0.5f, 1, 3, 1.5f, 1}))
                    .useAllAvailableWidth().setFontSize(8).setMarginBottom(12);
            ...
            for (int i = 0; i < partial.size(); i++) {
                ...
                phase2.addCell(new Cell().add(new Paragraph(String.valueOf(i + 1)).setFontSize(8)).setPadding(4));
                phase2.addCell(new Cell().add(new Paragraph(isEt ? "Oluline" : "Important").setFontSize(8).setFontColor(YELLOW_FG))
                        .setBackgroundColor(YELLOW_BG).setPadding(4));
                ...
                phase2.addCell(new Cell().add(new Paragraph(getEstimatedEffort(i + nonCompliant.size())).setFontSize(8)).setPadding(4));
            }
            doc.add(phase2);
        }

        // Phase 3: Enhancement
        addPhaseHeader(doc, isEt ? "Faas 3: Täiustused (6-12 kuud)" : "Phase 3: Enhancements (6-12 months)",
                fontBold, GREEN_FG, GREEN_BG);
        doc.add(new Paragraph(isEt
                ...
                .setFont(font).setFontSize(9).setFontColor(DARK).setMarginBottom(8));
    }

    // ── Methodology & Disclaimer ────────────────────────────

    private void addMethodology(Document doc, PdfFont fontBold, PdfFont font, boolean isEt) {
        addSectionTitle(doc, isEt ? "5. Metoodika ja Õiguslik Teave" : "5. Methodology & Legal Disclaimer", fontBold);
        ...
        doc.add(new Paragraph((isEt ? "Aruanne genereeritud: " : "Report generated: ")
    ...
    private void addPhaseHeader(Document doc, String text, PdfFont fontBold, DeviceRgb fg, DeviceRgb bg) {
        Table header = new Table(1).useAllAvailableWidth().setMarginTop(12).setMarginBottom(6);
        header.addCell(new Cell().add(new Paragraph(text).setFont(fontBold).setFontSize(11).setFontColor(fg))
    ...
    private void addMetricCell(Table table, String label, String value, PdfFont fontBold, PdfFont font, DeviceRgb valueColor) {
        ...
        cell.add(new Paragraph(value).setFont(fontBold).setFontSize(20).setFontColor(valueColor).setMarginBottom(2));
    ...
    private Cell statusCell(String status) {
        ...
        switch (status.toLowerCase()) {
            case "compliant", "yes" -> { bg = GREEN_BG; fg = GREEN_FG; label = status; }
            case "partial" -> { bg = YELLOW_BG; fg = YELLOW_FG; label = status; }
            case "non_compliant", "no" -> { bg = RED_BG; fg = RED_FG; label = status; }
