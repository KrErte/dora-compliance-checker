...
public class ComplianceReportService {
    ...
    // DORA 5 Pillars
    private static final List<String> PILLAR_ORDER = List.of(
            "ICT_RISK_MANAGEMENT", "INCIDENT_MANAGEMENT", "TESTING", "THIRD_PARTY", "INFORMATION_SHARING"
    );
    private static final Map<String, String> PILLAR_EN = Map.of(
    ...
    private static final Map<String, String> PILLAR_ET = Map.of(
    ...
    public byte[] generate(String userId, String language) {
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            List<AutopilotInsightEntity> insights = autopilotRepo.findByUserIdAndStatusInOrderByCreatedAtDesc(
            ...
            float overallScore = latestAssessment != null ? (float) latestAssessment.getScorePercentage() : 0;
            String complianceLevel = latestAssessment != null && latestAssessment.getComplianceLevel() != null
                    ? latestAssessment.getComplianceLevel() : (overallScore >= 80 ? "GREEN" : overallScore >= 50 ? "YELLOW" : "RED");

            // Branding
            UserBrandingEntity branding = brandingRepo.findByUserId(userId).orElse(null);
            String companyName = latestAssessment != null && latestAssessment.getCompanyName() != null
                    ? latestAssessment.getCompanyName() : "N/A";
            ...
            // === PAGE 1: Cover ===
            addCoverPage(doc, pdf, branding, fontBold, font, isEt, companyName, overallScore, complianceLevel);

            // === PAGE 2: Executive Summary ===
            doc.add(new AreaBreak(AreaBreakType.NEXT_PAGE));
            addExecutiveSummary(doc, fontBold, font, isEt, overallScore, complianceLevel,
                    evidence, remediations, insights, articleData);
    ...
    private void addCoverPage(Document doc, PdfDocument pdf, UserBrandingEntity branding,
                               PdfFont fontBold, PdfFont font, boolean isEt,
                               String companyName, float score, String level) {
        ...
        doc.add(new Paragraph(isEt ? "DORA VASTAVUSE" : "DORA COMPLIANCE")
                .setFont(fontBold).setFontSize(32).setFontColor(DARK)
                .setTextAlignment(TextAlignment.CENTER).setMarginBottom(0));
        doc.add(new Paragraph(isEt ? "RAPORT" : "REPORT")
                .setFont(fontBold).setFontSize(32).setFontColor(BRAND_GREEN)
                .setTextAlignment(TextAlignment.CENTER).setMarginBottom(30));

        doc.add(new Paragraph(companyName)
                .setFont(fontBold).setFontSize(18).setFontColor(DARK)
                .setTextAlignment(TextAlignment.CENTER));
        doc.add(new Paragraph(LocalDate.now().format(DATE_FMT))
                .setFont(font).setFontSize(11).setFontColor(new DeviceRgb(148, 163, 184))
        ...
        badge.addCell(new Cell().add(new Paragraph(badgeText).setFont(fontBold).setFontSize(12).setFontColor(badgeFg))
                .setBackgroundColor(badgeBg).setPadding(8).setPaddingLeft(24).setPaddingRight(24).setBorder(null));
        doc.add(badge);

        doc.add(new Paragraph(" ").setFontSize(60));
        doc.add(new Paragraph("DoraAudit.eu")
                .setFont(fontBold).setFontSize(11).setFontColor(BRAND_GREEN)
                .setTextAlignment(TextAlignment.CENTER));
        doc.add(new Paragraph(isEt ? "Professionaalne DORA vastavuse platvorm" : "Professional DORA Compliance Platform")
                .setFont(font).setFontSize(9).setFontColor(new DeviceRgb(148, 163, 184))
                .setTextAlignment(TextAlignment.CENTER));
    }
    ...
    private void drawScoreCircle(PdfDocument pdf, float percent, PdfFont fontBold) {
        ...
        DeviceRgb arcColor = percent >= 80 ? BRAND_GREEN : (percent >= 50 ? new DeviceRgb(251, 191, 36) : new DeviceRgb(248, 113, 113));
    ...
    private void addExecutiveSummary(Document doc, PdfFont fontBold, PdfFont font, boolean isEt,
                                      float score, String level,
        ...
        long completedRem = remediations.stream().filter(r -> "COMPLETED".equals(r.getStatus())).count();
        int remPct = remediations.isEmpty() ? 100 : (int) Math.round((completedRem * 100.0) / remediations.size());
        long criticalAlerts = insights.stream()
                .filter(i -> "CRITICAL".equals(i.getSeverity()) || "HIGH".equals(i.getSeverity())).count();
        String riskLevel = score >= 80 ? (isEt ? "Madal" : "Low") :
                score >= 50 ? (isEt ? "Keskmine" : "Medium") : (isEt ? "Korg" : "High");

        Table metrics = new Table(UnitValue.createPercentArray(new float[]{1, 1, 1, 1, 1}))
                .useAllAvailableWidth().setMarginBottom(16);
        addMetricCell(metrics, isEt ? "Skoor" : "Score", String.format("%.0f%%", score), fontBold, font, getLevelFg(level));
        addMetricCell(metrics, isEt ? "Toendid" : "Evidence", String.valueOf(evidenceCount), fontBold, font, BRAND_GREEN);
        addMetricCell(metrics, isEt ? "Parandatud" : "Remediated", remPct + "%", fontBold, font, remPct >= 75 ? GREEN_FG : YELLOW_FG);
        addMetricCell(metrics, isEt ? "Hoiatused" : "Alerts", String.valueOf(criticalAlerts), fontBold, font, criticalAlerts > 0 ? RED_FG : GREEN_FG);
        addMetricCell(metrics, isEt ? "Risktase" : "Risk Level", riskLevel, fontBold, font,
        ...
        doc.add(new Paragraph(riskText).setFont(font).setFontSize(10).setFontColor(DARK).setMarginBottom(12));
        ...
        Table coverageBar = new Table(UnitValue.createPercentArray(new float[]{1, 1, 1}))
                .useAllAvailableWidth().setMarginBottom(8);
        addMetricCell(coverageBar, isEt ? "Vastav" : "Compliant", String.valueOf(compliantArticles), fontBold, font, GREEN_FG);
        addMetricCell(coverageBar, isEt ? "Osaline" : "Partial", String.valueOf(partialArticles), fontBold, font, YELLOW_FG);
        addMetricCell(coverageBar, isEt ? "Puudulik" : "Non-Compliant", String.valueOf(nonCompliantArticles), fontBold, font, RED_FG);
        doc.add(coverageBar);
    }
    ...
    private void addArticleCoverageMatrix(Document doc, PdfFont fontBold, PdfFont font, boolean isEt,
                                           Map<String, Object> articleData) {
        addSectionTitle(doc, isEt ? "2. Artiklite kaetuse maatriks" : "2. Article Coverage Matrix", fontBold);

        doc.add(new Paragraph(isEt
                ...
                .setFont(font).setFontSize(10).setFontColor(DARK).setMarginBottom(12));
        ...
        for (Map<String, Object> chapter : chapters) {
            ...
            doc.add(new Paragraph(chTitle + " (" + progress + "%)")
            ...
            Table table = new Table(UnitValue.createPercentArray(new float[]{1.2f, 3f, 1.2f, 1f}))
                    .useAllAvailableWidth().setFontSize(8).setMarginBottom(6);
            ...
            for (Map<String, Object> art : articles) {
                ...
                int evCount = art.get("evidenceCount") instanceof Number
                        ? ((Number) art.get("evidenceCount")).intValue() : 0;
                ...
                table.addCell(new Cell().add(new Paragraph(String.valueOf(evCount)).setFontSize(8))
    ...
    @SuppressWarnings("unchecked")
    private void addPillarOverview(Document doc, PdfFont fontBold, PdfFont font, boolean isEt,
                                    Map<String, Object> articleData, AssessmentEntity latestAssessment) {
        ...
        doc.add(new Paragraph(isEt
                ...
                .setFont(font).setFontSize(10).setFontColor(DARK).setMarginBottom(16));
        ...
        Table pillarTable = new Table(UnitValue.createPercentArray(new float[]{2.5f, 4f, 1.5f}))
                .useAllAvailableWidth().setMarginBottom(16);
        ...
        for (String pillar : PILLAR_ORDER) {
            String name = isEt ? PILLAR_ET.getOrDefault(pillar, pillar) : PILLAR_EN.getOrDefault(pillar, pillar);
            int pScore = pillarScoresMap.getOrDefault(pillar, 0);

            pillarTable.addCell(new Cell().add(new Paragraph(name).setFont(fontBold).setFontSize(9).setFontColor(DARK)).setPadding(6));

            // Progress bar
            Cell barCell = new Cell().setPadding(6);
            DeviceRgb barColor = pScore >= 80 ? BRAND_GREEN : (pScore >= 50 ? new DeviceRgb(251, 191, 36) : new DeviceRgb(248, 113, 113));
            float filled = Math.max(1, pScore);
            Table barTable = new Table(UnitValue.createPercentArray(new float[]{filled, 100 - filled})).useAllAvailableWidth();
            ...
            pillarTable.addCell(new Cell().add(new Paragraph(pScore + "%").setFont(fontBold).setFontSize(9).setFontColor(scoreFg))
                    .setBackgroundColor(scoreBg).setPadding(6).setTextAlignment(TextAlignment.CENTER));
        }
        doc.add(pillarTable);
        ...
        for (String pillar : PILLAR_ORDER) {
            ...
            doc.add(new Paragraph(name).setFont(fontBold).setFontSize(9).setFontColor(DARK).setMarginBottom(1).setMarginTop(6));
            doc.add(new Paragraph(desc).setFont(font).setFontSize(8).setFontColor(new DeviceRgb(100, 116, 139)).setMarginBottom(4));
        }
    }
    ...
    @SuppressWarnings("unchecked")
    private void addEvidenceSummary(Document doc, PdfFont fontBold, PdfFont font, boolean isEt,
                                     List<EvidenceEntity> evidence, Map<String, Object> articleData) {
        addSectionTitle(doc, isEt ? "4. Toendite kokkuvote" : "4. Evidence Summary", fontBold);

        if (evidence.isEmpty()) {
            doc.add(new Paragraph(isEt ? "Toendeid pole veel esitatud." : "No evidence has been uploaded yet.")
                    .setFont(font).setFontSize(10).setFontColor(DARK).setMarginBottom(12));
            return;
        }
        ...
        Map<String, Long> byPillar = evidence.stream()
                .collect(Collectors.groupingBy(e -> e.getPillar() != null ? e.getPillar() : "GENERAL", Collectors.counting()));

        Table pillarEvTable = new Table(UnitValue.createPercentArray(new float[]{3, 1}))
                .useAllAvailableWidth().setFontSize(9).setMarginBottom(12);
        addHeaderCell(pillarEvTable, isEt ? "Sammas" : "Pillar", fontBold);
        addHeaderCell(pillarEvTable, isEt ? "Toendid" : "Count", fontBold);

        byPillar.entrySet().stream().sorted(Map.Entry.<String, Long>comparingByValue().reversed())
                .forEach(e -> {
                    String pName = isEt ? PILLAR_ET.getOrDefault(e.getKey(), e.getKey()) : PILLAR_EN.getOrDefault(e.getKey(), e.getKey());
                    pillarEvTable.addCell(new Cell().add(new Paragraph(pName).setFontSize(9)).setPadding(4));
                    pillarEvTable.addCell(new Cell().add(new Paragraph(String.valueOf(e.getValue())).setFontSize(9))
                            .setPadding(4).setTextAlignment(TextAlignment.CENTER));
                });
        ...
        Map<String, Long> byStatus = evidence.stream()
                .collect(Collectors.groupingBy(e -> e.getStatus() != null ? e.getStatus() : "PENDING", Collectors.counting()));

        Table statusTable = new Table(UnitValue.createPercentArray(new float[]{3, 1}))
                .useAllAvailableWidth().setFontSize(9).setMarginBottom(12);
        ...
        byStatus.forEach((status, count) -> {
            ...
            statusTable.addCell(new Cell().add(new Paragraph(String.valueOf(count)).setFontSize(9))
                    .setPadding(4).setTextAlignment(TextAlignment.CENTER));
        });
        doc.add(statusTable);
        ...
        if (gaps.isEmpty()) {
            doc.add(new Paragraph(isEt ? "Koik artiklid on toendiga kaetud." : "All articles are covered with evidence.")
                    .setFont(font).setFontSize(9).setFontColor(GREEN_FG)
                    .setBackgroundColor(GREEN_BG).setPadding(8).setMarginBottom(8));
        } else {
            doc.add(new Paragraph((isEt ? "Artiklid ilma toenditeta: " : "Articles without evidence: ") + gaps.size())
                    .setFont(font).setFontSize(9).setFontColor(RED_FG).setMarginBottom(4));
            int showMax = Math.min(gaps.size(), 15);
            for (int i = 0; i < showMax; i++) {
                doc.add(new Paragraph("  " + (i + 1) + ". " + gaps.get(i))
                        .setFont(font).setFontSize(8).setFontColor(DARK).setMarginBottom(1));
            }
            if (gaps.size() > showMax) {
                doc.add(new Paragraph("  ... " + (isEt ? "ja " : "and ") + (gaps.size() - showMax) + (isEt ? " veel" : " more"))
                        .setFont(font).setFontSize(8).setFontColor(new DeviceRgb(100, 116, 139)));
            }
        }
    }
    ...
    private void addRemediationStatus(Document doc, PdfFont fontBold, PdfFont font, boolean isEt,
                                       List<RemediationItemEntity> remediations) {
        addSectionTitle(doc, isEt ? "5. Parandustegevuste staatus" : "5. Remediation Status", fontBold);

        if (remediations.isEmpty()) {
            doc.add(new Paragraph(isEt ? "Parandustegevusi pole veel lisatud." : "No remediation items have been added yet.")
                    .setFont(font).setFontSize(10).setFontColor(DARK).setMarginBottom(12));
            return;
        }
        ...
        long inProgress = remediations.stream().filter(r -> "IN_PROGRESS".equals(r.getStatus())).count();
        ...
        Table summary = new Table(UnitValue.createPercentArray(new float[]{1, 1, 1, 1, 1}))
                .useAllAvailableWidth().setMarginBottom(12);
        addMetricCell(summary, isEt ? "Avatud" : "Open", String.valueOf(open), fontBold, font, RED_FG);
        addMetricCell(summary, isEt ? "Toos" : "In Progress", String.valueOf(inProgress), fontBold, font, YELLOW_FG);
        addMetricCell(summary, isEt ? "Lahendatud" : "Completed", String.valueOf(completed), fontBold, font, GREEN_FG);
        addMetricCell(summary, isEt ? "Edasi lukkatud" : "Deferred", String.valueOf(deferred), fontBold, font, DARK);
        addMetricCell(summary, isEt ? "Valmidus" : "Completion", completionPct + "%", fontBold, font, completionPct >= 75 ? GREEN_FG : YELLOW_FG);
        doc.add(summary);

        // By priority table
        addSubHeading(doc, isEt ? "Prioriteedi jargi" : "By Priority", fontBold);
        Map<String, Long> byPriority = remediations.stream()
                .collect(Collectors.groupingBy(r -> r.getPriority() != null ? r.getPriority() : "MEDIUM",
                        Collectors.counting()));

        Table prioTable = new Table(UnitValue.createPercentArray(new float[]{2, 1}))
                .useAllAvailableWidth().setFontSize(9).setMarginBottom(12);
        ...
        for (String prio : List.of("CRITICAL", "HIGH", "MEDIUM", "LOW")) {
            ...
            if (count > 0) {
                DeviceRgb fg = "CRITICAL".equals(prio) ? RED_FG : "HIGH".equals(prio) ? new DeviceRgb(234, 88, 12) : "MEDIUM".equals(prio) ? YELLOW_FG : BRAND_GREEN;
                prioTable.addCell(new Cell().add(new Paragraph(prio).setFontSize(9).setFontColor(fg)).setPadding(4));
                prioTable.addCell(new Cell().add(new Paragraph(String.valueOf(count)).setFontSize(9))
                        .setPadding(4).setTextAlignment(TextAlignment.CENTER));
            }
        }
        doc.add(prioTable);
        ...
        // Overdue items
        List<RemediationItemEntity> overdue = remediations.stream()
                .filter(r -> r.getDueDate() != null && r.getDueDate().isBefore(LocalDate.now())
                        && !"COMPLETED".equals(r.getStatus()) && !"DEFERRED".equals(r.getStatus()))
                .toList();

        if (!overdue.isEmpty()) {
            addSubHeading(doc, isEt ? "Tahtaja uletatanud" : "Overdue Items", fontBold);
            Table overdueTable = new Table(UnitValue.createPercentArray(new float[]{3, 1.5f, 1, 1}))
            ...
            for (RemediationItemEntity r : overdue.subList(0, Math.min(10, overdue.size()))) {
                overdueTable.addCell(new Cell().add(new Paragraph(r.getTitle() != null ? r.getTitle() : "").setFontSize(8)).setPadding(4));
                overdueTable.addCell(new Cell().add(new Paragraph(r.getDueDate().format(DATE_FMT)).setFontSize(8).setFontColor(RED_FG)).setPadding(4));
                overdueTable.addCell(new Cell().add(new Paragraph(r.getPriority() != null ? r.getPriority() : "").setFontSize(8)).setPadding(4));
                overdueTable.addCell(new Cell().add(new Paragraph(r.getStatus() != null ? r.getStatus() : "").setFontSize(8)).setPadding(4));
            }
    ...
    private void addAutopilotInsights(Document doc, PdfFont fontBold, PdfFont font, boolean isEt,
                                       List<AutopilotInsightEntity> insights) {
        addSectionTitle(doc, isEt ? "6. Autopiloodi ulevaated" : "6. Autopilot Insights", fontBold);

        if (insights.isEmpty()) {
            doc.add(new Paragraph(isEt ? "Aktiivseid ulevaateid ei leitud." : "No active insights found.")
                    .setFont(font).setFontSize(10).setFontColor(DARK).setMarginBottom(12));
            return;
        }
        ...
        Map<String, Long> bySeverity = insights.stream()
                .collect(Collectors.groupingBy(i -> i.getSeverity() != null ? i.getSeverity() : "MEDIUM",
                        Collectors.counting()));

        Table sevTable = new Table(UnitValue.createPercentArray(new float[]{1, 1, 1, 1}))
                .useAllAvailableWidth().setMarginBottom(12);
        addMetricCell(sevTable, "Critical", String.valueOf(bySeverity.getOrDefault("CRITICAL", 0L)), fontBold, font, RED_FG);
        addMetricCell(sevTable, "High", String.valueOf(bySeverity.getOrDefault("HIGH", 0L)), fontBold, font, new DeviceRgb(234, 88, 12));
        addMetricCell(sevTable, "Medium", String.valueOf(bySeverity.getOrDefault("MEDIUM", 0L)), fontBold, font, YELLOW_FG);
        addMetricCell(sevTable, "Low", String.valueOf(bySeverity.getOrDefault("LOW", 0L)), fontBold, font, BRAND_GREEN);
        doc.add(sevTable);

        // Top critical/high items
        List<AutopilotInsightEntity> topItems = insights.stream()
                .filter(i -> "CRITICAL".equals(i.getSeverity()) || "HIGH".equals(i.getSeverity()))
                .limit(5)
                .toList();

        if (!topItems.isEmpty()) {
            addSubHeading(doc, isEt ? "Kriitilised ulevaated" : "Critical & High Priority Insights", fontBold);
            Table insightTable = new Table(UnitValue.createPercentArray(new float[]{1, 1, 3, 3}))
                    .useAllAvailableWidth().setFontSize(8).setMarginBottom(8);
            ...
            for (int i = 0; i < topItems.size(); i++) {
                ...
                insightTable.addCell(new Cell().add(new Paragraph(String.valueOf(i + 1)).setFontSize(8)).setPadding(4));
                DeviceRgb sevFg = "CRITICAL".equals(ins.getSeverity()) ? RED_FG : new DeviceRgb(234, 88, 12);
                DeviceRgb sevBg = "CRITICAL".equals(ins.getSeverity()) ? RED_BG : new DeviceRgb(255, 237, 213);
                insightTable.addCell(new Cell().add(new Paragraph(ins.getSeverity()).setFontSize(8).setFontColor(sevFg))
                        .setBackgroundColor(sevBg).setPadding(4).setTextAlignment(TextAlignment.CENTER));
                insightTable.addCell(new Cell().add(new Paragraph(ins.getTitle() != null ? ins.getTitle() : "").setFontSize(8)).setPadding(4));
                insightTable.addCell(new Cell().add(new Paragraph(ins.getRecommendedAction() != null ? ins.getRecommendedAction() : "").setFontSize(8)).setPadding(4));
            }
            doc.add(insightTable);
        }

        // Remaining insights (medium/low)
        List<AutopilotInsightEntity> remaining = insights.stream()
        ...
        if (!remaining.isEmpty()) {
            ...
            for (int i = 0; i < remaining.size(); i++) {
                ...
                doc.add(new Paragraph((i + 1) + ". " + (ins.getTitle() != null ? ins.getTitle() : ""))
                        .setFont(font).setFontSize(9).setFontColor(DARK).setMarginBottom(1));
                if (ins.getRecommendedAction() != null) {
                    doc.add(new Paragraph("   " + ins.getRecommendedAction())
                            .setFont(font).setFontSize(8).setFontColor(new DeviceRgb(100, 116, 139)).setMarginBottom(3));
                }
            }
        }
    }
    ...
    @SuppressWarnings("unchecked")
    private void addRecommendations(Document doc, PdfFont fontBold, PdfFont font, boolean isEt,
                                     Map<String, Object> articleData,
                                     List<EvidenceEntity> evidence,
                                     List<RemediationItemEntity> remediations,
                                     List<AutopilotInsightEntity> insights) {
        ...
        if (!noEvidenceArticles.isEmpty()) {
            recommendations.add(new String[]{
                    ...
                    isEt ? "Laadige ules toendid mittevastava artiklite jaoks: " + String.join(", ", noEvidenceArticles.subList(0, Math.min(5, noEvidenceArticles.size())))
                            : "Upload evidence for non-compliant articles: " + String.join(", ", noEvidenceArticles.subList(0, Math.min(5, noEvidenceArticles.size())))
            });
        }

        // 2. Open critical remediations
        long criticalOpen = remediations.stream()
                .filter(r -> "CRITICAL".equals(r.getPriority()) && !"COMPLETED".equals(r.getStatus())).count();
        if (criticalOpen > 0) {
            recommendations.add(new String[]{
                    isEt ? "Kriitiline" : "Critical",
                    isEt ? "Lahendage " + criticalOpen + " kriitilist parandusulesan"
        ...
        long overdueCount = remediations.stream()
                .filter(r -> r.getDueDate() != null && r.getDueDate().isBefore(LocalDate.now())
                        && !"COMPLETED".equals(r.getStatus()) && !"DEFERRED".equals(r.getStatus())).count();
        if (overdueCount > 0) {
            recommendations.add(new String[]{
                    isEt ? "Korgel" : "High",
                    isEt ? "Kasitlege " + overdueCount + " tahtaja uletatanud parandustegevust"
        ...
        if (critInsights > 0) {
            recommendations.add(new String[]{
                    isEt ? "Korgel" : "High",
                    isEt ? "Vaadake ule " + critInsights + " kriitilist autopiloodi ulevaated"
                            : "Review " + critInsights + " critical autopilot insight(s)"
            });
        }

        // 5. General improvement
        long expiring = evidence.stream()
                .filter(e -> e.getExpiryDate() != null && e.getExpiryDate().isBefore(LocalDate.now().plusDays(30))).count();
        if (expiring > 0) {
            recommendations.add(new String[]{
                    isEt ? "Keskmine" : "Medium",
                    isEt ? "Uuendage " + expiring + " aeguvat toendit lahima 30 paeva jooksul"
                            : "Renew " + expiring + " evidence item(s) expiring within 30 days"
            });
        }

        if (recommendations.isEmpty()) {
            doc.add(new Paragraph(isEt ? "Kriitilisi puudusi ei tuvastatud. Jatkake regulaarset ulevaatust."
                    : "No critical gaps identified. Continue regular review.")
                    .setFont(font).setFontSize(10).setFontColor(GREEN_FG)
                    .setBackgroundColor(GREEN_BG).setPadding(12).setMarginBottom(8));
            return;
        }

        Table recTable = new Table(UnitValue.createPercentArray(new float[]{0.5f, 1.2f, 5f}))
                .useAllAvailableWidth().setFontSize(9).setMarginBottom(8);
        ...
        for (int i = 0; i < Math.min(5, recommendations.size()); i++) {
            ...
            DeviceRgb prioFg = rec[0].contains("Crit") || rec[0].contains("Kriit") ? RED_FG :
                    rec[0].contains("High") || rec[0].contains("Korg") ? new DeviceRgb(234, 88, 12) : YELLOW_FG;
            DeviceRgb prioBg = rec[0].contains("Crit") || rec[0].contains("Kriit") ? RED_BG :
                    rec[0].contains("High") || rec[0].contains("Korg") ? new DeviceRgb(255, 237, 213) : YELLOW_BG;

            recTable.addCell(new Cell().add(new Paragraph(String.valueOf(i + 1)).setFontSize(9)).setPadding(4));
            recTable.addCell(new Cell().add(new Paragraph(rec[0]).setFontSize(8).setFontColor(prioFg))
                    .setBackgroundColor(prioBg).setPadding(4).setTextAlignment(TextAlignment.CENTER));
    ...
    private void addMethodology(Document doc, PdfFont fontBold, PdfFont font, boolean isEt) {
        addSectionTitle(doc, isEt ? "8. Metoodika ja Oiguslik Teave" : "8. Methodology & Legal Disclaimer", fontBold);
        ...
        doc.add(new Paragraph(isEt
                ...
                .setFont(font).setFontSize(9).setFontColor(DARK).setMarginBottom(10));
        ...
        doc.add(new Paragraph(isEt
                ...
                .setFont(font).setFontSize(9).setFontColor(DARK).setMarginBottom(16));

        // Disclaimer box
        Table disclaimer = new Table(1).useAllAvailableWidth().setMarginTop(16);
        String disclaimerText = isEt
        ...
        disclaimer.addCell(new Cell()
                .add(new Paragraph(disclaimerText).setFont(font).setFontSize(8).setFontColor(new DeviceRgb(100, 116, 139)))
                .setBackgroundColor(LIGHT_BG).setPadding(12).setBorder(null));
        doc.add(disclaimer);

        // Generation timestamp
        doc.add(new Paragraph(" ").setFontSize(16));
        doc.add(new Paragraph((isEt ? "Raport genereeritud: " : "Report generated: ")
                + LocalDateTime.now().format(DateTimeFormatter.ofPattern("dd.MM.yyyy HH:mm:ss")))
                .setFont(font).setFontSize(8).setFontColor(new DeviceRgb(148, 163, 184))
                .setTextAlignment(TextAlignment.CENTER));
        doc.add(new Paragraph("DoraAudit.eu - Professional DORA Compliance Platform")
                .setFont(fontBold).setFontSize(8).setFontColor(BRAND_GREEN)
                .setTextAlignment(TextAlignment.CENTER));
    }
    ...
    private static class FooterHandler implements IEventHandler {
        ...
        public void handleEvent(Event event) {
            ...
            canvas.setStrokeColor(new DeviceRgb(226, 232, 240))
                    .setLineWidth(0.5f)
                    .moveTo(40, 40)
    ...
    private void addSectionTitle(Document doc, String title, PdfFont fontBold) {
        doc.add(new Paragraph(title)
                .setFont(fontBold).setFontSize(18).setFontColor(DARK).setMarginBottom(12));
    ...
    private void addSubHeading(Document doc, String text, PdfFont fontBold) {
        doc.add(new Paragraph(text)
                .setFont(fontBold).setFontSize(12).setFontColor(DARK).setMarginTop(10).setMarginBottom(4));
    }

    private void addMetricCell(Table table, String label, String value, PdfFont fontBold, PdfFont font, DeviceRgb valueColor) {
        Cell cell = new Cell().setPadding(8).setBackgroundColor(LIGHT_BG).setBorder(null).setTextAlignment(TextAlignment.CENTER);
        cell.add(new Paragraph(value).setFont(fontBold).setFontSize(18).setFontColor(valueColor).setMarginBottom(2));
        cell.add(new Paragraph(label).setFont(font).setFontSize(8).setFontColor(new DeviceRgb(100, 116, 139)));
        table.addCell(cell);
    }

    private void addHeaderCell(Table table, String text, PdfFont fontBold) {
        table.addHeaderCell(new Cell()
                .add(new Paragraph(text).setFont(fontBold).setFontColor(ColorConstants.WHITE).setFontSize(8))
                .setBackgroundColor(DARK).setPadding(5));
    }
    ...
    private Cell statusCell(String status, boolean isEt) {
        ...
        switch (status.toLowerCase()) {
            case "compliant" -> { bg = GREEN_BG; fg = GREEN_FG; label = isEt ? "Vastav" : "Compliant"; }
            case "partial" -> { bg = YELLOW_BG; fg = YELLOW_FG; label = isEt ? "Osaline" : "Partial"; }
            case "non_compliant" -> { bg = RED_BG; fg = RED_FG; label = isEt ? "Puudulik" : "Non-Compliant"; }
            default -> { bg = LIGHT_BG; fg = new DeviceRgb(55, 65, 81); label = status; }
        }
        return new Cell().add(new Paragraph(label).setFontSize(8).setFontColor(fg))
                .setBackgroundColor(bg).setPadding(4).setTextAlignment(TextAlignment.CENTER);
    }
    ...
    private String mapChapterToPillar(String chapterTitle) {
        ...
        if (chapterTitle.contains("Third-Party") || chapterTitle.contains("Managing ICT")) return "THIRD_PARTY";
