...
public class AlertMatchingService {
    ...
    public AlertMatchingService(ComplianceProfileRepository profileRepository,
    ...
    public void matchAndCreateAlerts(RegulatoryUpdateEntity update) {
        ...
        for (ComplianceProfileEntity profile : profiles) {
            ...
            if (alertRepository.existsByUserIdAndRegulatoryUpdateId(profile.getUserId(), update.getId())) {
                continue;
            }
        ...
        if (matched > 0) {
            log.info("Regulatiivsele uuendusele '{}' loodi {} hoiatust", truncate(update.getTitle(), 60), matched);
        }
    }

    private boolean isMatch(ComplianceProfileEntity profile,
        ...
        boolean regulationMatch = updateRegulations.isEmpty() ||
        ...
        boolean companyMatch = updateCompanyTypes.isEmpty() ||
                profileType.isEmpty() ||
        ...
        boolean countryMatch = updateCountries.isEmpty() ||
