...
/**
 * Factory methods for creating test data. Keeps test methods short by providing sensible defaults.
 */
public final class TestDataFactory {
  ...
  /**
   * Creates a verified LOCAL user with the given email and role. All required fields are populated
  ...
  public static UserEntity aFreeUser() {
    return aVerifiedUser(
        "free-" + UUID.randomUUID().toString().substring(0, 8) + "@test.com", UserEntity.Role.USER);
  }
  ...
  /**
   * Generates a valid JWT token for the given user. Requires a wired JwtService (call from
