...
class ArticleTrackerServiceTest {
  ...
  @Mock private RemediationItemRepository remediationRepo;

  @InjectMocks private ArticleTrackerService service;
  ...
  class GetArticleComplianceStatus {
    ...
    void highScoreWithEvidenceIsCompliant() {
      ...
      when(evidenceRepo.findByUserIdOrderByCreatedAtDesc(USER_ID)).thenReturn(List.of(evidence));
    ...
    void evidenceLinkedToSpecificArticle() {
      ...
      when(evidenceRepo.findByUserIdOrderByCreatedAtDesc(USER_ID)).thenReturn(List.of(ev1));
      when(remediationRepo.findByUserIdOrderByCreatedAtDesc(USER_ID)).thenReturn(List.of());
      ...
      Map<String, Object> art5 =
          chapterII.stream().filter(a -> "art5".equals(a.get("id"))).findFirst().orElseThrow();
      assertThat(art5.get("evidenceCount")).isEqualTo(1);
      assertThat(art5.get("status")).isEqualTo("partial"); // evidenceCount >= 1 but score < 75

      // art6
      Map<String, Object> art6 =
          chapterII.stream().filter(a -> "art6".equals(a.get("id"))).findFirst().orElseThrow();
      ...
      Map<String, Object> art7 =
          chapterII.stream().filter(a -> "art7".equals(a.get("id"))).findFirst().orElseThrow();
    ...
    void remediationItemsAffectStatus() {
      ...
      for (Map<String, Object> article : chapterIV) {
        assertThat(article.get("status"))
            .as(
                "Article %s in TESTING should be partial due to completed remediation",
                article.get("id"))
            .isEqualTo("partial");
      }
    ...
    @Test
    @DisplayName(
        "open remediation items without completed items do not change status from non_compliant")
    void openRemediationItemsDoNotMakePartial() {
      ...
      when(remediationRepo.findByUserIdOrderByCreatedAtDesc(USER_ID)).thenReturn(List.of(open));
      ...
      for (Map<String, Object> article : chapterIV) {
        assertThat(article.get("status"))
            .as(
                "Article %s in TESTING should remain non_compliant with only OPEN remediation",
                article.get("id"))
            .isEqualTo("non_compliant");
      }
    }
    ...
    void overallPercentageCalculation() {
      ...
      // All other articles (22) = partial (score>=40, no evidence via pillar but score triggers
      // partial)
      ...
      // Overall % = ((12 + 22*0.5) / 34) * 100 = ((12 + 11) / 34) * 100 = (23/34)*100 = 67.647...
      // => Math.round = 68
    ...
    void allCompliantGives100Percent() {
      ...
      when(evidenceRepo.findByUserIdOrderByCreatedAtDesc(USER_ID)).thenReturn(evidenceList);
      when(remediationRepo.findByUserIdOrderByCreatedAtDesc(USER_ID)).thenReturn(List.of());
    ...
    void manualStatusOverrideTakesPrecedence() {
      ...
      List<Map<String, Object>> chapterI = getArticles(getChapters(result).get(0));
      Map<String, Object> art1 =
          chapterI.stream().filter(a -> "art1".equals(a.get("id"))).findFirst().orElseThrow();
      assertThat(art1.get("status")).isEqualTo("compliant");

      // art2 should still be non_compliant (no override, no data)
      Map<String, Object> art2 =
          chapterI.stream().filter(a -> "art2".equals(a.get("id"))).findFirst().orElseThrow();
    ...
    void multipleEvidenceForSameArticle() {
      ...
      when(evidenceRepo.findByUserIdOrderByCreatedAtDesc(USER_ID)).thenReturn(List.of(ev1, ev2));
      when(remediationRepo.findByUserIdOrderByCreatedAtDesc(USER_ID)).thenReturn(List.of());
    ...
    void remediationNullPillarUsesGeneralFallback() {
      ...
      rem.setPillar(null);
      when(remediationRepo.findByUserIdOrderByCreatedAtDesc(USER_ID)).thenReturn(List.of(rem));
      ...
      for (Map<String, Object> article : chapterI) {
        assertThat(article.get("status"))
            .as(
                "Article %s should be partial (null pillar remediation -> GENERAL)",
                article.get("id"))
            .isEqualTo("partial");
      }
    }
  ...
  class UpdateArticleStatus {
    ...
    void returnsSuccessWithArticleId() {
      Map<String, Object> result =
          service.updateArticleStatus(USER_ID, "art28", Map.of("notes", "test"));
    ...
    void overwritesPreviousData() {
      ...
      service.updateArticleStatus(
          USER_ID, "art10", Map.of("notes", "updated note", "manualStatus", "compliant"));
  ...
  class GetArticleDetail {
    ...
    void manualStatusOverrideTakesPrecedenceInComplianceStatus() {
      ...
      service.updateArticleStatus(
          USER_ID, "art17", Map.of("manualStatus", "compliant", "notes", "Manually verified"));
      ...
      Map<String, Object> art17 =
          chapterIII.stream().filter(a -> "art17".equals(a.get("id"))).findFirst().orElseThrow();
      ...
      Map<String, Object> art18 =
          chapterIII.stream().filter(a -> "art18".equals(a.get("id"))).findFirst().orElseThrow();
