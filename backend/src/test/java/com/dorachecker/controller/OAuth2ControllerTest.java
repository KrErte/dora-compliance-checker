...
class OAuth2ControllerTest {
  ...
  private UserEntity buildUser(
      String id, String email, String fullName, UserEntity.AuthProvider provider) {
  ...
  /**
   * Stubs RestTemplate so that the Google token-exchange and userinfo calls return the supplied
   * values.
   */
  private void stubGoogleOAuthFlow(String email, String name, String sub) {
    // token exchange
    when(mockRestTemplate.postForEntity(contains("googleapis.com/token"), any(), eq(Map.class)))
  ...
  /**
   * Stubs RestTemplate for Microsoft token-exchange and Graph /me call. Accepts a full userInfo map
   * so callers can control which fields are present.
   */
  ...
  class GoogleCallback {
    ...
    void withError_redirectsToLoginError() throws Exception {
      ...
      controller.googleCallback(
          "unused-code", "access_denied", null, new MockHttpSession(), response);
    ...
    void success_redirectsWithTokens() throws Exception {
      ...
      UserEntity savedUser =
          buildUser("u1", "alice@example.com", "Alice", UserEntity.AuthProvider.GOOGLE);
      when(userRepository.findByEmail("alice@example.com")).thenReturn(Optional.empty());
      when(userRepository.save(any(UserEntity.class))).thenReturn(savedUser);
      when(jwtService.generateToken("u1", "alice@example.com", "USER")).thenReturn("jwt-token-123");
  ...
  class FindOrCreateOAuthUser {
    ...
    void existingLocalUser_rejectsOAuthToPreventHijack() throws Exception {
      ...
      UserEntity existingUser =
          buildUser("u1", "local@example.com", "Local User", UserEntity.AuthProvider.LOCAL);
    ...
    void existingGoogleUser_returnsWithoutProviderUpdate() throws Exception {
      ...
      UserEntity existingUser =
          buildUser("u2", "google@example.com", "Google User", UserEntity.AuthProvider.GOOGLE);
    ...
    void newUser_createsWithGoogleProvider() throws Exception {
      ...
      UserEntity savedUser =
          buildUser("u3", "new@example.com", "New User", UserEntity.AuthProvider.GOOGLE);
  ...
  class GenerateAndSaveRefreshToken {
    ...
    void generatesUniqueToken() throws Exception {
      ...
      UserEntity user =
          buildUser("u-rt", "rt@example.com", "RT User", UserEntity.AuthProvider.GOOGLE);
    ...
    void savesTokenToUser() throws Exception {
      ...
      UserEntity user =
          buildUser("u-save", "save@example.com", "Save User", UserEntity.AuthProvider.GOOGLE);
    ...
    void setsExpirationDate() throws Exception {
      ...
      UserEntity user =
          buildUser("u-exp", "exp@example.com", "Exp User", UserEntity.AuthProvider.GOOGLE);
  ...
  class MicrosoftCallback {
    ...
    void success_withPrimaryFields() throws Exception {
      ...
      UserEntity savedUser =
          buildUser("u-ms", "ms@example.com", "MS User", UserEntity.AuthProvider.MICROSOFT);
      when(userRepository.findByEmail("ms@example.com")).thenReturn(Optional.empty());
      when(userRepository.save(any(UserEntity.class))).thenReturn(savedUser);
      when(jwtService.generateToken("u-ms", "ms@example.com", "USER")).thenReturn("ms-jwt");
    ...
    void success_withFallbackFields() throws Exception {
      ...
      UserEntity savedUser =
          buildUser(
              "u-fb", "fallback@contoso.com", "Fallback Name", UserEntity.AuthProvider.MICROSOFT);
      when(userRepository.findByEmail("fallback@contoso.com")).thenReturn(Optional.empty());
      when(userRepository.save(any(UserEntity.class))).thenReturn(savedUser);
      when(jwtService.generateToken("u-fb", "fallback@contoso.com", "USER")).thenReturn("fb-jwt");
