...
@Component({
  ...
  template: `
    <div *ngIf="checklist.isVisible()" class="mb-6 animate-fade-in-up relative">
      ...
      <div class="bg-white backdrop-blur border border-blue-200 rounded-xl overflow-hidden
                  shadow-lg shadow-sm">
        <!-- Header -->
        <div class="px-6 py-4 bg-gradient-to-r from-blue-600/10 to-blue-500/10 border-b border-slate-200
                    flex items-center justify-between">
          <div class="flex items-center gap-3">
            <div class="w-9 h-9 rounded-lg bg-blue-100 border border-blue-200 flex items-center justify-center">
              <svg class="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/>
              </svg>
            </div>
            <div>
              <h3 class="text-sm font-semibold text-slate-700">{{ lang.t('checklist.title') }}</h3>
              <p class="text-xs text-slate-400">{{ lang.t('checklist.subtitle') }}</p>
            </div>
          </div>
          <div class="flex items-center gap-3">
            <button (click)="tourService.startTour()" type="button"
                    class="text-[10px] text-blue-500 hover:text-blue-400 font-medium px-2 py-1 rounded hover:bg-slate-100 transition-colors flex items-center gap-1">
              <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
              {{ lang.t('onboarding.restart_tour') }}
            </button>
            <span class="text-xs text-blue-600 font-medium">{{ checklist.completedCount() }}/{{ checklist.items().length }}</span>
            <button (click)="checklist.dismiss()" type="button"
                    class="w-7 h-7 rounded-lg hover:bg-slate-100 flex items-center justify-center transition-colors group"
                    [title]="lang.t('checklist.dismiss')">
              <svg class="w-4 h-4 text-slate-500 group-hover:text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="px-4 py-3 space-y-1">
          <a *ngFor="let item of checklist.items(); let i = index"
            ...
            <div class="w-7 h-7 rounded-full flex items-center justify-center shrink-0 transition-all duration-300 text-xs font-bold"
                 [class]="item.completed
                   ? 'bg-blue-100 border border-blue-500/40 text-blue-600'
                   : 'bg-slate-700/50 border border-slate-200 text-slate-500 group-hover:border-blue-200 group-hover:text-blue-600'">
            ...
            <div class="flex-1 min-w-0">
              <p class="text-sm font-medium transition-all duration-200"
                 [class]="item.completed ? 'text-slate-500 line-through' : 'text-slate-700 group-hover:text-blue-500'">
            ...
            <!-- Arrow -->
            <svg *ngIf="!item.completed" class="w-4 h-4 text-slate-600 group-hover:text-blue-600 transition-all duration-200 group-hover:translate-x-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
