...
@Component({
  ...
  template: `
    ...
    <nav *ngIf="hideNav" class="bg-[#0d1322] backdrop-blur-xl border-b border-slate-700/50 sticky top-0 z-50">
      <div class="max-w-5xl mx-auto px-3 sm:px-4 py-3 flex items-center justify-between">
        <a routerLink="/" class="flex items-center gap-3 group">
          <div class="w-9 h-9 rounded-lg bg-blue-600 flex items-center justify-center text-white font-bold text-xs
                      group-hover:shadow-lg group-hover:shadow-lg transition-all duration-300 group-hover:scale-105">
            DA
          </div>
        ...
        <a routerLink="/dashboard"
           class="text-sm text-slate-400 hover:text-blue-600 transition-colors duration-200 flex items-center gap-1.5 px-3 py-2 rounded-lg hover:bg-slate-800">
    ...
    <nav *ngIf="!hideNav" class="bg-[#0d1322] backdrop-blur-xl border-b border-slate-700/50 sticky top-0 z-50" [attr.aria-label]="lang.t('nav.main_nav')">
      <div class="max-w-5xl mx-auto px-3 sm:px-4 py-3 flex items-center justify-between">
        ...
        <div class="hidden lg:flex items-center gap-1">
          <a *ngIf="auth.isLoggedIn()" routerLink="/dashboard" routerLinkActive="nav-link-active"
             class="text-sm text-slate-400 hover:text-blue-600 transition-colors duration-200 px-3 py-2 rounded-lg hover:bg-slate-800">
          ...
          <div class="relative nav-dropdown-trigger">
            <button type="button" (click)="toggleMenu('more', $event)"
                    ...
                    class="text-sm text-slate-400 hover:text-blue-600 transition-colors duration-200 flex items-center gap-1.5 px-3 py-2 rounded-lg hover:bg-slate-800">
              {{ lang.t('nav.more') }}
              <svg class="w-3.5 h-3.5 transition-transform" [class.rotate-180]="moreMenu" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
                <path d="m6 9 6 6 6-6"/>
              </svg>
            </button>
            <div *ngIf="moreMenu" class="absolute left-0 top-full mt-1 w-64 bg-[#111827] border border-slate-700/50 rounded-xl shadow-xl py-2 z-50 max-h-[calc(100vh-5rem)] overflow-y-auto">
              <a routerLink="/workspace" (click)="closeAllMenus()"
                 class="flex items-center gap-2.5 px-4 py-2.5 text-sm text-slate-300 hover:text-violet-400 hover:bg-slate-800 transition-colors">
              ...
              <a routerLink="/chat" (click)="closeAllMenus()"
                 class="flex items-center gap-2.5 px-4 py-2.5 text-sm text-white hover:text-blue-500 hover:bg-blue-900/30 transition-colors mx-1 rounded-lg bg-gradient-to-r from-blue-700/10 to-blue-600/10 border border-blue-500/30 mb-1">
              ...
              <a routerLink="/audit-readiness" (click)="closeAllMenus()"
                 class="flex items-center gap-2.5 px-4 py-2 text-sm text-slate-300 hover:text-amber-400 hover:bg-slate-800 transition-colors">
              ...
              <a routerLink="/autopilot" (click)="closeAllMenus()"
                 class="flex items-center gap-2.5 px-4 py-2 text-sm text-slate-300 hover:text-violet-400 hover:bg-slate-800 transition-colors">
                <div class="w-4 h-4 rounded bg-gradient-to-br from-violet-400 to-purple-400 flex items-center justify-center text-white text-[7px] font-bold shrink-0">AI</div>
              ...
              <a routerLink="/remediation" (click)="closeAllMenus()"
                 class="flex items-center gap-2.5 px-4 py-2 text-sm text-slate-300 hover:text-blue-600 hover:bg-slate-800 transition-colors">
                <svg class="w-4 h-4 text-blue-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
                  <path d="m9 11 3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/>
                </svg>
                {{ lang.t('nav.remediation') }}
              </a>
              <a routerLink="/evidence-vault" (click)="closeAllMenus()"
                 class="flex items-center gap-2.5 px-4 py-2 text-sm text-slate-300 hover:text-indigo-400 hover:bg-slate-800 transition-colors">
              ...
              <a routerLink="/board-report" (click)="closeAllMenus()"
                 class="flex items-center gap-2.5 px-4 py-2 text-sm text-slate-300 hover:text-sky-400 hover:bg-slate-800 transition-colors">
              ...
              <a routerLink="/incident-reporting" (click)="closeAllMenus()"
                 class="flex items-center gap-2.5 px-4 py-2 text-sm text-slate-300 hover:text-red-400 hover:bg-slate-800 transition-colors">
              ...
              <a routerLink="/supply-chain" (click)="closeAllMenus()"
                 class="flex items-center gap-2.5 px-4 py-2 text-sm text-slate-300 hover:text-blue-500 hover:bg-slate-800 transition-colors">
                <svg class="w-4 h-4 text-blue-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
                  <path d="M16 16v1a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2h2m5.66 0H14a2 2 0 0 1 2 2v3.34"/><path d="M3 15h12"/><path d="M18 12h4"/><path d="M18 8h4"/>
                </svg>
              ...
              <a routerLink="/alerts" (click)="closeAllMenus()"
                 class="flex items-center gap-2.5 px-4 py-2 text-sm text-slate-300 hover:text-yellow-400 hover:bg-slate-800 transition-colors">
              ...
              <!-- AI Act -->
              <p class="px-4 pt-3 pb-1 text-[10px] font-bold uppercase tracking-wider text-slate-500">AI Act</p>
              <a routerLink="/ai-systems" (click)="closeAllMenus()"
                 class="flex items-center gap-2.5 px-4 py-2 text-sm text-slate-300 hover:text-blue-400 hover:bg-slate-800 transition-colors">
                ...
                {{ lang.t('nav.ai_systems') }}
              </a>
              <a routerLink="/ai-act-classifier" (click)="closeAllMenus()"
                 class="flex items-center gap-2.5 px-4 py-2 text-sm text-slate-300 hover:text-purple-400 hover:bg-slate-800 transition-colors">
              ...
              <a routerLink="/gpai-models" (click)="closeAllMenus()"
                 class="flex items-center gap-2.5 px-4 py-2 text-sm text-slate-300 hover:text-teal-400 hover:bg-slate-800 transition-colors">
          ...
          <button type="button" (click)="themeService.toggle()"
                  ...
                  class="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-amber-400 hover:bg-slate-800 transition-all"
          ...
          <button type="button" (click)="lang.toggle()"
                  ...
                  class="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium
                         border border-slate-700/50 text-slate-300 hover:border-blue-500/50 hover:text-blue-600
          ...
          @if (auth.isLoggedIn()) {
            <div class="relative nav-dropdown-trigger">
              <button type="button" (click)="toggleMenu('notif', $event)"
                      ...
                      class="relative w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-rose-400 hover:bg-slate-800 transition-all">
              ...
              <div *ngIf="notifMenu" class="absolute right-0 top-full mt-1 w-80 bg-[#111827] border border-slate-700/50 rounded-xl shadow-xl z-50 overflow-hidden">
                <div class="px-4 py-3 border-b border-slate-700/50 flex items-center justify-between">
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <a routerLink="/notifications" (click)="closeAllMenus()"
                       class="text-[10px] text-blue-600 hover:text-blue-500 font-medium">{{ lang.t('notifications.view_all') }}</a>
                  </div>
                </div>
                ...
                @if (notificationService.alerts().length > 0) {
                  ...
                  @for (alert of notificationService.alerts().slice(0, 3); track alert.alertKey) {
                    <a [routerLink]="alert.link" (click)="closeAllMenus()"
                       class="flex items-start gap-3 px-4 py-2.5 hover:bg-slate-800 transition-colors border-b border-slate-700/50 last:border-0">
                ...
                @if (notificationService.notifications().length > 0) {
                  ...
                  @for (notif of notificationService.notifications().slice(0, 5); track notif.id) {
                    <div (click)="onNotificationClick(notif)" class="flex items-start gap-3 px-4 py-2.5 hover:bg-slate-800 transition-colors cursor-pointer border-b border-slate-700/50 last:border-0">
                      ...
                      <div class="min-w-0 flex-1">
                        ...
                        <p class="text-[9px] text-slate-300 mt-0.5">{{ notificationService.timeAgo(notif.createdAt) }}</p>
                      </div>
                      @if (!notif.read) {
                ...
                @if (notificationService.alerts().length === 0 && notificationService.notifications().length === 0) {
                  <div class="px-4 py-6 text-center">
                    <svg class="w-8 h-8 mx-auto text-blue-600/50 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <!-- Footer -->
                <div class="px-4 py-2.5 border-t border-slate-700/50 text-center">
                  <a routerLink="/notifications" (click)="closeAllMenus()"
                     class="text-xs text-blue-600 hover:text-blue-500 font-medium">{{ lang.t('notifications.view_all') }}</a>
                </div>
              </div>
            </div>
          }
          ...
          @if (auth.isLoggedIn()) {
            <div class="relative nav-dropdown-trigger">
              ...
              <div *ngIf="userMenu" class="absolute right-0 top-full mt-1 w-48 bg-[#111827] border border-slate-700/50 rounded-xl shadow-xl py-2 z-50">
                @if (auth.user()?.fullName) {
                  <span class="block px-4 pt-1.5 text-sm font-semibold text-slate-400 truncate">{{ auth.user()?.fullName }}</span>
                }
                ...
                <a routerLink="/dashboard" (click)="userMenu = false"
                   class="flex items-center gap-2 px-4 py-2 text-sm text-slate-300 hover:text-blue-600 hover:bg-slate-800 transition-colors">
                  {{ lang.t('nav.my_account') }}
                </a>
                @if (subscriptionService.isPremium()) {
                  <a routerLink="/settings/branding" (click)="userMenu = false"
                     class="flex items-center gap-2 px-4 py-2 text-sm text-slate-300 hover:text-violet-400 hover:bg-slate-800 transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @if (auth.isAdmin()) {
                  <a routerLink="/admin/users" (click)="userMenu = false"
                     class="flex items-center gap-2 px-4 py-2 text-sm text-amber-400 hover:text-amber-300 hover:bg-slate-800 transition-colors">
                ...
                <button type="button" (click)="restartTour(); userMenu = false"
                        class="w-full text-left flex items-center gap-2 px-4 py-2 text-sm text-slate-300 hover:text-blue-500 hover:bg-slate-800 transition-colors">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button type="button" (click)="auth.logout(); userMenu = false"
                        class="w-full text-left flex items-center gap-2 px-4 py-2 text-sm text-red-400 hover:bg-slate-800 transition-colors">
                  {{ lang.t('auth.logout') }}
                </button>
              </div>
            </div>
            ...
            <a routerLink="/login"
               class="text-sm text-blue-600 hover:text-blue-500 transition-colors duration-200 px-2.5 py-2 rounded-lg hover:bg-slate-800">
              {{ lang.t('auth.login') }}
            </a>
        ...
        <div class="flex items-center gap-2 lg:hidden">
          @if (auth.isLoggedIn()) {
            <div class="w-7 h-7 rounded-full bg-blue-600 flex items-center justify-center text-white text-[10px] font-bold shrink-0">
              {{ getUserInitials() }}
            </div>
          }
          <button type="button" (click)="mobileMenu = !mobileMenu"
                  ...
                  class="p-2 rounded-lg text-slate-400 hover:text-blue-600 hover:bg-slate-800 transition-colors">
            <!-- Lucide: menu -->
      ...
      <!-- Mobile menu -->
      <div *ngIf="mobileMenu" class="lg:hidden border-t border-slate-700/50 bg-[#111827] backdrop-blur-xl max-h-[calc(100vh-4rem)] overflow-y-auto">
        <div class="px-4 py-3 flex flex-col gap-1">
          <!-- Primary nav links -->
          @if (auth.isLoggedIn()) {
            <a routerLink="/dashboard" (click)="mobileMenu = false"
               class="text-sm text-slate-300 hover:text-blue-600 px-3 py-2 rounded-lg hover:bg-slate-800 font-medium">{{ lang.t('nav.dashboard') }}</a>
          }
          <a routerLink="/assessment" (click)="mobileMenu = false"
             class="text-sm text-slate-400 hover:text-blue-500 px-3 py-2 rounded-lg hover:bg-slate-800">{{ lang.t('nav.assessment') }}</a>
          <a routerLink="/tools" (click)="mobileMenu = false"
             class="text-sm text-slate-400 hover:text-blue-600 px-3 py-2 rounded-lg hover:bg-slate-800">{{ lang.l('Tööriistad', 'Tools') }}</a>
          <a routerLink="/pricing" (click)="mobileMenu = false"
             class="text-sm text-slate-400 hover:text-blue-600 px-3 py-2 rounded-lg hover:bg-slate-800">{{ lang.t('nav.pricing') }}</a>
          <a routerLink="/blog" (click)="mobileMenu = false"
             class="text-sm text-slate-400 hover:text-blue-600 px-3 py-2 rounded-lg hover:bg-slate-800">{{ lang.t('nav.blog') }}</a>
          ...
          <div class="border-t border-slate-700/50 mt-2 pt-2">
            <button type="button" (click)="mobileMoreOpen = !mobileMoreOpen"
              ...
              <span class="text-[10px] text-slate-300 font-bold uppercase tracking-wider">{{ lang.t('nav.more') }}</span>
              <svg class="w-3.5 h-3.5 text-slate-300 transition-transform" [class.rotate-180]="mobileMoreOpen" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <path d="m6 9 6 6 6-6"/>
              </svg>
            </button>
            <div *ngIf="mobileMoreOpen" class="flex flex-col gap-0.5">
              <a routerLink="/chat" (click)="mobileMenu = false"
                 class="flex items-center gap-2 text-sm text-white px-3 py-2.5 rounded-lg bg-gradient-to-r from-blue-700/10 to-blue-600/10 border border-blue-500/30 mb-1">
                <div class="w-5 h-5 rounded bg-blue-600 flex items-center justify-center text-white text-[8px] font-bold">AI</div>
                DoraBot
              </a>
              <!-- Compliance Management -->
              <p class="px-3 pt-2 pb-0.5 text-[10px] font-bold uppercase tracking-wider text-slate-300">{{ lang.l('Vastavuse haldus', 'Compliance') }}</p>
              <a routerLink="/audit-readiness" (click)="mobileMenu = false"
                 class="text-sm text-slate-400 hover:text-amber-400 px-3 py-2 rounded-lg hover:bg-slate-800">{{ lang.t('nav.audit_readiness') }}</a>
              <a routerLink="/autopilot" (click)="mobileMenu = false"
                 class="text-sm text-slate-400 hover:text-violet-400 px-3 py-2 rounded-lg hover:bg-slate-800 flex items-center gap-2">
              ...
              <a routerLink="/remediation" (click)="mobileMenu = false"
                 class="text-sm text-slate-400 hover:text-blue-600 px-3 py-2 rounded-lg hover:bg-slate-800">{{ lang.t('nav.remediation') }}</a>
              <a routerLink="/evidence-vault" (click)="mobileMenu = false"
                 class="text-sm text-slate-400 hover:text-indigo-400 px-3 py-2 rounded-lg hover:bg-slate-800">{{ lang.t('nav.evidence_vault') }}</a>
              <a routerLink="/board-report" (click)="mobileMenu = false"
                 class="text-sm text-slate-400 hover:text-sky-400 px-3 py-2 rounded-lg hover:bg-slate-800">{{ lang.t('nav.board_report') }}</a>
              <!-- Risk & Incidents -->
              <p class="px-3 pt-2 pb-0.5 text-[10px] font-bold uppercase tracking-wider text-slate-300">{{ lang.l('Risk ja intsidendid', 'Risk & Incidents') }}</p>
              <a routerLink="/incident-reporting" (click)="mobileMenu = false"
                 class="text-sm text-slate-400 hover:text-red-400 px-3 py-2 rounded-lg hover:bg-slate-800">{{ lang.t('nav.incidents') }}</a>
              <a routerLink="/supply-chain" (click)="mobileMenu = false"
                 class="text-sm text-slate-400 hover:text-blue-500 px-3 py-2 rounded-lg hover:bg-slate-800 flex items-center justify-between">
                {{ lang.t('nav.supply_short') }}
              ...
              <a routerLink="/alerts" (click)="mobileMenu = false"
                 class="text-sm text-slate-400 hover:text-yellow-400 px-3 py-2 rounded-lg hover:bg-slate-800">{{ lang.t('nav.alerts') }}</a>
              <!-- AI Act -->
              <p class="px-3 pt-2 pb-0.5 text-[10px] font-bold uppercase tracking-wider text-slate-300">AI Act</p>
              <a routerLink="/ai-systems" (click)="mobileMenu = false"
                 class="text-sm text-slate-400 hover:text-blue-400 px-3 py-2 rounded-lg hover:bg-slate-800">{{ lang.t('nav.ai_systems') }}</a>
              <a routerLink="/ai-act-classifier" (click)="mobileMenu = false"
                 class="text-sm text-slate-400 hover:text-purple-400 px-3 py-2 rounded-lg hover:bg-slate-800">{{ lang.t('nav.ai_act_classifier') }}</a>
              <a routerLink="/gpai-models" (click)="mobileMenu = false"
                 class="text-sm text-slate-400 hover:text-teal-400 px-3 py-2 rounded-lg hover:bg-slate-800">{{ lang.t('nav.gpai_models') }}</a>
              <!-- Reference -->
              <p class="px-3 pt-2 pb-0.5 text-[10px] font-bold uppercase tracking-wider text-slate-300">{{ lang.l('Teave', 'Reference') }}</p>
              <a routerLink="/dora-explorer" (click)="mobileMenu = false"
                 class="text-sm text-slate-400 hover:text-blue-600 px-3 py-2 rounded-lg hover:bg-slate-800">{{ lang.t('nav.dora_explorer') }}</a>
              <a routerLink="/fine-calculator" (click)="mobileMenu = false"
                 class="text-sm text-slate-400 hover:text-red-400 px-3 py-2 rounded-lg hover:bg-slate-800">{{ lang.t('nav.fine_calculator') }}</a>
            </div>
          </div>
          @if (auth.isLoggedIn()) {
            <div class="border-t border-slate-700/50 mt-2 pt-2">
              ...
              @if (auth.isAdmin()) {
                <a routerLink="/admin/users" (click)="mobileMenu = false"
                   class="text-sm text-amber-400 hover:text-amber-300 px-3 py-2 rounded-lg hover:bg-slate-800">Admin Panel</a>
                <a routerLink="/admin/leads" (click)="mobileMenu = false"
                   class="text-sm text-amber-400 hover:text-amber-300 px-3 py-2 rounded-lg hover:bg-slate-800">Leads</a>
              }
              ...
              <button type="button" (click)="restartTour(); mobileMenu = false"
                      class="w-full text-left text-sm text-slate-400 hover:text-blue-500 px-3 py-2 rounded-lg hover:bg-slate-800 mt-1 flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
                </svg>
                {{ lang.t('onboarding.restart_tour') }}
              ...
              <button type="button" (click)="auth.logout(); mobileMenu = false"
                      class="w-full text-left text-sm text-red-400 px-3 py-2 rounded-lg hover:bg-slate-800 mt-1">{{ lang.t('auth.logout') }}</button>
            </div>
          } @else if (isBrowser) {
            <div class="border-t border-slate-700/50 mt-2 pt-2 flex flex-col gap-1">
              <a routerLink="/login" (click)="mobileMenu = false"
                 class="text-sm text-blue-600 px-3 py-2 rounded-lg hover:bg-slate-800">{{ lang.t('auth.login') }}</a>
              <a routerLink="/register" (click)="mobileMenu = false"
                 class="text-sm text-blue-700 bg-blue-600/20 px-3 py-2 rounded-lg hover:bg-blue-600/30 text-center">{{ lang.t('auth.register') }}</a>
            </div>
          }
          <div class="border-t border-slate-700/50 mt-2 pt-2 flex flex-col gap-1">
            <button type="button" (click)="themeService.toggle(); mobileMenu = false"
                    class="w-full text-left text-sm text-slate-400 px-3 py-2 rounded-lg hover:bg-slate-800 flex items-center gap-2">
              @if (themeService.isDark()) {
    ...
    <footer *ngIf="!hideFooter" class="border-t border-slate-700/50 mt-16 py-10 bg-[#0a0f1e]">
      <div class="max-w-5xl mx-auto px-4">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
          <!-- Brand -->
          <div class="md:col-span-1">
            <div class="flex items-center gap-3 mb-3">
              <div class="w-8 h-8 rounded-lg bg-blue-600 flex items-center justify-center text-white font-bold text-xs">DA</div>
          ...
          <div>
            ...
            <div class="flex flex-col gap-2">
              <a routerLink="/contract-analysis" class="text-xs text-slate-500 hover:text-blue-600 transition-colors">{{ lang.t('nav.contract') }}</a>
              <a routerLink="/assessment" class="text-xs text-slate-500 hover:text-blue-600 transition-colors">{{ lang.t('nav.assessment') }}</a>
              ...
              <a routerLink="/dora-explorer" class="text-xs text-slate-500 hover:text-blue-500 transition-colors">{{ lang.t('nav.explorer') }}</a>
          ...
          <div>
            ...
            <div class="flex flex-col gap-2">
              <a routerLink="/about" class="text-xs text-slate-500 hover:text-blue-600 transition-colors">{{ lang.t('nav.about') }}</a>
              <a routerLink="/methodology" class="text-xs text-slate-500 hover:text-blue-600 transition-colors">{{ lang.t('nav.methodology') }}</a>
              <a routerLink="/pricing" class="text-xs text-slate-500 hover:text-blue-600 transition-colors">{{ lang.t('nav.pricing') }}</a>
          ...
          <div>
            ...
            <div class="flex flex-col gap-2 text-xs text-slate-500">
              <a href="mailto:info@doraaudit.eu" class="hover:text-blue-600 transition-colors">info&#64;doraaudit.eu</a>
              <p>DoraAudit.eu</p>
        ...
        <!-- Bottom bar -->
        <div class="border-t border-slate-700/50 pt-6 flex flex-col md:flex-row items-center justify-between gap-4">
          <p class="text-xs text-slate-300">&copy; 2026 DoraAudit.eu. {{ lang.t('footer.rights') }}</p>
          <div class="flex flex-col md:flex-row items-center gap-2 md:gap-4 text-center">
            <p class="text-[10px] text-slate-400">{{ lang.t('footer.regulation') }}</p>
            <p class="text-[10px] text-slate-400">{{ lang.t('footer.disclaimer') }}</p>
          </div>
        </div>
      </div>
    </footer>
    ...
    <div class="fixed top-4 right-4 z-[100] flex flex-col gap-2 max-w-sm" *ngIf="toast.toasts().length > 0">
      <div *ngFor="let t of toast.toasts()" (click)="toast.dismiss(t.id)"
           ...
           [ngClass]="{
             ...
             'bg-slate-100/90 text-white border-slate-300': t.type === 'info'
           }">
        {{ t.message }}
      </div>
    </div>
...
export class AppComponent implements OnInit, OnDestroy {
  ...
  toggleMenu(menu: 'more' | 'notif', event: Event) {
    event.stopPropagation();
    const key = (menu + 'Menu') as 'moreMenu' | 'notifMenu';
