      ...
      <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex items-center gap-3">
          ...
          <button (click)="runCrawler()"
                  [disabled]="crawling()"
                  class="px-4 py-2 text-sm bg-blue-700 hover:bg-blue-600 disabled:bg-blue-900 disabled:cursor-not-allowed text-white rounded-lg transition-colors flex items-center gap-2">
            @if (crawling()) {
      ...
      @if (stats()) {
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div class="bg-white border border-slate-200 rounded-xl p-4">
            ...
            <p class="text-xl font-bold text-blue-600 mt-1">{{ stats()!.byStatus['NEW'] || 0 }}</p>
          </div>
          <div class="bg-white border border-slate-200 rounded-xl p-4">
                    ...
                    <td class="px-4 py-3">
                      @if (lead.website) {
                        <a [href]="lead.website" target="_blank" (click)="$event.stopPropagation()"
                           class="text-blue-600 hover:text-blue-500 text-xs truncate max-w-[150px] block">
