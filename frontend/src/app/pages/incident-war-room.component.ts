      ...
      @if (loading()) {
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div class="flex items-center gap-3 mb-1">
              ...
              <div>
                <h1 class="text-2xl font-bold text-slate-900">{{ lang.t('warroom.title') }}</h1>
          ...
          <div class="flex items-center gap-3">
            @if (incident()!.warRoomActive) {
              ...
              <button (click)="closeWarRoom()"
                      class="px-4 py-2 rounded-xl bg-slate-100/50 text-slate-600 text-sm hover:bg-slate-50 transition-colors">
        ...
        @if (incident()!.warRoomActive) {
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
            @if (incident()!.initialReportDueAt && !incident()!.initialReportSentAt) {
              <div class="bg-white border border-red-500/30 rounded-xl p-4">
                ...
                <div class="text-xl font-mono font-bold" [class]="isOverdue(incident()!.initialReportDueAt) ? 'text-red-400' : 'text-slate-900'">
            ...
            @if (incident()!.intermediateReportDueAt && !incident()!.intermediateReportSentAt) {
              <div class="bg-white border border-amber-500/30 rounded-xl p-4">
                ...
                <div class="text-xl font-mono font-bold" [class]="isOverdue(incident()!.intermediateReportDueAt) ? 'text-red-400' : 'text-slate-900'">
            ...
            @if (incident()!.finalReportDueAt && !incident()!.finalReportSentAt) {
              <div class="bg-white border border-blue-500/30 rounded-xl p-4">
                ...
                <div class="text-xl font-mono font-bold" [class]="isOverdue(incident()!.finalReportDueAt) ? 'text-red-400' : 'text-slate-900'">
          ...
          <div class="bg-white border border-slate-200 rounded-xl p-5">
            <h3 class="text-slate-900 font-semibold text-sm mb-4">{{ lang.t('warroom.phase_timeline') }}</h3>
                ...
                <div class="flex items-center flex-shrink-0">
                  <button (click)="updatePhase(phase)"
                    ...
                    <div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold"
                         [class]="getCurrentPhaseIndex() >= i ? 'bg-red-500 text-white' : 'bg-slate-200 text-slate-500'">
                      {{ i + 1 }}
                    </div>
                  ...
                  @if (i < phases.length - 1) {
                    <div class="w-6 h-px mx-1" [class]="getCurrentPhaseIndex() > i ? 'bg-red-500' : 'bg-slate-200'"></div>
                  }
                </div>
              }
            </div>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
              <!-- Communication Log -->
              <div class="bg-white border border-slate-200 rounded-xl p-5">
                <h3 class="text-slate-900 font-semibold text-sm mb-3">{{ lang.t('warroom.comms_log') }}</h3>
                ...
                <div class="flex gap-2">
                  <input [(ngModel)]="newCommMessage" (keyup.enter)="addCommunication()"
                         class="flex-1 bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-900 focus:outline-none focus:border-blue-500/50"
                         [placeholder]="lang.t('warroom.type_message')"/>
                  <select [(ngModel)]="commChannel" class="bg-white border border-slate-200 rounded-lg px-2 py-2 text-xs text-slate-900 focus:outline-none">
              ...
              <!-- Decision Log -->
              <div class="bg-white border border-slate-200 rounded-xl p-5">
                <h3 class="text-slate-900 font-semibold text-sm mb-3">{{ lang.t('warroom.decision_log') }}</h3>
                <div class="space-y-3 max-h-48 overflow-y-auto mb-3">
                  @for (entry of decisionLog(); track $index) {
                    <div class="bg-white rounded-lg p-3 border-l-2 border-amber-500">
                      ...
                      <p class="text-sm text-slate-900 font-medium">{{ entry.decision }}</p>
                      @if (entry.rationale) {
                ...
                <div class="space-y-2">
                  <input [(ngModel)]="newDecision" [placeholder]="lang.t('warroom.decision_placeholder')"
                         class="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-900 focus:outline-none focus:border-amber-500/50"/>
                  <div class="flex gap-2">
                    <input [(ngModel)]="decisionRationale" [placeholder]="lang.t('warroom.rationale_placeholder')"
                           class="flex-1 bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-900 focus:outline-none"/>
            ...
            <div class="space-y-4">
              ...
              <div class="bg-white border border-slate-200 rounded-xl p-4 space-y-3">
                <h3 class="text-slate-900 font-semibold text-sm">{{ lang.t('warroom.status_board') }}</h3>
                <div class="space-y-2 text-sm">
                  ...
                  <div class="flex justify-between">
                    ...
                    <span class="text-slate-900">{{ incident()!.incidentType }}</span>
                  </div>
                  <div class="flex justify-between">
                  ...
                  <div class="flex justify-between">
                    ...
                    <span class="text-slate-900">{{ incident()!.warRoomPhase || 'N/A' }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-slate-400">{{ lang.t('warroom.status') }}</span>
                    <span class="text-slate-900">{{ incident()!.reportingStatus }}</span>
                  </div>
                </div>
              </div>
              ...
              <!-- Role Assignment -->
              <div class="bg-white border border-slate-200 rounded-xl p-4 space-y-3">
                <h3 class="text-slate-900 font-semibold text-sm">{{ lang.t('warroom.roles') }}</h3>
                @for (role of roleKeys; track role) {
                  <div>
                    ...
                    <input [(ngModel)]="roles[role]" (blur)="saveRoles()"
                           class="w-full bg-white border border-slate-200 rounded-lg px-3 py-1.5 text-sm text-slate-900 focus:outline-none focus:border-slate-500/50"
                           placeholder="Name"/>
                  </div>
                }
              </div>
              ...
              <div class="bg-white border border-slate-200 rounded-xl p-4 space-y-2">
                <h3 class="text-slate-900 font-semibold text-sm mb-1">{{ lang.t('warroom.quick_actions') }}</h3>
                <a routerLink="/incident-reporting" class="block px-3 py-2 rounded-lg bg-slate-100/30 text-slate-600 text-xs hover:bg-slate-100 transition-colors">
                  {{ lang.t('warroom.submit_fsa_report') }}
                </a>
                <button (click)="exportPdf()" class="w-full text-left px-3 py-2 rounded-lg bg-slate-100/30 text-slate-600 text-xs hover:bg-slate-100 transition-colors">
          ...
          <!-- War room not active -->
          <div class="text-center py-12 bg-slate-100/30 border border-slate-200 rounded-xl">
            <svg class="w-16 h-16 text-slate-600 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z"/>
            </svg>
            <h3 class="text-slate-900 font-semibold mb-2">{{ lang.t('warroom.inactive_title') }}</h3>
