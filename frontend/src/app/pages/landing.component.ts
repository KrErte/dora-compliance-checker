...
@Component({
  ...
  host: { class: 'block bg-[#0a0f1e]' },
  imports: [CommonModule, RouterLink, FormsModule],
  template: `
    <!-- Logged-in banner -->
    <div *ngIf="auth.isLoggedIn()" class="bg-slate-900/80 border-b border-slate-700/50 backdrop-blur-xl">
      <div class="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
        ...
        <a routerLink="/dashboard"
           class="bg-emerald-500 hover:bg-emerald-400
                  ...
                  hover:shadow-lg hover:shadow-emerald-500/30 flex items-center gap-1.5 whitespace-nowrap">
          Go to Dashboard
    ...
    <div class="relative overflow-hidden">
      <div class="absolute inset-0 bg-gradient-to-b from-[#0a0f1e] via-[#0d1528] to-[#111b33]"></div>
      <div class="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[400px] bg-blue-900/20 rounded-full blur-3xl"></div>

      <div class="relative flex flex-col items-center justify-center min-h-[55vh] text-center z-10 py-14 px-4">
        <!-- Live badge -->
        <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-red-500/10 border border-red-500/30 mb-6">
          <span class="relative flex h-2 w-2">
            <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-500 opacity-75"></span>
            <span class="relative inline-flex rounded-full h-2 w-2 bg-red-500"></span>
          </span>
          <span class="text-xs font-bold text-red-400">{{ lang.l('Jõustunud — järelevalve aktiivne', 'In force — regulators actively enforcing') }}</span>
        </div>

        <h1 class="text-4xl md:text-5xl font-extrabold mb-5 leading-tight max-w-3xl text-white">
          {{ lang.l('Kas sinu IKT-leping läbib DORA auditi?', 'Will your ICT contract survive a DORA audit?') }}
        </h1>

        <p class="text-lg text-slate-400 max-w-2xl mb-4 leading-relaxed">
        ...
        <div class="flex flex-col items-center gap-4">
          @if (auth.isLoggedIn()) {
            <a routerLink="/dashboard"
               class="group inline-flex items-center gap-3 bg-emerald-500 hover:bg-emerald-400
            ...
            <p class="text-sm mt-2 flex items-center justify-center gap-1.5">
              ...
              <span class="font-bold text-slate-400">{{ lang.l('Andmed on krüpteeritud ja kustutatakse pärast analüüsi', 'Your data is encrypted and deleted after analysis') }}</span>
            </p>
          }
        </div>
      </div>
    ...
    <div class="bg-slate-900/80 border-y border-slate-700/50 py-6">
      <div class="max-w-4xl mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
        ...
        <div>
          <div class="text-2xl font-extrabold text-white">5 {{ lang.l('min', 'min') }}</div>
          <div class="text-xs text-slate-500 mt-1">{{ lang.l('Analüüsi aeg', 'Analysis time') }}</div>
        </div>
        <div>
          <div class="text-2xl font-extrabold text-white">PDF</div>
          <div class="text-xs text-slate-500 mt-1">{{ lang.l('Raport juhatusele', 'Board-ready report') }}</div>
        </div>
        <div>
          <div class="text-2xl font-extrabold text-red-400">{{ lang.l('2M EUR', '2M EUR') }}</div>
    ...
    <div class="flex flex-wrap justify-center items-center gap-4 sm:gap-6 py-5 border-b border-slate-800">
      <a routerLink="/nis2/scope-check"
         class="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg border border-amber-500/40 text-sm text-amber-400 hover:bg-amber-500/10 hover:border-amber-400 transition-all font-medium">
      ...
      <a routerLink="/fine-calculator"
         class="text-sm text-slate-400 hover:text-red-400 transition-colors inline-flex items-center gap-1.5 font-medium">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <a routerLink="/assessment"
         class="text-sm text-slate-400 hover:text-emerald-400 transition-colors inline-flex items-center gap-1.5 font-medium">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="py-14 relative">
      <div class="relative z-10">
        <div class="text-center mb-10">
          <h2 class="text-2xl md:text-3xl font-bold text-white">{{ lang.t('landing.steps_title') }}</h2>
        </div>
          ...
          <div class="how-it-works-grid">
            <ng-container *ngFor="let step of steps; let i = index; let last = last">
              <div class="how-it-works-card group" [style.animation-delay]="i * 150 + 'ms'">
                ...
                <h3 class="font-bold text-base text-white mb-1">{{ lang.t(step.titleKey) }}</h3>
                <p class="text-sm text-slate-500 leading-snug line-clamp-2">{{ lang.t(step.descKey) }}</p>
              </div>
    ...
    <div class="py-10 px-4 bg-slate-900/60 border-y border-slate-700/50">
      <div class="max-w-4xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div class="glass-card p-5 rounded-xl border border-slate-700/50 bg-slate-800/50">
            ...
            <p class="text-sm text-slate-400 leading-relaxed mb-4">{{ lang.l('"Leidis meie lepingust 3 kriitilist DORA puudust, mida meie juristid olid vahele jätnud. Ajas mitu tundi kokku säästnud."', '"Found 3 critical DORA gaps in our contract that our lawyers had missed. The PDF report went straight to the board."') }}</p>
            <div class="flex items-center gap-2">
              <div class="w-8 h-8 rounded-full bg-blue-900/40 flex items-center justify-center text-blue-400 text-xs font-bold">M.K.</div>
              <div>
                <p class="text-xs font-medium text-slate-300">{{ lang.l('Riskijuht', 'Risk Manager') }}</p>
          ...
          <div class="glass-card p-5 rounded-xl border border-slate-700/50 bg-slate-800/50">
            ...
            <p class="text-sm text-slate-400 leading-relaxed mb-4">{{ lang.l('"DoraBot vastas regulatsiooniküsimustele kiiremini kui meie konsultant. Praktilised, artiklitele viitavad vastused."', '"Used it before our FSA audit — the gap analysis showed exactly what we needed to fix. Worth every cent."') }}</p>
            <div class="flex items-center gap-2">
              <div class="w-8 h-8 rounded-full bg-blue-600/20 flex items-center justify-center text-blue-500 text-xs font-bold">L.V.</div>
              <div>
                <p class="text-xs font-medium text-slate-300">{{ lang.l('Vastavusjuht', 'Compliance Officer') }}</p>
          ...
          <div class="glass-card p-5 rounded-xl border border-slate-700/50 bg-slate-800/50">
            ...
            <p class="text-sm text-slate-400 leading-relaxed mb-4">{{ lang.l('"Intsidendihalduse moodul aitas meil struktureerida regulaatori teavitamise protsessi. Lihtsaim DORA tööriist turul."', '"Replaced a EUR 15,000 consultant engagement. The AI policy writer generated our ICT risk framework in minutes."') }}</p>
            <div class="flex items-center gap-2">
              ...
              <div>
                <p class="text-xs font-medium text-slate-300">{{ lang.l('IKT direktor', 'ICT Director') }}</p>
    ...
    <div class="py-16 relative">
      <div class="absolute inset-0 overflow-hidden">
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[300px] bg-slate-800/40 rounded-full blur-3xl"></div>
      </div>
      <div class="relative z-10 max-w-5xl mx-auto px-4">
        <div class="grid md:grid-cols-2 gap-8 items-center">
          <!-- Left: Text -->
          <div>
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-800/400/10 border border-blue-500/30 mb-4">
              <div class="w-4 h-4 rounded bg-blue-600 flex items-center justify-center text-slate-900 text-[7px] font-bold">AI</div>
              <span class="text-xs font-medium text-blue-400 uppercase tracking-wider">{{ lang.t('landing.ai_badge') }}</span>
            </div>
            <h2 class="text-2xl md:text-3xl font-bold text-white mb-3">{{ lang.t('landing.ai_title') }}</h2>
            ...
            <div class="space-y-3 mb-6">
              <div class="flex items-start gap-3">
                ...
                <span class="text-sm text-slate-400">{{ lang.t('landing.ai_f1') }}</span>
              </div>
              <div class="flex items-start gap-3">
                <svg class="w-5 h-5 text-blue-600 shrink-0 mt-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
                <span class="text-sm text-slate-400">{{ lang.t('landing.ai_f2') }}</span>
              </div>
              <div class="flex items-start gap-3">
                <svg class="w-5 h-5 text-blue-600 shrink-0 mt-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
                <span class="text-sm text-slate-400">{{ lang.t('landing.ai_f3') }}</span>
              </div>
            </div>
          ...
          <div class="relative">
            <div class="bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl shadow-black/50 overflow-hidden max-w-sm mx-auto">
              <!-- Chat header -->
              <div class="flex items-center gap-2.5 px-4 py-3 bg-gradient-to-r from-blue-700/20 to-blue-600/20 border-b border-slate-700">
              ...
              <div class="px-3 py-3 space-y-3">
                <div class="flex justify-end">
                  <div class="max-w-[80%] px-3 py-2 rounded-xl rounded-br-sm bg-blue-600/20 text-blue-200 border border-blue-500/30 text-sm">
                    {{ lang.t('landing.ai_q') }}
                  </div>
                </div>
                <div class="flex">
                  <div class="max-w-[85%] px-3 py-2 rounded-xl rounded-bl-sm bg-slate-700/50 text-slate-300 border border-slate-600 text-sm leading-relaxed">
                    {{ lang.t('landing.ai_a') }}
                  </div>
                </div>
                <div class="flex">
                  <div class="max-w-[85%]">
                    <div class="px-3 py-1.5 rounded-lg bg-slate-800/400/10 border border-blue-500/30 text-blue-600 text-xs flex items-center gap-1.5">
            ...
            <!-- Glow effect -->
            <div class="absolute -inset-4 bg-gradient-to-r from-blue-600/20 to-blue-500/20 rounded-3xl blur-2xl -z-10"></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="py-16 max-w-4xl mx-auto px-4">
      <div class="text-center mb-10">
        ...
        <h2 class="text-2xl font-bold text-white">{{ lang.t('landing.interactive_title') }}</h2>
        ...
        <table class="w-full min-w-[480px]">
          ...
          <tbody>
            <ng-container *ngFor="let req of requirements; let i = index">
              <tr class="requirement-row border-t border-slate-700/40 cursor-pointer hover:bg-slate-700/30 transition-colors"
                ...
                <td class="px-4 py-4 text-right">
                  <span *ngIf="req.checked" class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-slate-800/400/20 text-blue-400">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                    </svg>
              ...
              <tr *ngIf="req.expanded" class="bg-slate-700/30 animate-slide-down">
    ...
    <div class="py-16 px-4">
      <div class="max-w-5xl mx-auto">
        <div class="text-center mb-10">
          ...
          <h2 class="text-2xl md:text-3xl font-bold text-white mb-3">{{ lang.l('AI-põhine vastavuse automatiseerimine', 'AI-Powered Compliance Automation') }}</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <a routerLink="/document-analyzer"
             class="glass-card p-5 rounded-xl border border-emerald-500/30 hover:border-emerald-400 cursor-pointer transition-all duration-300 hover:bg-emerald-500/10 hover:shadow-lg hover:shadow-emerald-500/10 group">
            <div class="flex items-start gap-4">
              ...
              <div class="flex-1">
                <div class="flex items-center gap-2 mb-1">
                  <h3 class="text-sm font-semibold text-slate-300 group-hover:text-emerald-500 transition-colors">{{ lang.l('Dokumendianalüüs (Zero-Upload)', 'Document Analyzer (Zero-Upload)') }}</h3>
                  <span class="px-1.5 py-0.5 text-[10px] font-bold bg-emerald-500/20 text-emerald-400 rounded">{{ lang.l('UUS', 'NEW') }}</span>
                </div>
          ...
          <a routerLink="/evidence-gap-analyzer"
             class="glass-card p-5 rounded-xl border border-teal-500/30 hover:border-teal-400 cursor-pointer transition-all duration-300 hover:bg-teal-500/10 hover:shadow-lg hover:shadow-teal-500/10 group">
            <div class="flex items-start gap-4">
              ...
              <div class="flex-1">
                <div class="flex items-center gap-2 mb-1">
                  <h3 class="text-sm font-semibold text-slate-300 group-hover:text-teal-300 transition-colors">{{ lang.l('Tõendite lünkaanalüüs', 'Evidence Gap Analyzer') }}</h3>
                </div>
          ...
          <a routerLink="/ai-policy-writer"
             class="glass-card p-5 rounded-xl border border-violet-500/30 hover:border-violet-400 cursor-pointer transition-all duration-300 hover:bg-violet-500/10 hover:shadow-lg hover:shadow-violet-500/10 group">
            <div class="flex items-start gap-4">
              ...
              <div class="flex-1">
                <div class="flex items-center gap-2 mb-1">
                  <h3 class="text-sm font-semibold text-slate-300 group-hover:text-violet-300 transition-colors">{{ lang.l('AI poliitikakirjutaja', 'AI Policy Writer') }}</h3>
          ...
          <a routerLink="/incident-reporting"
             class="glass-card p-5 rounded-xl border border-amber-500/30 hover:border-amber-400 cursor-pointer transition-all duration-300 hover:bg-amber-500/10 hover:shadow-lg hover:shadow-amber-500/10 group">
            <div class="flex items-start gap-4">
              ...
              <div class="flex-1">
                <div class="flex items-center gap-2 mb-1">
                  <h3 class="text-sm font-semibold text-slate-300 group-hover:text-amber-300 transition-colors">{{ lang.l('IKT intsidentide haldamine', 'ICT Incident Management') }}</h3>
                </div>
          ...
          <a routerLink="/prosecutor"
             class="glass-card p-5 rounded-xl border border-indigo-500/30 hover:border-indigo-400 cursor-pointer transition-all duration-300 hover:bg-indigo-500/10 hover:shadow-lg hover:shadow-indigo-500/10 group">
            <div class="flex items-start gap-4">
              ...
              <div class="flex-1">
                <div class="flex items-center gap-2 mb-1">
                  <h3 class="text-sm font-semibold text-slate-300 group-hover:text-indigo-300 transition-colors">{{ lang.l('AI prokurör', 'AI Prosecutor') }}</h3>
          ...
          <a routerLink="/pricing"
             class="glass-card p-5 rounded-xl border border-blue-500/30 hover:border-blue-400 cursor-pointer transition-all duration-300 hover:bg-slate-800/40 hover:shadow-lg hover:shadow-blue-500/10 group">
            <div class="flex items-start gap-4">
              <div class="w-12 h-12 rounded-xl bg-slate-800/40 flex items-center justify-center shrink-0 group-hover:bg-blue-600/20 transition-colors">
              ...
              <div class="flex-1">
                <div class="flex items-center gap-2 mb-1">
                  <h3 class="text-sm font-semibold text-slate-300 group-hover:text-blue-400 transition-colors">{{ lang.l('xBRL-CSV regulatiivne eksport', 'xBRL-CSV Regulatory Export') }}</h3>
    ...
    <div class="py-16 relative">
      ...
      <div class="absolute inset-0 overflow-hidden">
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[400px] bg-slate-800/40 rounded-full blur-3xl"></div>
      </div>
      ...
      <div class="relative z-10">
        <div class="text-center mb-10">
          <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-800/400/10 border border-blue-500/30 mb-4">
            ...
            <span class="text-xs font-medium text-blue-400 uppercase tracking-wider">{{ lang.t('landing.pillars_label') }}</span>
          </div>
          <h2 class="text-2xl md:text-3xl font-bold text-white mb-3">{{ lang.t('landing.pillars_title') }}</h2>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 max-w-4xl mx-auto px-4 stagger-children">
          <a *ngFor="let pillar of pillars; let i = index" [routerLink]="'/pillar/' + pillar.id"
            ...
            <div class="tilt-card-inner feature-card glass-card p-5 text-center rounded-xl border border-slate-700/50 group-hover:border-blue-400/40 transition-all duration-300 relative overflow-hidden">
              ...
              <h3 class="text-sm font-semibold text-slate-300 group-hover:text-blue-500 transition-colors mb-1">{{ lang.t(pillar.labelKey) }}</h3>
              <p class="text-xs text-slate-500 mb-2">{{ pillar.articles }}</p>
              <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-800/400/20 text-blue-400 badge-shine">
                <span class="w-1.5 h-1.5 rounded-full bg-slate-800/400 animate-pulse"></span>
    ...
    <!-- FAQ Section -->
    <div class="py-16 px-4 bg-slate-900/60">
      <div class="max-w-3xl mx-auto">
        <h2 class="text-2xl font-bold text-white text-center mb-8">{{ lang.t('landing.faq_title') }}</h2>
    ...
    <!-- Contact Form -->
    <div id="contact" class="py-16 bg-slate-700/30">
      <div class="max-w-4xl mx-auto px-4">
        <div class="text-center mb-8">
          <h2 class="text-2xl font-bold text-white mb-2">{{ lang.t('landing.contact_title') }}</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div class="flex flex-col gap-4">
                <div>
                  ...
                  <input type="text" [(ngModel)]="contactName" name="name" id="contact-name" required
                         [class]="'w-full px-4 py-3 rounded-xl bg-slate-800/60 border text-slate-200 placeholder-slate-500 bg-slate-800 focus:outline-none transition-colors ' +
                                  (contactNameError ? 'border-red-500 focus:border-red-400' : 'border-slate-600 focus:border-teal-500')"
                ...
                <div>
                  ...
                  <input type="email" [(ngModel)]="contactEmail" name="email" id="contact-email" [placeholder]="lang.t('landing.contact_email_placeholder')"
                         [class]="'w-full px-4 py-3 rounded-xl bg-slate-800/60 border text-slate-200 placeholder-slate-500 bg-slate-800 focus:outline-none transition-colors ' +
                                  (contactEmailError ? 'border-red-500 focus:border-red-400' : 'border-slate-600 focus:border-teal-500')"
                         required>
                ...
                <div>
                  ...
                  <select [(ngModel)]="contactReason" name="reason" id="contact-reason"
                          [class]="'w-full px-4 py-3 rounded-xl bg-slate-800/60 border text-slate-200 bg-slate-800 focus:outline-none transition-colors cursor-pointer ' +
                                   (contactReasonError ? 'border-red-500 focus:border-red-400' : 'border-slate-600 focus:border-teal-500')">
                ...
                <div>
                  ...
                  <textarea [(ngModel)]="contactMessage" name="message" id="contact-message" rows="3"
                            [class]="'w-full px-4 py-3 rounded-xl bg-slate-800/60 border text-slate-200 placeholder-slate-500 bg-slate-800 focus:outline-none transition-colors resize-none ' +
                                     (contactMessageError ? 'border-red-500 focus:border-red-400' : 'border-slate-600 focus:border-teal-500')"
                ...
                <button type="submit" [disabled]="contactSending"
                        class="cta-button w-full px-6 py-3 rounded-xl bg-gradient-to-r from-teal-500 to-blue-600 hover:from-teal-400 hover:to-blue-400 text-white font-semibold disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
          ...
          <div class="glass-card p-6 h-fit">
            <h3 class="text-lg font-semibold text-white mb-6">{{ lang.t('landing.contact_info_title') }}</h3>
            ...
            <div class="space-y-5">
              ...
              <div class="flex items-start gap-3">
                ...
                <div>
                  ...
                  <a href="mailto:info@doraaudit.eu" class="text-slate-300 hover:text-teal-400 transition-colors">info&#64;doraaudit.eu</a>
                </div>
              </div>

              <!-- Book Demo -->
              <div class="flex items-start gap-3">
                <div class="w-10 h-10 rounded-lg bg-slate-800/40 flex items-center justify-center shrink-0">
              ...
              <div class="flex items-start gap-3">
                ...
                <div>
                  <p class="text-sm font-medium text-slate-300">DoraAudit.eu</p>
              ...
              <div class="pt-4 border-t border-blue-200/50">
                ...
                <div class="flex flex-col gap-2">
                  <a href="https://www.linkedin.com/company/doraaudit" target="_blank" rel="noopener"
                     class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-800/60 border border-slate-700 hover:border-blue-500/30 hover:bg-slate-800/40 transition-all group">
                    <svg class="w-5 h-5 text-blue-400" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                    </svg>
                    <span class="text-sm text-slate-400 group-hover:text-blue-300">DoraAudit.eu <span class="text-xs text-slate-500">{{ lang.l('(ametlik leht)', '(company page)') }}</span></span>
                  </a>
