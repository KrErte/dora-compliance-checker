...
import { AssessmentResult, CATEGORY_LABELS, ContractClause, CategoryMaturity } from '../models';
  ...
  template: `
    <div class="max-w-4xl mx-auto">
      ...
      <div *ngIf="result">
            ...
            <div class="relative w-40 h-40 shrink-0">
              <svg class="w-full h-full -rotate-90" viewBox="0 0 100 100">
                ...
                <circle cx="50" cy="50" r="45" fill="none"
                        ...
                        [attr.stroke-dashoffset]="282.74 - (282.74 * result.weightedScorePercentage / 100)"
              ...
              <div class="absolute inset-0 flex flex-col items-center justify-center animate-score-count delay-500">
                <span class="text-4xl font-extrabold" [class]="scoreTextClass">{{ result.weightedScorePercentage | number:'1.0-0' }}</span>
                <span class="text-xs text-slate-500">kaalutud skoor</span>
              </div>
            </div>
        ...
        <!-- Penalty Risk + Weighted Score Banner -->
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          <div class="bg-gradient-to-br from-red-900/20 to-slate-800/50 backdrop-blur border border-red-700/20 rounded-xl p-6 animate-fade-in-up delay-400">
            <h2 class="text-sm font-semibold text-red-300 mb-3 flex items-center gap-2">
              <svg class="w-4 h-4 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z"/>
              </svg>
              Regulatiivse sanktsiooni risk
            </h2>
            <div class="flex items-end gap-3">
              <span class="text-5xl font-extrabold" [class]="result.estimatedPenaltyRiskPercent < 20 ? 'text-emerald-400' : result.estimatedPenaltyRiskPercent < 50 ? 'text-amber-400' : 'text-red-400'">
                {{ result.estimatedPenaltyRiskPercent | number:'1.0-0' }}%
              </span>
              <span class="text-sm text-slate-500 mb-2">t&otilde;en&auml;osus</span>
            </div>
            <p class="text-xs text-slate-500 mt-2">DORA v&otilde;imaldab trahve kuni 1% keskmisest p&auml;evasest &uuml;lemaailmsest k&auml;ibest</p>
          </div>
          <div class="bg-gradient-to-br from-violet-900/20 to-slate-800/50 backdrop-blur border border-violet-700/20 rounded-xl p-6 animate-fade-in-up delay-500">
            <h2 class="text-sm font-semibold text-violet-300 mb-3 flex items-center gap-2">
              <svg class="w-4 h-4 text-violet-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/>
              </svg>
              Kategooriate k&uuml;psustase
            </h2>
            <div class="space-y-2">
              <div *ngFor="let cat of result.categoryMaturity" class="flex items-center gap-2">
                <span class="text-xs text-slate-400 w-28 truncate" [title]="cat.categoryLabel">{{ cat.categoryLabel }}</span>
                <div class="flex gap-0.5 flex-1">
                  <div *ngFor="let lvl of [1,2,3,4,5]"
                       class="h-3 flex-1 rounded-sm transition-all duration-500"
                       [class]="lvl <= cat.maturityLevel
                         ? (cat.maturityLevel >= 4 ? 'bg-emerald-500' : cat.maturityLevel >= 3 ? 'bg-amber-500' : 'bg-red-500')
                         : 'bg-slate-700'">
                  </div>
                </div>
                <span class="text-xs font-mono w-6 text-right" [class]="cat.maturityLevel >= 4 ? 'text-emerald-400' : cat.maturityLevel >= 3 ? 'text-amber-400' : 'text-red-400'">{{ cat.maturityLevel }}</span>
              </div>
            </div>
          </div>
        </div>
            ...
            <div *ngFor="let item of nonCompliantItems; let i = index"
              ...
              <div class="shrink-0 flex items-center gap-1.5">
                <span class="text-xs px-2 py-0.5 rounded-full"
                      [class]="item.severity === 'CRITICAL' ? 'bg-red-500/15 text-red-400 border border-red-500/20' :
                               item.severity === 'HIGH' ? 'bg-amber-500/15 text-amber-400 border border-amber-500/20' :
                               'bg-blue-500/15 text-blue-400 border border-blue-500/20'">
                  {{ item.severity === 'CRITICAL' ? 'Kriitiline' : item.severity === 'HIGH' ? 'K&otilde;rge' : 'Keskmine' }}
                </span>
                <span class="text-xs text-slate-600">{{ item.weight }}x</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- AUTO-GENERATED CONTRACT CLAUSES — the killer differentiator -->
        <div *ngIf="result.missingClauses && result.missingClauses.length > 0"
             class="bg-gradient-to-br from-cyan-900/10 to-slate-800/50 backdrop-blur border border-cyan-700/20 rounded-xl p-6 mb-8 animate-fade-in-up delay-600">
          <div class="flex items-center justify-between mb-4">
            <h2 class="text-lg font-semibold text-cyan-300 flex items-center gap-2">
              ...
              Genereeritud lepinguklauslid
            </h2>
            <span class="text-xs px-3 py-1 rounded-full bg-cyan-500/10 text-cyan-400 border border-cyan-500/20">
              {{ result.missingClauses.length }} klauslit
            </span>
          </div>
          <p class="text-xs text-slate-500 mb-4">Automaatselt genereeritud eestikeelsed lepinguklauslid iga puuduoleva DORA n&otilde;ude jaoks. Kopeerige klauslid otse oma lepingusse v&otilde;i laadige alla PDF aruanne.</p>

          <div *ngFor="let clause of result.missingClauses; let i = index"
               class="mb-3 animate-fade-in-up"
               [style.animation-delay]="(i * 80 + 700) + 'ms'">
            <div class="flex items-center gap-2 mb-1.5">
              <span class="text-xs font-bold text-slate-300">{{ clause.priority }}.</span>
              <span class="text-xs text-slate-400">{{ clause.articleReference }}</span>
              <span class="text-xs px-2 py-0.5 rounded-full"
                    [class]="clause.severity === 'CRITICAL' ? 'bg-red-500/15 text-red-400 border border-red-500/20' :
                             clause.severity === 'HIGH' ? 'bg-amber-500/15 text-amber-400 border border-amber-500/20' :
                             'bg-blue-500/15 text-blue-400 border border-blue-500/20'">
                {{ clause.severity === 'CRITICAL' ? 'Kriitiline' : clause.severity === 'HIGH' ? 'K\u00f5rge' : 'Keskmine' }}
              </span>
              <button (click)="copyClause(clause.clauseTextEt)"
                      class="ml-auto text-xs text-slate-500 hover:text-cyan-400 transition-colors flex items-center gap-1">
            ...
            <div class="bg-slate-900/60 border border-slate-700/40 rounded-lg p-4 text-sm text-slate-300 leading-relaxed font-serif italic">
              {{ clause.clauseTextEt }}
            </div>
          </div>
        </div>
...
export class ResultsComponent implements OnInit {
  ...
  nonCompliantItems: { question: string; articleReference: string; category: string; severity: string; weight: number }[] = [];
  ...
  saveToHistory() {
    ...
    if (!exists) {
      history.unshift({
        ...
        scorePercentage: this.result.weightedScorePercentage,
  ...
  exportPdf() {
    if (this.result) {
      this.api.downloadReport(this.result.id);
    }
  }

  copyClause(text: string) {
