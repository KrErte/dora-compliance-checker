...
@Component({
  ...
  template: `
    <div class="space-y-8">
      ...
      <!-- Create form modal -->
      @if (showCreateForm()) {
        <div class="fixed inset-0 bg-white/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" (click)="showCreateForm.set(false)">
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input [(ngModel)]="newIncident.clientsAffected" type="number"
                           class="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-900 text-sm focus:outline-none focus:border-blue-500/50">
                  </div>
                  <div>
              ...
              <div class="flex justify-end gap-3 pt-4">
                <button (click)="showCreateForm.set(false)"
                        class="px-5 py-2.5 rounded-xl bg-slate-100/50 border border-slate-200 text-slate-600 text-sm hover:bg-slate-100 transition-all">
      ...
      @if (loading()) {
        <div class="text-center py-16">
          <div class="w-10 h-10 mx-auto mb-3 rounded-full border-4 border-slate-200 border-t-red-400 animate-spin"></div>
        </div>
      }
      ...
      @if (!loading() && incidents().length === 0 && !showCreateForm()) {
        <div class="bg-white border border-slate-200 rounded-2xl p-12 text-center">
          ...
          <h3 class="text-slate-900 font-semibold mb-2">{{ lang.t('incident.no_incidents') }}</h3>
              ...
              <div class="flex flex-col md:flex-row md:items-center gap-4">
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2 mb-1">
                    ...
                    <!-- Type badge -->
                    <span class="px-2 py-0.5 rounded-full text-[10px] font-medium bg-slate-100/50 text-slate-600">
                      {{ getTypeLabel(incident.incidentType) }}
                    </span>
                  </div>
                  <h3 class="text-slate-900 font-semibold truncate">{{ incident.incidentTitle }}</h3>
      ...
      @if (selectedIncident()) {
        <div class="fixed inset-0 bg-white/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" (click)="selectedIncident.set(null)">
          <div class="bg-white border border-slate-200 rounded-2xl p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto" (click)="$event.stopPropagation()">
            <div class="flex items-start justify-between mb-6">
              <div>
                ...
                <h2 class="text-xl font-bold text-slate-900">{{ selectedIncident()!.incidentTitle }}</h2>
              </div>
              <button (click)="selectedIncident.set(null)" class="text-slate-400 hover:text-slate-900">
              ...
              <div class="space-y-3">
                ...
                <div class="flex items-start gap-3">
                  <div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0"
                       [class]="selectedIncident()!.classifiedAt ? 'bg-blue-100 text-blue-600 border border-blue-200' : 'bg-slate-200 text-slate-400 border border-slate-300'">1</div>
                  <div class="flex-1">
                    <div class="text-sm font-medium text-slate-900">{{ lang.t('incident.detection_classification') }}</div>
                ...
                <div class="flex items-start gap-3">
                  <div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0"
                       [class]="selectedIncident()!.initialReportSentAt ? 'bg-blue-100 text-blue-600 border border-blue-200' : 'bg-slate-200 text-slate-400 border border-slate-300'">2</div>
                  <div class="flex-1">
                    <div class="text-sm font-medium text-slate-900">{{ lang.t('incident.initial_notification') }}</div>
                    <div class="text-xs text-slate-400">
                ...
                <div class="flex items-start gap-3">
                  <div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0"
                       [class]="selectedIncident()!.intermediateReportSentAt ? 'bg-blue-100 text-blue-600 border border-blue-200' : 'bg-slate-200 text-slate-400 border border-slate-300'">3</div>
                  <div class="flex-1">
                    <div class="text-sm font-medium text-slate-900">{{ lang.t('incident.intermediate_report') }}</div>
                    <div class="text-xs text-slate-400">
                ...
                <div class="flex items-start gap-3">
                  <div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0"
                       [class]="selectedIncident()!.finalReportSentAt ? 'bg-blue-100 text-blue-600 border border-blue-200' : 'bg-slate-200 text-slate-400 border border-slate-300'">4</div>
                  <div class="flex-1">
                    <div class="text-sm font-medium text-slate-900">{{ lang.t('incident.final_report') }}</div>
                    <div class="text-xs text-slate-400">
            ...
            <div class="grid grid-cols-2 gap-4 mb-6">
              <div class="bg-white rounded-lg p-3">
                ...
                <div class="text-sm text-slate-900">{{ getTypeLabel(selectedIncident()!.incidentType) }}</div>
              </div>
              <div class="bg-white rounded-lg p-3">
                <div class="text-xs text-slate-400 mb-1">{{ lang.t('incident.clients_affected') }}</div>
                <div class="text-sm text-slate-900">{{ selectedIncident()!.clientsAffected || '-' }}</div>
              </div>
              <div class="bg-white rounded-lg p-3">
                <div class="text-xs text-slate-400 mb-1">{{ lang.t('incident.economic_impact_label') }}</div>
                <div class="text-sm text-slate-900">{{ selectedIncident()!.economicImpact ? '\u20ac' + (selectedIncident()!.economicImpact | number) : '-' }}</div>
              </div>
              <div class="bg-white rounded-lg p-3">
                <div class="text-xs text-slate-400 mb-1">{{ lang.t('incident.duration_label') }}</div>
                <div class="text-sm text-slate-900">{{ selectedIncident()!.durationMinutes ? selectedIncident()!.durationMinutes + ' min' : '-' }}</div>
              </div>
            </div>
            ...
            <div class="flex flex-wrap gap-2">
              ...
              @if (selectedIncident()!.reportingStatus === 'FINAL_SENT') {
                <button (click)="closeIncident()" class="px-4 py-2 rounded-xl bg-slate-100/50 text-slate-600 text-sm font-semibold hover:bg-slate-100 transition-all">
              ...
              <button (click)="exportPdf()" [disabled]="exportingPdf()"
                      ...
                      [class]="subscriptionService.canAccess('PDF_EXPORT') ? 'bg-violet-500/20 text-violet-400 hover:bg-violet-500/30' : 'bg-slate-100/50 text-slate-400'">
                @if (exportingPdf()) {
      ...
      @if (showReportForm()) {
        <div class="fixed inset-0 bg-white/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4" (click)="showReportForm.set(null)">
          <div class="bg-white border border-slate-200 rounded-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto" (click)="$event.stopPropagation()">
            ...
            <div class="space-y-4">
              <div>
                ...
                <textarea [(ngModel)]="reportForm.summary" rows="3"
                          class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-slate-900 placeholder-slate-500 focus:outline-none focus:border-blue-500/50"></textarea>
              </div>
              <div>
              ...
              <div class="flex justify-end gap-3 pt-2">
                <button (click)="showReportForm.set(null)" class="px-4 py-2 rounded-xl bg-slate-100/50 text-slate-600 text-sm">
                  {{ lang.t('incident.cancel') }}
                </button>
...
export class IncidentReportingComponent implements OnInit {
  ...
  getStatusClass(status: string): string {
    const classes: Record<string, string> = {
      DRAFT: 'bg-slate-100/50 text-slate-600',
