...
@Component({
    ...
    <div class="max-w-5xl mx-auto animate-fade-in-up">
      ...
      <div *ngIf="!selectedProfile()" class="text-center py-12">
        ...
        <div class="max-w-lg mx-auto mb-8">
          <div class="relative">
            <input type="text"
                   ...
                   class="w-full px-5 py-4 pl-12 bg-white border border-slate-200 rounded-xl
                          text-slate-900 placeholder:text-slate-500 focus:outline-none focus:border-blue-500/50
        ...
        <div *ngIf="searchResults().length > 0" class="max-w-lg mx-auto">
          <div class="glass-card divide-y divide-slate-200">
            <button *ngFor="let result of searchResults()"
                    (click)="selectCompany(result.registryCode)"
                    class="w-full px-4 py-3 text-left hover:bg-slate-100 transition-colors flex items-center justify-between group">
              <div>
                <p class="text-slate-900 font-medium group-hover:text-blue-600 transition-colors">{{ result.name }}</p>
        ...
        <div class="mt-12">
          <h2 class="text-lg font-semibold text-slate-600 mb-4">{{ lang.t('company.fi_licensed_title') }}</h2>
          <div *ngIf="fiLoadingState() === 'loading'" class="text-slate-500 flex items-center gap-2">
            <div class="w-4 h-4 border-2 border-slate-500/30 border-t-blue-500 rounded-full animate-spin"></div>
          ...
          <div *ngIf="fiLoadingState() === 'error'" class="text-amber-400 text-sm p-4 rounded-xl bg-amber-500/10 border border-amber-500/20">
            ...
            <button (click)="loadFiLicensedCompanies()" class="ml-2 text-blue-600 hover:text-blue-500 underline">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            <button *ngFor="let company of fiLicensedCompanies().slice(0, 12)"
                    ...
                    class="glass-card p-4 text-left hover:border-blue-200 transition-all group">
              <p class="text-slate-900 font-medium text-sm group-hover:text-blue-600 transition-colors truncate">{{ company.name }}</p>
          ...
          <button *ngIf="fiLicensedCompanies().length > 12"
                  ...
                  class="mt-4 text-blue-600 text-sm hover:text-blue-500 transition-colors">
      ...
      <div *ngIf="selectedProfile()" class="space-y-6">
        ...
        <div class="flex items-start justify-between">
          <div>
            <button (click)="clearSelection()" class="text-slate-400 hover:text-blue-600 transition-colors text-sm mb-2 flex items-center gap-1">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
              </svg>
              {{ lang.t('company.back_to_search') }}
            </button>
            <h1 class="text-2xl md:text-3xl font-extrabold text-slate-900">{{ selectedProfile()!.name }}</h1>
          ...
          <div class="flex items-center gap-3">
            ...
            <button (click)="refreshProfile()" [disabled]="isRefreshing()"
                    class="p-2 rounded-lg bg-slate-700/50 text-slate-400 hover:text-blue-600 hover:bg-slate-50 transition-all disabled:opacity-50">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div class="glass-card p-5">
            ...
            <p class="text-lg font-semibold text-slate-900 capitalize">{{ selectedProfile()!.fiLicenseType || 'N/A' }}</p>
            <p class="text-xs text-slate-500 mt-1">
              <span class="inline-block w-2 h-2 rounded-full mr-1"
                    [class.bg-blue-600]="selectedProfile()!.fiLicenseStatus === 'active'"
          ...
          <div class="glass-card p-5">
            ...
            <p class="text-lg font-semibold text-slate-900">{{ formatDate(selectedProfile()!.lastCrawledAt) }}</p>
        ...
        <div class="glass-card p-6">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div class="bg-white rounded-lg p-4">
              ...
              <div class="flex items-center gap-2">
                <span class="text-slate-900 font-medium">{{ selectedProfile()!.securityHeadersScore ?? 0 }}/7</span>
              </div>
            ...
            <div class="bg-white rounded-lg p-4">
              ...
              <div class="flex gap-2">
                <span class="px-2 py-0.5 text-[10px] font-medium rounded"
                      [class]="selectedProfile()!.spfRecordExists ? 'bg-blue-100 text-blue-600' : 'bg-red-500/20 text-red-400'">
                  SPF {{ selectedProfile()!.spfRecordExists ? '✓' : '✗' }}
                </span>
                <span class="px-2 py-0.5 text-[10px] font-medium rounded"
                      [class]="selectedProfile()!.dmarcRecordExists ? 'bg-blue-100 text-blue-600' : 'bg-red-500/20 text-red-400'">
            ...
            <div class="bg-white rounded-lg p-4">
              ...
              <div class="flex items-center gap-2">
                <span class="w-3 h-3 rounded-full" [class]="selectedProfile()!.httpsRedirect ? 'bg-blue-600' : 'bg-red-500'"></span>
                <span class="text-slate-900 font-medium">{{ selectedProfile()!.httpsRedirect ? lang.t('company.enabled') : lang.t('company.disabled') }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div *ngIf="parsedBoardMembers().length > 0" class="glass-card p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div *ngFor="let member of parsedBoardMembers()" class="flex items-center gap-3 p-3 bg-slate-100/30 rounded-lg">
              ...
              <div>
                <p class="text-slate-900 font-medium">{{ member.name }}</p>
        ...
        <!-- Next Steps CTAs -->
        <div class="glass-card p-6">
          <h2 class="text-lg font-semibold text-slate-900 mb-4">{{ lang.t('company.next_steps') }}</h2>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <a routerLink="/contract-analysis" class="flex items-center gap-4 p-4 bg-slate-100/30 rounded-lg hover:bg-white transition-colors group">
              ...
              <div>
                <p class="text-slate-900 font-medium group-hover:text-blue-600 transition-colors">{{ lang.t('company.cta_contracts') }}</p>
                <p class="text-xs text-slate-500">{{ lang.t('company.cta_contracts_desc') }}</p>
              </div>
            </a>
            <a routerLink="/assessment" class="flex items-center gap-4 p-4 bg-slate-100/30 rounded-lg hover:bg-white transition-colors group">
              ...
              <div>
                <p class="text-slate-900 font-medium group-hover:text-blue-500 transition-colors">{{ lang.t('company.cta_assessment') }}</p>
                <p class="text-xs text-slate-500">{{ lang.t('company.cta_assessment_desc') }}</p>
              </div>
            </a>
            <a routerLink="/supply-chain" class="flex items-center gap-4 p-4 bg-slate-100/30 rounded-lg hover:bg-white transition-colors group">
              ...
              <div>
                <p class="text-slate-900 font-medium group-hover:text-amber-400 transition-colors">{{ lang.t('company.cta_supply_chain') }}</p>
                <p class="text-xs text-slate-500">{{ lang.t('company.cta_supply_chain_desc') }}</p>
              </div>
            </a>
            <a routerLink="/board-risk" class="flex items-center gap-4 p-4 bg-slate-100/30 rounded-lg hover:bg-white transition-colors group">
              ...
              <div>
                <p class="text-slate-900 font-medium group-hover:text-violet-400 transition-colors">{{ lang.t('company.cta_board_risk') }}</p>
  ...
  styles: [`
    .glass-card {
      @apply bg-white backdrop-blur border border-slate-200 rounded-xl;
    }
  `]
})
export class CompanyProfileComponent implements OnInit {
  ...
  getDoraTierClass(tier: number): string {
    switch (tier) {
      ...
      case 3: return 'bg-blue-600/20 text-blue-500 border border-blue-500/30';
