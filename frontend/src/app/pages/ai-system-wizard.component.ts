  ...
  template: `
    <div class="max-w-3xl mx-auto">
        ...
        @for (s of steps; track s.num; let i = $index) {
          <div class="flex items-center gap-2">
            <div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all"
                 [class]="step() >= s.num ? 'bg-gradient-to-br from-blue-500 to-purple-500 text-white' : 'bg-slate-700/50 text-slate-500 border border-slate-600/50'">
              {{ s.num }}
            </div>
            <span class="text-xs hidden sm:inline" [class]="step() >= s.num ? 'text-blue-400' : 'text-slate-500'">{{ lang.l(s.labelEt, s.labelEn) }}</span>
            @if (i < steps.length - 1) {
              <div class="w-8 h-px" [class]="step() > s.num ? 'bg-blue-500' : 'bg-slate-700'"></div>
            }
          </div>
        }
      </div>
      ...
      <div class="bg-slate-800/50 backdrop-blur border border-slate-700/50 rounded-xl p-6">
        ...
        <!-- Step 1: Basic Info -->
        @if (step() === 1) {
          <h2 class="text-lg font-semibold text-white mb-4">{{ lang.l('P\u00f5hiinfo', 'Basic Information') }}</h2>
          <div class="space-y-4">
            <div>
              <label class="block text-sm text-slate-400 mb-1">{{ lang.l('Nimi', 'Name') }} *</label>
              <input type="text" [(ngModel)]="form.name" class="w-full bg-slate-900/50 border border-slate-700/50 rounded-lg px-4 py-2.5 text-white text-sm focus:border-blue-500/50 focus:outline-none" [placeholder]="lang.l('AI s\u00fcsteemi nimi', 'AI system name')">
            </div>
            <div>
              <label class="block text-sm text-slate-400 mb-1">{{ lang.l('Kirjeldus', 'Description') }}</label>
              <textarea [(ngModel)]="form.description" rows="3" class="w-full bg-slate-900/50 border border-slate-700/50 rounded-lg px-4 py-2.5 text-white text-sm focus:border-blue-500/50 focus:outline-none resize-none" [placeholder]="lang.l('Kirjeldage AI s\u00fcsteemi', 'Describe the AI system')"></textarea>
            </div>
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-sm text-slate-400 mb-1">{{ lang.l('Tarnija', 'Vendor') }}</label>
                <input type="text" [(ngModel)]="form.vendor" class="w-full bg-slate-900/50 border border-slate-700/50 rounded-lg px-4 py-2.5 text-white text-sm focus:border-blue-500/50 focus:outline-none" [placeholder]="lang.l('nt OpenAI', 'e.g. OpenAI')">
              </div>
              <div>
                <label class="block text-sm text-slate-400 mb-1">{{ lang.l('Versioon', 'Version') }}</label>
                <input type="text" [(ngModel)]="form.version" class="w-full bg-slate-900/50 border border-slate-700/50 rounded-lg px-4 py-2.5 text-white text-sm focus:border-blue-500/50 focus:outline-none" placeholder="e.g. 1.0">
              </div>
            </div>
          </div>
        }
        ...
        <!-- Step 2: Use Context -->
        @if (step() === 2) {
          <h2 class="text-lg font-semibold text-white mb-4">{{ lang.l('Kasutuskontekst', 'Use Context') }}</h2>
          <div class="space-y-4">
            <div>
              <label class="block text-sm text-slate-400 mb-1">{{ lang.l('Kasutuse eesm\u00e4rk', 'Purpose') }}</label>
              <textarea [(ngModel)]="form.purpose" rows="3" class="w-full bg-slate-900/50 border border-slate-700/50 rounded-lg px-4 py-2.5 text-white text-sm focus:border-blue-500/50 focus:outline-none resize-none" [placeholder]="lang.l('Milleks AI s\u00fcsteemi kasutatakse?', 'What is the AI system used for?')"></textarea>
            </div>
            <div>
              ...
              <div class="grid grid-cols-2 gap-3">
                @for (ctx of deploymentContexts; track ctx.value) {
                  <button type="button" (click)="form.deploymentContext = ctx.value"
                          ...
                          [class]="form.deploymentContext === ctx.value ? 'border-blue-500/50 bg-blue-500/10 text-blue-300' : 'border-slate-700/50 bg-slate-900/30 text-slate-400 hover:border-slate-600/50'">
        ...
        <!-- Step 3: Role -->
        @if (step() === 3) {
          <h2 class="text-lg font-semibold text-white mb-4">{{ lang.l('Organisatsiooni roll', 'Organization Role') }}</h2>
          ...
          <div class="space-y-3">
            @for (role of orgRoles; track role.value) {
              <button type="button" (click)="form.organizationRole = role.value"
                      ...
                      [class]="form.organizationRole === role.value ? 'border-blue-500/50 bg-blue-500/10 text-blue-300' : 'border-slate-700/50 bg-slate-900/30 text-slate-400 hover:border-slate-600/50'">
        ...
        <!-- Step 4: Summary -->
        @if (step() === 4) {
          <h2 class="text-lg font-semibold text-white mb-4">{{ lang.l('Kokkuv\u00f5te', 'Summary') }}</h2>
          <div class="space-y-3">
        ...
        <!-- Navigation Buttons -->
        <div class="flex items-center justify-between mt-6 pt-4 border-t border-slate-700/30">
          <button type="button" (click)="prevStep()"
                  class="px-4 py-2 rounded-lg text-sm text-slate-400 hover:text-white hover:bg-slate-700/30 transition-all"
          ...
          @if (step() < 4) {
            ...
            <button type="button" (click)="submit()" [disabled]="saving()"
                    class="px-6 py-2 rounded-lg text-sm font-medium bg-gradient-to-r from-emerald-500 to-cyan-500 text-slate-900 hover:from-emerald-400 hover:to-cyan-400 transition-all disabled:opacity-50 flex items-center gap-2">
              @if (saving()) {
