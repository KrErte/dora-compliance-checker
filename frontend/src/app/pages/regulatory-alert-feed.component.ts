    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center gap-3">
          @if (alerts().length > 0 && unreadCount() > 0) {
            <button (click)="markAllRead()"
                    class="px-4 py-2 text-sm font-medium text-emerald-400 border border-emerald-500/30 rounded-lg hover:bg-emerald-500/10 transition-colors">
              {{ lang.t('alerts.mark_all_read') }}
            </button>
          }
          <a routerLink="/alert-profile"
             class="px-4 py-2 text-sm font-medium text-slate-300 border border-slate-600 rounded-lg hover:bg-slate-700/50 transition-colors">
      ...
      @if (alerts().length > 0) {
        <div class="flex flex-wrap gap-2">
          <button (click)="filterSeverity.set('')"
                  [class]="!filterSeverity() ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' : 'bg-slate-800 text-slate-400 border-slate-700 hover:bg-slate-700/50'"
          ...
          @for (sev of severities; track sev) {
            @if (countBySeverity(sev) > 0) {
              <button (click)="filterSeverity.set(sev)"
                      [class]="filterSeverity() === sev ? severityActiveClass(sev) : 'bg-slate-800 text-slate-400 border-slate-700 hover:bg-slate-700/50'"
                      class="px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors">
          ...
          <button (click)="filterRead.set('')"
                  [class]="!filterRead() ? 'bg-slate-600/30 text-slate-300 border-slate-500/30' : 'bg-slate-800 text-slate-400 border-slate-700 hover:bg-slate-700/50'"
                  class="px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors">
            {{ lang.t('alerts.filter_all_status') }}
          </button>
          <button (click)="filterRead.set('unread')"
                  [class]="filterRead() === 'unread' ? 'bg-blue-500/20 text-blue-400 border-blue-500/30' : 'bg-slate-800 text-slate-400 border-slate-700 hover:bg-slate-700/50'"
                  class="px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors">
            {{ lang.t('alerts.unread') }} ({{ unreadCount() }})
          </button>
          <button (click)="filterRead.set('read')"
                  [class]="filterRead() === 'read' ? 'bg-slate-500/20 text-slate-300 border-slate-500/30' : 'bg-slate-800 text-slate-400 border-slate-700 hover:bg-slate-700/50'"
                  class="px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors">
            {{ lang.t('alerts.read') }}
          </button>
        </div>
      ...
      @if (!loading() && alerts().length === 0) {
        <div class="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-12 text-center">
          ...
          <a routerLink="/alert-profile"
             class="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-medium bg-gradient-to-r from-emerald-500 to-cyan-500 text-white hover:from-emerald-400 hover:to-cyan-400 transition-all">
        ...
        <div class="space-y-3">
          @for (alert of filteredAlerts(); track alert.id) {
            <div (click)="toggleExpand(alert.id)"
                 class="bg-slate-800/50 border rounded-xl overflow-hidden cursor-pointer transition-all duration-200 hover:bg-slate-800/80"
                 [class]="alert.isRead ? 'border-slate-700/30' : 'border-l-2 ' + severityBorderClass(alert.severity) + ' border-slate-700/50'">
              <div class="px-5 py-4">
                  ...
                  <div class="flex items-center gap-2 shrink-0">
                    @if (!alert.isRead) {
                      <button (click)="markRead(alert, $event)"
                              ...
                              class="w-7 h-7 rounded-lg flex items-center justify-center text-slate-500 hover:text-emerald-400 hover:bg-slate-700/50 transition-colors">
              ...
              @if (expandedId().has(alert.id)) {
                <div class="border-t border-slate-700/30 px-5 py-4 bg-slate-900/30 space-y-4">
                  @if (alertDetails()[alert.id]; as detail) {
                    ...
                    @if (detail.message) {
                      <div>
                        ...
                        <p class="text-sm text-slate-300 leading-relaxed">{{ detail.message }}</p>
                      </div>
                    }
                    ...
                    @if (detail.impactAnalysis) {
                      <div>
                        ...
                        <p class="text-sm text-slate-300 leading-relaxed">{{ detail.impactAnalysis }}</p>
                      </div>
                    }
                        ...
                        <div>
                          ...
                          <div class="flex flex-wrap gap-1 mt-1">
                            @for (reg of detail.regulations.split(','); track reg) {
                              <span class="px-2 py-0.5 text-[10px] font-medium rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">{{ reg.trim() }}</span>
                            }
                          </div>
                        </div>
                      }
                    ...
                    <div class="flex items-center gap-2 py-2">
                      <div class="w-4 h-4 border-2 border-emerald-400/30 border-t-emerald-400 rounded-full animate-spin"></div>
      ...
      @if (!loading() && alerts().length > 0 && filteredAlerts().length === 0) {
        <div class="bg-slate-800/30 border border-slate-700/30 rounded-xl p-8 text-center">
