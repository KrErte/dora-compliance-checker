    ...
    <div class="space-y-8">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div class="bg-white border border-slate-200 rounded-xl p-4 text-center">
          <div class="text-2xl font-bold text-slate-900">{{ records().length }}</div>
      ...
      @for (record of records(); track record.id) {
        <div class="bg-white border border-slate-200 rounded-xl p-4 flex items-center gap-4">
          ...
          <div class="flex-1 min-w-0">
            <div class="text-sm text-slate-900 font-medium">{{ record.personName }}</div>
      ...
      @if (showForm) {
        <div class="fixed inset-0 bg-white/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" (click)="showForm = false">
          <div class="bg-white border border-slate-200 rounded-2xl p-6 w-full max-w-lg" (click)="$event.stopPropagation()">
            <h2 class="text-lg font-bold text-slate-900 mb-4">{{ lang.t('training.add_training_record') }}</h2>
            <div class="space-y-4">
              <input [(ngModel)]="newRecord.personName" type="text" [placeholder]="lang.t('training.person_name')" class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-slate-900 focus:outline-none">
              <select [(ngModel)]="newRecord.role" class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-slate-900 focus:outline-none">
              ...
              <input [(ngModel)]="newRecord.trainingTitle" type="text" [placeholder]="lang.t('training.training_title')" class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-slate-900 focus:outline-none">
              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label class="block text-xs text-slate-400 mb-1">{{ lang.t('training.completed_date') }}</label>
                  <input [(ngModel)]="newRecord.completedDate" type="date" class="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-900 text-sm focus:outline-none">
                </div>
                <div>
                  <label class="block text-xs text-slate-400 mb-1">{{ lang.t('training.next_due_date') }}</label>
                  <input [(ngModel)]="newRecord.nextDueDate" type="date" class="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-900 text-sm focus:outline-none">
                </div>
              </div>
