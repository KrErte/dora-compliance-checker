    ...
    <div class="min-h-[60vh] flex items-center justify-center p-4">
      ...
      @if (!loading() && verified()) {
        <div class="max-w-md w-full">
          <div class="bg-slate-800/50 border border-emerald-500/30 rounded-2xl overflow-hidden">
            <!-- Header -->
            <div class="bg-gradient-to-r from-emerald-500/20 to-cyan-500/20 border-b border-emerald-500/20 p-6 text-center">
              <div class="w-16 h-16 mx-auto mb-3 rounded-full bg-emerald-500/20 border-2 border-emerald-500 flex items-center justify-center">
              ...
              <p class="text-sm text-emerald-400 mt-1">Verified by DoraAudit.eu</p>
            </div>
            ...
            <div class="p-6 space-y-4">
              ...
              <div class="flex justify-between items-center py-2 border-b border-slate-700/50">
                ...
                <div class="flex items-center gap-2">
                  <div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold"
                       [class]="(data()?.score || 0) >= 80 ? 'bg-emerald-500/20 text-emerald-400' : (data()?.score || 0) >= 60 ? 'bg-amber-500/20 text-amber-400' : 'bg-red-500/20 text-red-400'">
                    {{ data()?.score }}%
                  </div>
                </div>
              </div>
              <div class="flex justify-between items-center py-2 border-b border-slate-700/50">
                ...
                <span class="text-sm text-slate-300">{{ data()?.assessmentDate }}</span>
              </div>
              <div class="flex justify-between items-center py-2">
                <span class="text-sm text-slate-400">{{ lang.l('Kehtiv kuni', 'Valid Until') }}</span>
                <span class="text-sm font-medium" [class]="data()?.expired ? 'text-red-400' : 'text-emerald-400'">
                  {{ data()?.validUntil }}
                  @if (data()?.expired) {
            ...
            <!-- Footer -->
            <div class="bg-slate-900/50 border-t border-slate-700/50 p-4 text-center">
              <a href="https://doraaudit.eu" target="_blank" class="text-xs text-slate-500 hover:text-cyan-400 transition-colors">
      ...
      @if (!loading() && !verified()) {
        <div class="max-w-md w-full text-center">
          <div class="bg-slate-800/50 border border-red-500/30 rounded-2xl p-8">
            ...
            <a routerLink="/" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-slate-700 text-slate-300 text-sm hover:bg-slate-600 transition-colors">
