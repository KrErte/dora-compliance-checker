      ...
      @if (showForm) {
        <div class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" (click)="showForm = false">
          <div class="bg-slate-800 border border-slate-700 rounded-2xl p-6 w-full max-w-lg" (click)="$event.stopPropagation()">
            ...
            <div class="space-y-4">
              <input [(ngModel)]="newStrategy.providerName" type="text" [placeholder]="lang.t('exitstrat.provider_name')" class="w-full px-4 py-2.5 bg-slate-900/50 border border-slate-600/50 rounded-xl text-white focus:outline-none">
              <div class="grid grid-cols-2 gap-4">
                <input [(ngModel)]="newStrategy.serviceType" type="text" [placeholder]="lang.t('exitstrat.service_type')" class="px-3 py-2 bg-slate-900/50 border border-slate-600/50 rounded-lg text-white text-sm focus:outline-none">
              ...
              <input [(ngModel)]="newStrategy.transitionPeriodMonths" type="number" [placeholder]="lang.t('exitstrat.transition_period_months')" class="w-full px-3 py-2 bg-slate-900/50 border border-slate-600/50 rounded-lg text-white text-sm focus:outline-none">
              <textarea [(ngModel)]="newStrategy.alternativeProviders" rows="2" [placeholder]="lang.t('exitstrat.alternative_providers_15')" class="w-full px-3 py-2 bg-slate-900/50 border border-slate-600/50 rounded-lg text-white text-sm focus:outline-none"></textarea>
              <textarea [(ngModel)]="newStrategy.keyRisks" rows="2" [placeholder]="lang.t('exitstrat.key_risks_16')" class="w-full px-3 py-2 bg-slate-900/50 border border-slate-600/50 rounded-lg text-white text-sm focus:outline-none"></textarea>
              <textarea [(ngModel)]="newStrategy.dataReturnPlan" rows="2" [placeholder]="lang.t('exitstrat.data_return_plan')" class="w-full px-3 py-2 bg-slate-900/50 border border-slate-600/50 rounded-lg text-white text-sm focus:outline-none"></textarea>
              <div class="flex justify-end gap-3">
                <button (click)="showForm = false" class="px-4 py-2 rounded-xl bg-slate-700/50 text-slate-300 text-sm">{{ lang.t('exitstrat.cancel') }}</button>
