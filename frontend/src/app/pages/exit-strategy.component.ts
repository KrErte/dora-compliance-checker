      ...
      @for (strategy of strategies(); track strategy.id) {
        <div class="bg-white border border-slate-200 rounded-xl p-5">
          <div class="flex items-start justify-between mb-3">
            <div>
              <h3 class="text-slate-900 font-semibold">{{ strategy.providerName }}</h3>
            ...
            <div class="flex gap-2">
              ...
              <span class="px-2 py-0.5 rounded text-[10px] font-bold"
                    [class]="strategy.status === 'TESTED' ? 'bg-blue-100 text-blue-600' : strategy.status === 'APPROVED' ? 'bg-blue-500/20 text-blue-400' : 'bg-slate-100 text-slate-600'">
                {{ strategy.status }}
              </span>
            </div>
          </div>
          ...
          <div class="flex gap-2 mt-3">
            <select [(ngModel)]="strategy.status" (ngModelChange)="save()" class="px-2 py-1 bg-white border border-slate-200 rounded-lg text-xs text-slate-900 focus:outline-none">
      ...
      @if (showForm) {
        <div class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" (click)="showForm = false">
          <div class="bg-white border border-slate-200 rounded-2xl p-6 w-full max-w-lg" (click)="$event.stopPropagation()">
            <h2 class="text-lg font-bold text-slate-900 mb-4">{{ lang.t('exitstrat.new_exit_strategy') }}</h2>
            <div class="space-y-4">
              <input [(ngModel)]="newStrategy.providerName" type="text" [placeholder]="lang.t('exitstrat.provider_name')" class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-slate-900 focus:outline-none">
              <div class="grid grid-cols-2 gap-4">
                <input [(ngModel)]="newStrategy.serviceType" type="text" [placeholder]="lang.t('exitstrat.service_type')" class="px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-900 text-sm focus:outline-none">
                <select [(ngModel)]="newStrategy.criticality" class="px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-900 text-sm focus:outline-none">
                  <option value="CRITICAL">Critical</option>
                  <option value="IMPORTANT">Important</option>
                  <option value="STANDARD">Standard</option>
                </select>
              </div>
              <input [(ngModel)]="newStrategy.transitionPeriodMonths" type="number" [placeholder]="lang.t('exitstrat.transition_period_months')" class="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-900 text-sm focus:outline-none">
              <textarea [(ngModel)]="newStrategy.alternativeProviders" rows="2" [placeholder]="lang.t('exitstrat.alternative_providers_15')" class="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-900 text-sm focus:outline-none"></textarea>
              <textarea [(ngModel)]="newStrategy.keyRisks" rows="2" [placeholder]="lang.t('exitstrat.key_risks_16')" class="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-900 text-sm focus:outline-none"></textarea>
              <textarea [(ngModel)]="newStrategy.dataReturnPlan" rows="2" [placeholder]="lang.t('exitstrat.data_return_plan')" class="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-900 text-sm focus:outline-none"></textarea>
              <div class="flex justify-end gap-3">
                <button (click)="showForm = false" class="px-4 py-2 rounded-xl bg-slate-100 text-slate-600 text-sm">{{ lang.t('exitstrat.cancel') }}</button>
