      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        @if (notifications().length > 0) {
          <button (click)="markAllRead()"
                  class="text-sm text-slate-400 hover:text-blue-600 transition-colors px-4 py-2 rounded-lg border border-slate-200 hover:border-blue-200 hover:bg-blue-50">
      ...
      @if (!loading()) {
        ...
        @if (activeTab() === 'history') {
          ...
          @for (notif of notifications(); track notif.id) {
            <div class="bg-white border border-slate-200 rounded-xl p-4 transition-all"
                ...
                <div class="flex items-center gap-2 flex-shrink-0">
                  @if (notif.link) {
                    <a [routerLink]="notif.link"
                       class="text-xs text-slate-400 hover:text-blue-600 px-2.5 py-1.5 rounded-lg hover:bg-slate-100 transition-all">
                      {{ lang.t('notifications.view') }}
                    </a>
                  }
                  @if (!notif.read) {
