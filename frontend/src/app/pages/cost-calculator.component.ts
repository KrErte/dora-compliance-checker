...
export class CostCalculatorComponent {
  ...
  async copyToClipboard(): Promise<void> {
    ...
    const sizeLabel: Record<string, string> = {
      small: isEt ? 'Väike' : 'Small',
      medium: isEt ? 'Keskmine' : 'Medium',
      large: isEt ? 'Suur' : 'Large',
      enterprise: isEt ? 'Suurettevõte' : 'Enterprise'
    };
    const sectorLabel: Record<string, string> = {
      banking: isEt ? 'Pangandus' : 'Banking',
      insurance: isEt ? 'Kindlustus' : 'Insurance',
      investment: isEt ? 'Investeerimine' : 'Investment',
      payment: isEt ? 'Makseteenused' : 'Payment',
      fund: isEt ? 'Fondihaldus' : 'Fund Management',
      crypto: isEt ? 'Krüptovarad' : 'Crypto'
    };

    const lines = [
      isEt ? '=== DORA VASTAVUSE KULUKALKULAATOR ===' : '=== DORA COMPLIANCE COST CALCULATOR ===',
      isEt ? `Ettevõtte suurus: ${sizeLabel[this.companySize] || this.companySize}` : `Company size: ${sizeLabel[this.companySize] || this.companySize}`,
      isEt ? `Sektor: ${sectorLabel[this.sector] || this.sector}` : `Sector: ${sectorLabel[this.sector] || this.sector}`,
      isEt ? `Aastane käive: EUR ${this.formatNumber(this.revenue)}` : `Annual revenue: EUR ${this.formatNumber(this.revenue)}`,
      isEt ? `IKT teenusepakkujaid: ${this.providerCount}` : `ICT providers: ${this.providerCount}`,
      '',
      isEt ? '--- Kulude jaotus ---' : '--- Cost Breakdown ---',
      ...this.result.lines.map(l =>
        `${isEt ? l.labelEt : l.labelEn}: EUR ${this.formatNumber(l.amount)}`
      ),
      '',
      isEt ? `Ühekordne investeering: EUR ${this.formatNumber(this.result.totalAfterDiscount)}` : `One-time investment: EUR ${this.formatNumber(this.result.totalAfterDiscount)}`,
      isEt ? `Aastane kulu: EUR ${this.formatNumber(this.result.annualAfterDiscount)}` : `Annual cost: EUR ${this.formatNumber(this.result.annualAfterDiscount)}`,
      isEt ? `Kulu töötaja kohta: EUR ${this.formatNumber(this.result.costPerEmployee)}` : `Cost per employee: EUR ${this.formatNumber(this.result.costPerEmployee)}`,
      '',
      isEt ? `Potentsiaalne trahv (DORA Art. 50-51): EUR ${this.formatNumber(this.result.potentialFine)}` : `Potential fine (DORA Art. 50-51): EUR ${this.formatNumber(this.result.potentialFine)}`,
      isEt ? `ROI kordaja: ${this.getRoiMultiple()}x` : `ROI multiple: ${this.getRoiMultiple()}x`,
      isEt ? `Tasuvusperiood: ${this.result.breakEvenMonths} kuud` : `Break-even period: ${this.result.breakEvenMonths} months`,
      '',
      isEt ? 'Genereeritud: DoraAudit.eu' : 'Generated by: DoraAudit.eu'
    ];
